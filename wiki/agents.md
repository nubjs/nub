...
- Never open a pull request, issue or comment against `pnpm/pnpm`, and never mention Nub's engine plans on GitHub, without the maintainer's explicit, in-the-moment instruction. Pushing branches to the `nubjs/pnpm` fork is normal work. ([The pnpm engine fork and pin](#the-pnpm-engine-fork-and-pin))
...
- **Lockfile compatibility follows project identity, a separate axis from CLI-compat.** A pnpm project reads and writes `pnpm-lock.yaml` as pnpm 12 does, checked in both directions against real pnpm by `.github/workflows/lockfile-roundtrip.yml`; a nub-identity project writes `nub.lock`. No install reads an npm, Yarn or Bun lockfile — `nub pm migrate` converts one, once (`crates/nub-cli/src/pm_engine/migrate.rs`). "pnpm-only" governs the CLI grammar; the lockfile axis is pnpm or Nub.
- **No Yarn lockfile is written.** `yarn.lock` is input to `nub pm migrate` and nothing else, and `nub pm use yarn` refuses. Superseded: the 2026-06-27 reversal that put Yarn berry `yarn.lock` + `.pnp.cjs` writes in scope.
- **The PM is pnpm 12's Rust engine, embedded as a library** — seven git dependencies on the fork `nubjs/pnpm`, pinned by one `rev` in `crates/nub-cli/Cargo.toml`, called in-process, no subprocess. No engine source lives in this tree: an engine change is a commit on the fork's `nub-embedder` branch, then a pin move in nub. **Never open a pull request, issue or comment against `pnpm/pnpm`, and never mention Nub's engine plans on GitHub, without the maintainer's explicit, in-the-moment instruction.** Full detail: [The pnpm engine fork and pin](#the-pnpm-engine-fork-and-pin).
- **Config identity model:** a **nub-identity** project consumes ONLY neutral cross-tool config (`overrides`, `resolutions`, `catalog`, `workspaces`) — never another PM's branded fields or files. A **pnpm** project (`packageManager: pnpm@…`, `pnpm-lock.yaml` or `pnpm-workspace.yaml`) is mirrored faithfully, branded fields included. There is no third identity: npm, Yarn and Bun markers confer none (`crates/nub-cli/src/pm_engine/project_identity.rs`). pnpm-NAMED files are never read unless pnpm is the incumbent.
...
- **The compat target is pnpm 12.** A pnpm project runs the embedded engine under pnpm's own profile and behaves as pnpm 12 does (`crates/nub-cli/src/pm_engine/project_identity.rs`). A `packageManager` pin naming another pnpm version is honored the way pnpm honors it: Nub provisions that pnpm and delegates the command to it. There is no per-major model beyond that, and no npm, Yarn or Bun target. Superseded: the 2026-06-20 per-major map (npm, pnpm 9/10/11, yarn 1, yarn 2/3/4, bun).
...
The Lat check runs on main pushes and on pull requests against `main` when the `ci` label requests a run. It fails on broken wiki links, missing `@lat:` targets, invalid section summaries, or incomplete directory indexes. The repo root carries a `lat.md` symlink pointing at `wiki/`, because Lat finds its graph by that directory name.
...
- **Read the graph before designing anything non-trivial.** Use the `lat_search` and `lat_section` MCP tools, or `nub run lat search "<question>"` from the repo root. Both Claude and Codex have the Lat server and a prompt reminder configured. Run `nub run lat:index` once per checkout to build the local embedding index; subsequent searches refresh changed sections. No global `lat` install or API key is needed. The `lat-md` skill covers setup, navigation, and recovery.
- **Update the graph in the same commit as the behavior.** A doc naming a symbol you just renamed now fails a gate instead of rotting quietly. Three run it for you, cheapest first: `.githooks/pre-commit` when the staged changes touch `wiki/` or the lat skill, `.githooks/pre-push` on every push, and the `lat-check` CI job. Run `nub run lat:check` yourself to see it sooner. Both hooks warn rather than block if the checker cannot run, and both take `NUB_SKIP_LAT_CHECK=1`.
...
- **Engine-branded env vars are not Nub's surface.** Distinct from the exemption above: that covers vars nub *sets* for its children; this covers vars the runtime *reads* to decide behavior, which are user-facing knobs — and in a nub-identity project the user sees Nub, with the engine an invisible implementation detail. There the profile reads none of pnpm's configuration files, and the settings the engine receives come from `nub.jsonc`, `.npmrc`, `npm_config_*` and `NUB_CACHE_DIR` (`crates/nub-cli/src/pm_engine/host_settings.rs`). Preferred for a new knob: a **neutral npmrc field** where one reads naturally (also reachable as `npm_config_<field>`), else a **`NUB_*`** env var or nub-namespaced config (`NUB_CACHE_DIR`). Two disciplines: keep the exposed set as SMALL as possible, and design each knob to GENERALIZE — a *duration*, never an on/off boolean. A pnpm project is configured the way pnpm 12 configures it.
...
- **Error codes are internal — don't brand-sweep them.** `ERR_NUB_*` / `ERR_PNPM_*` / `WARN_*` identifiers are mechanism. Adding a missing code is good UX; rebranding an existing one for purity is not required.
...
nub's package manager IS pnpm 12's Rust engine, consumed from the fork `nubjs/pnpm`. Ground every engine claim here or in the code — never in memory.

- **Embed, not subprocess.** `crates/nub-cli/Cargo.toml` takes seven git dependencies on `https://github.com/nubjs/pnpm` — `pnpm-cli`, `pnpm-config`, `pnpm-diagnostics`, `pnpm-lockfile`, `pnpm-package-manager`, `pnpm-store-dir`, `pnpm-workspace` — all pinned to one `rev`. nub has its own CLI (clap dispatch + `pm_engine::ENGINE_VERBS`) and hands the engine an argv through `pnpm_cli::run(argv, embedder)` in-process (`crates/nub-cli/src/pm_engine/pnpm_engine.rs`). No engine source lives in this tree, and only the root `Cargo.lock` records it.
- **Two profiles, chosen by project identity** (`crates/nub-cli/src/pm_engine/project_identity.rs`). A pnpm project runs under `Embedder::PNPM` and gets pnpm 12's output unchanged, `ERR_PNPM_*` codes included. Every other project runs under nub's `NUB` profile. For those, `rebrand` in `pnpm_engine.rs` rewrites what the profile cannot reach — `ERR_PNPM_*`→`ERR_NUB_*`, `WARN_PNPM_*`→`WARN_NUB_*`, `Usage: pnpm`, and the commands a report tells the user to run — and `present.rs` is only a credential scrub. Consequence when asserting on output: the spelling follows the fixture's identity, so grepping a nub-identity project's stderr for `ERR_PNPM_*` silently finds nothing. Match `(PNPM|NUB)` where a test does not pin the identity.
- **Fork vs upstream.** `nubjs/pnpm` is a GitHub fork of `pnpm/pnpm`. Its `nub-embedder` branch is a pnpm 12 release (v12.4.1 at the current pin) plus the embedder patches nub needs. The branch is where engine source lives; nub `main` records which commit it builds through the `rev` pin.
- **Never engage `pnpm/pnpm` without the maintainer's explicit, in-the-moment instruction** — no pull request, issue or comment against `pnpm/pnpm`, and no mention of Nub's engine plans anywhere on GitHub. Pushing branches to `nubjs/pnpm` is normal work.
- **The embedder profile** is compile-time pluggability: `pnpm_config::Embedder`, a struct the host fills. nub's `NUB` sets `program_name: "nub"`, `lockfile_basename: "nub.lock"`, the `.store` virtual-store directory, `reads_pnpm_config: false`, `writes_settings_file: false`, `reads_npm_config_env: true`, `manage_package_manager_versions: false`, and host hooks for `allowBuilds`, overrides and the phantom-dependency scan — this is what makes the brand and config boundary hold.
- **Stale-doc warning:** `internal/architecture.md`'s "no own package manager" line and the augmenter/fork section's Node-only framing predate the embedded PM. Trust this section and the code.

### The fork-and-pin workflow

An engine change is two commits in two repositories: the change on the fork, then the pin move in nub.

**(1) Change engine code on `nubjs/pnpm`.** Commit on `nub-embedder` in a clone of the fork and push it. Run the touched crate's tests there (`cargo test -p <crate>`; the fork's `justfile` `test` recipe runs the whole suite) — nub's root `cargo test` never builds a git dependency's test targets. The commit must be on GitHub before nub can pin it, because Cargo fetches a git dependency from its URL.

**(2) Move the pin in nub.** Set the same new `rev` on all seven engine dependencies in `crates/nub-cli/Cargo.toml`, refresh the root `Cargo.lock`, and confirm every tracked lock is still `--locked`-clean (the recipe in step 2a of the [pre-push loop](#implementation-quality-discipline)). The move touches only those two files. Name the fork commit in the subject, as in `pm: move the engine pin to nubjs/pnpm 47120fe`, say in the body what that commit changes, and land it like any other code change.

**(3) Take a new pnpm release** — infrequent and deliberate. Bring the release into `nub-embedder` under the embedder patches, run the engine tests on the fork, then move the pin as in (2). Keep the patch set thin so this stays cheap.
...
- `RUST_LOG=pacquet::install::phase=info nub install` prints one `INFO phase complete phase=<name> elapsed_ms=<n>` line per engine phase (`resolve_workspace`, `create_virtual_store`, `link.*`, `apply.*`, …) — the coarse resolve/link split. `RUST_LOG` is the switch because nub installs its own tracing subscriber before the engine can (`crates/nub-cli/src/pm_engine/log.rs`); the engine's `TRACE` variable does nothing under nub.
- Adding `pacquet::package_import_method=info` prints `INFO selected package import method method=<clone|hardlink|copy>` once for each method the install used — which link tier ran, not a per-file tally. The `create_virtual_store_partition` phase line carries warm / cold / skipped slot counts and the linker.
- Judge by **those counts plus an A/B ratio** (the default isolated linker, `NodeLinker::Isolated` in the engine's `pnpm-config` crate, vs `--node-linker hoisted`), never a contended-host absolute. Always run a verified-clean warm `--offline` loop with rc=0.
...
- **Verify locally before pushing — don't outsource verification to CI.** A requested run is ~5 workflows × the 8-platform matrix (~30–40 min each), and several pull requests asking for round after round saturate the runner pool so head-commit jobs queue for hours. Get it green locally, then ask for ONE run ([PR CI is opt-in](#pr-ci-is-opt-in--ask-for-a-run-with-the-ci-label)). The loop, in your worktree:
  ...
  2a. **The root workspace is not the whole repo.** `crates/nub-native` is its OWN workspace — a cdylib loaded into the user's Node process needs `panic = "unwind"`, which cannot coexist with the root release profile's `panic = "abort"` — so `-p nub-native` from the root fails with `package ID specification … did not match any packages`. That is the workspace boundary, not a typo. Run its gates from inside the crate, as CI does, and note `cargo fmt --check` is per-workspace too:
      ...
      **There are FOUR tracked lockfiles, and enumerating them from the `--locked` gates undercounts.** `git ls-files '*/Cargo.lock' 'Cargo.lock'` is the honest list: the root, `crates/nub-native`, `crates/nub-launcher` and `crates/nub-phantom`. `crates/nub-phantom` is the one that gets missed, because until 2026-09-02 NO gate passed `--locked` for it — its only CI step re-resolved silently, so its lock could describe a dependency graph that no longer existed and every check stayed green. Corrected that day, after a `flate2` backend swap left it naming a crate the manifest no longer selected and `cargo metadata --locked` exited 101 with nothing in CI to say so. Check the list, not the gates; a workspace with no gate is exactly the one that rots.
      ...
      ```sh
      for d in . crates/nub-native crates/nub-launcher crates/nub-phantom; do
      ...
      **The engine's crates are git dependencies, and only the root lock records them.** A `rev` move rewrites the `source = "git+https://github.com/nubjs/pnpm?rev=…"` entries in the root `Cargo.lock` and in no other lock. The `--offline` loop above cannot fetch a commit Cargo has never seen, so run the root `cargo metadata` once without `--offline` after moving the pin. The engine's tests never run from nub — `--all-targets` from the root never builds a dependency's test targets — so a test for engine code belongs on the fork beside the code it guards; put it nub-side only when it needs the `nub` binary.
