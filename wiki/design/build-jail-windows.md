...
**Why this row matters beyond itself.** It is what refuted the earlier claim that the two halves are mutually exclusive. `build_jail_net()` returns `json!(false)` on Windows (`compiler/preset.rs:482-490`), so the Windows build jail is deny-all by construction.
...
**It is the ONLY enforcement path the `packageNetwork` table has, on any platform.** The shim's JS branches on nothing OS-specific and its tests run everywhere, but the only thing that reads the table is `net_gate_node_options` (`compiler/defaults.rs:929`), and the only thing that stamps that term is the Windows branch of `build_jail.rs`. So **package identity gates egress here and nowhere else**: Linux gets a flat seccomp family denial and macOS a flat `$downloads` host allowlist, neither of which consults the package. Read "the catalog admitted this package to the network" as a Windows-only behaviour until another platform stamps the gate.
