...
**The effective-rights API is no longer used, because it cannot evaluate ordinary DACLs at all.** It returns `ERROR_INVALID_ACL` (1336) on two legal ACE arrangements — a deny ACE after an allow ACE, and explicit/inherited allow ACEs alternating past ~3 pairs — which made the build jail refuse **every** working root on a profile carrying either (552 real directories under one `%LOCALAPPDATA%\nub`). `verify_clean_root` now walks the ACEs directly. Full measurement, including the `EIEIEI`-fails / `EEEIII`-passes control and the refuted hypotheses: [`../research/geteffectiverightsfromacl-invalid-acl.md`](../research/geteffectiverightsfromacl-invalid-acl.md).
