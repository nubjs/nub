...
⛔ **But the grep has a failure mode, and it is the one to internalise: FINDING A DISCUSSION IS NOT FINDING A DECISION.** A comment that criticises something tells you a person thought about it. Whether that thought *entails the state you are looking at* is a separate question, and assuming it does is how a stale comment gets promoted into a design rationale it never was.

**Worked example — the same absence read two ways, and the first reading was wrong.** v2 records all carry `"nubGitSha": null`. Grepping the name lands on `measure.sh:183`: *"WHICH BINARY, NOT WHICH COMMIT — `nubGitSha` PROVABLY CANNOT ANSWER THIS,"* citing a measured case where a build missing only `build-jail-catalog-override` voided four cells while recording a sha identical to the working binary's. That reads like a decision to retire the field. **It is not one.** Read to the end and the same comment says a feature list belongs ***beside*** `nubGitSha` — an argument to ADD, which presumes the sha stays.

**What actually produces the null is a wiring path, and a control identifies it in one step.** `record.mjs` takes the value from `--nub-sha`, which `run-batch-v2.mjs` does pass — sourced from `process.env.NUB_GIT_SHA`. Its two siblings in the very same call, `corpusGitSha` and `nubVersion`, are computed IN-PROCESS and both land in every record; only the one read from the environment is empty. **Two of three values populated, and the odd one out is the only one crossing a process boundary — that is a wiring signature, not a design choice.** Upstream, the binary is restored from a cache keyed `nub-bin-v1-<os>-${inputs.nub_sha}` with a bare `nub-bin-v1-<os>-` restore-key, so an empty input prefix-matches ANY previously cached binary; the step that computes a real sha is skipped on a cache hit, and the step that runs instead echoes the empty input. **The null is HONEST — the pipeline genuinely cannot name what it restored.**

⇒ So the fix is neither "restore the field" nor "leave it alone": it is to make the cache key identify the artifact, and to record the feature list the comment already asked for. MEASURED: 0 of 10 sampled v2 records carry `nubBinary`, `contentHash`, `features` or any variant, while `storeLayout` and `venue` populate via the same probe.

⛔ **And note the shape of the error, because it is the same one §14 documents, committed one level up.** §14's case was a criticism of the ladder *as a primary method* misread as a decision against it *as a fallback*. This was a criticism of `nubGitSha` *as insufficient* misread as a decision to *null* it. **A criticism of X is not a decision about X's existence** — check that the argument you found actually reaches the state in front of you, and prefer a control over a comment when one is available. Here the control took one command and the comment took three wrong turns.
