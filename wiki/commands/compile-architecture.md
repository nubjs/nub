...
4. `acquire_node` extracts or provisions Node when no external `--smol` Node was found. `ensure_app` extracts the bundled entry and assets. The staged app tree is flushed and validated against the payload's names, recorded lengths and executable bits before atomic publication. A warm run checks the private app directory and its parent, the completion marker, and the entry and bootstrap files without enumerating the tree. Missing markers, entries or bootstraps trigger re-extraction; changes to other files within a completed cache do not. Removing the marker or the extraction directory requests repair. The embedded Node retains its executable and length checks.
