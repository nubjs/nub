...
Companion to [`cross-runtime-compat-benchmark.md`](./cross-runtime-compat-benchmark.md). That doc is the measurement (Bun 1.4, the skip-pass census, the 70-regression count). This doc is the **diagnosis**: the 70 confirmed nub-vs-node regressions collapse into **9 root causes**, and this records what each one is, **when it landed**, whether it is a real bug, and the recommendation.

## Read this first — three things that were getting conflated

1. **The benchmark introduced NOTHING.** Running Deno's corpus against nub is a *measurement* — it changed zero runtime code. Verified: no `runtime/`, `crates/nub-core/src/node/`, or `crates/nub-native/` commit landed during the benchmark window (2026-07-22/23); the only code commits nearby are the v0.5.0 release train from 2026-07-21, before the benchmark ran. So none of these bugs were "introduced by the benchmark" or "since the benchmark."

2. **"Regression" here means: grew relative to the June measurement, not caused by the benchmark.** The June benchmark (nub v0.0.49, ~2026-06-16) counted ~53–57 nub-vs-node regressions. The July benchmark (nub v0.5.0) counts 70. The delta is **normal v0.1→v0.5 feature development** adding augmentation surface between the two measurements — every offending line below landed between 2026-06-03 and 2026-07-09, i.e. *before* the July benchmark, during ordinary development. The benchmark is what *surfaced* them by re-measuring.

3. **The "revert" is a SEPARATE event from any regression.** On 2026-07-23 a fix I had applied to `runtime/polyfills.cjs` (and, later, the process-group guard in `crates/nub-cli/tests/node_compat.rs`) was discarded from the working tree — no git-reflog entry, not in any stash, sibling edit `flags.rs` untouched. That signature is a **per-file `git restore` / editor "Discard Changes"**, not an automation and not a code regression. It removed a *fix in progress*, not introduced a bug. Don't confuse "the fix got reverted" (an editing accident on 07-23) with "the bug regressed" (feature work over June–July).
...
- **VERDICT — accept, no fix (2026-07-24).** The bug is narrow: only a bare `assert.ok(false)`/`assert(false)` with **no message** (asserts with a message, `strictEqual`, plain throws are all fine). A 26.x-wide gate was briefly landed then **dropped** — disabling TS stack-trace remapping for every Node-26 user to fix that one edge case (trivial workaround: pass a message) was the wrong trade. Not documented, not filed upstream, by decision. `source_maps_safe` stays as-is (26.2-only, unrelated).
...
- **What breaks:** nub injects `--disable-warning=ExperimentalWarning` to mute warnings for the flags *it* injects. But it rides `NODE_OPTIONS` into every child, so a `node` child the user spawns has its own legitimate experimental warnings suppressed too. (I verified: the failing subtest expects a child's `--experimental-loader` warning and gets empty stderr.)
- **Landed:** ~2026-07-09 (#398, the flag-intersect refactor). Between benchmarks.
- **Severity:** Med — and it's a judgment call: the same flag legitimately mutes nub's own noise.
- **Recommendation:** replace the blunt flag with a selective preload-side `warning` listener that drops only warnings for features **nub itself unflagged**, delegating the rest. Also: two config ignore-entries (`test-process-warnings`, `test-domain-multi`) currently blame "webstorage ExperimentalWarning" — both reasons are factually wrong and should be corrected.

### Cause 7 — Transpiled TypeScript gets a bare-path `sourceURL`
- **Tests:** ~2 (typescript-coverage; contributes to test-runner-coverage).
- **What breaks:** `crates/nub-native/src/cache.rs:135` emits `//# sourceURL=<path>` (a filesystem path) instead of a `file://` URL. Node's coverage collector skips any URL not starting with `file:`, so **`nub --test --experimental-test-coverage` on any TypeScript project reports an empty coverage table and a fake 100%** (I verified end-to-end — node shows `lib.ts | 100 | 100 | 50.00`, nub drops the row entirely).
...
- **What happens:** `crates/nub-cli/src/cli.rs` runs `expand_env_map` on the explicit `--env-file` map. Node keeps values literal; nub expands `$VAR`, so `PW="p@ss$WORD{x}"` → `p@ss{x}`.
- **Landed:** 2026-06-27 (#207/#214). A deliberate DX choice.
- **VERDICT — intentional divergence, keep it (2026-07-24).** Measured: node keeps `--env-file` literal, but **bun expands identically** (`p@ss{x}`), and nub's compat bar for this DX surface is bun-parity, not Node-parity. So `test-dotenv` is an *intentional* divergence, not a bug — mark it `ignore` in `node-compat-config.jsonc` with that reason.
- **Correction:** an earlier revision of this doc rated this "High — silent secret corruption, fix recommended." That over-stated it: it's a deliberate, bun-matching DX feature, and it only triggers on an **explicit** `--env-file`.
- **Verified NOT a `--node` auto-load bug:** nub does **not** auto-load `.env` in a plain file run — `nub script.js`, `nub --node script.js`, and plain `node script.js` all leave a cwd `.env` unread (only an explicit `--env-file` loads it). By contrast bun *does* auto-load `.env`; nub is the more conservative of the two here.
...
- The 12 tests gated on `--experimental-eventsource` (module-hooks + http-parser) — plain Node + the flag fails identically, so within the augmenter contract; `EventSource` is non-enumerable so real app code never trips it. **Your posture call:** accept-as-divergence (ignore-list) or author a lazy EventSource constructor.

## The durable guard (independent of every fix above)
nub already has a compat gate — `crates/nub-cli/tests/node_compat.rs` over `tests/node-compat-config.jsonc` — but its **CI job was deleted 2026-06-03 and never restored**, so there's been zero automated compat signal for the whole June→July window in which these grew. It was pulled because the harness killed the CI runner by orphaning grandchildren of timed-out tests. The fix (process-group spawn + reap, ported from `run.mjs`) is written and verified; restoring the CI job on top of it is the actual "prevent future regressions" answer — bigger than any single adopted test.
