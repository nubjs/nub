...
- **FIXED 2026-08-27 by [#800](https://github.com/nubjs/nub/pull/800).** The emitter writes the `file:` URL, spelled to match the host Node's own `pathToFileURL`: `nub-cache-key::source_url` derives the spelling and the post-transform step in `crates/nub-native/src/cache.rs` calls it. The inspector symptom is pinned by a regression test that asserts the reported script URL against the host's own `pathToFileURL` for the same file; the coverage symptom was re-verified by hand against the fixed build (the table matches node's exactly) but carries no committed test.
