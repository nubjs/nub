...
A prior agent claimed the `port.start()` docs "APOLOGIZE" for a "dual-contract WART" — specifically an **auto-start asymmetry** where `.on('message')` (EventEmitter) auto-starts the port but `addEventListener('message')` (EventTarget) does NOT (you'd need `port.start()` or `onmessage`). Verified from primary sources + empirically; the lead is **partly right, partly wrong**.
...
- **It is a MessagePort concern, not a Worker-handle concern.** The `start()`/auto-start mechanics live on `MessagePort` (from `MessageChannel` or transferred ports). The `Worker` handle (`extends EventEmitter`, no web counterpart) has no `start()` — so this does not bear on the global-`Worker` constructor decision. It only matters if/when nub exposes `MessagePort`/`MessageChannel` (tracked as an open follow-up in `wiki/runtime/web-worker.md`).
