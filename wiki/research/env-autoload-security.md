...
- **Blocked by:** `--node`, `NODE_COMPAT=1`, no `package.json`. Running `nub run <script>` is also transitively safe (Nub pre-sets `NODE_OPTIONS`, so the inner `node`'s auto-load skips the hostile value under shell-wins).
...
- **Recommendation:** ship a dangerous-variable DENYLIST (`NODE_OPTIONS` at minimum) as the floor — cheap, closes the RCE escalation, and independent of the larger question of whether committed mode files should be auto-loaded at all.
...
**RCE fired.** The printed `NODE_OPTIONS` is ONLY the attacker's value — Nub's own preload token is gone. The `.env` value did not merely append; it fully clobbered Nub's `NODE_OPTIONS`. Augmentation broke as a side effect, but the RCE fires regardless (Nub's preload is not what triggers it).
