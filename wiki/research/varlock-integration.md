...
> **REVERSAL (2026-08-02).** The in-process design this doc argues for through §"FINAL DESIGN" was
> built, then abandoned. nub now spawns `varlock run --path <schema-dir> -- <node> …` and resolves
> nothing itself. Sections below that describe priming a graph in Rust, `init-server`, the
> install-shape split, or nub-side redaction are **historical**. The reasoning that survives is in
> §"Why nub cannot keep its own cascade", both defect sections, and §"Redaction: nub does NOT match
> varlock run". See §"IMPLEMENTED DESIGN" immediately below for what shipped.
...
2. **`nub.jsonc` `preload: ["varlock/auto-load"]` is an infinite fork bomb.** nub compiles `preload` into `NODE_OPTIONS`, which every Node descendant inherits; varlock's auto-load `execSync`s the varlock CLI, which is itself Node, which re-preloads auto-load, forever.
...
Colin's call, in his own words: *"We don't want a fucking fallback. We also wanna have parity here
with what Varlok users expect. And Varlok only truly runs to its maximum capability when you invoke
it with `varlock run`. Which is why I think we should have Nub spawn varlock spawn node."* And:
*"The whole point is to defer everything environment-related to Varlok."*

`SpawnConfig::env_owner` is `Option<(cli, schema_dir)>`. When set, `spawn_node` builds the same Node
command it always would — every flag, the whole `NODE_OPTIONS` augmentation chain — and puts the
loader in front of it. `arg0("node")` is skipped there, since the process nub launches IS the loader.

Why this beats the in-process design it replaced:

- **Redaction actually works.** In-process patches cover `console` only. `varlock run` pipes the
  stream, so raw `process.stdout.write` and subprocess output are covered too (measured; see
  §"Redaction").
- **No partial product.** The install-shape split (importable vs standalone binary) collapses —
  `node_modules/.bin/varlock` exists in every install shape, verified against a real
  `npm install varlock@1.16.0`.
...
Three things that are non-obvious and were each found by running it, not reading it:

1. **`nub run`, `nubx`, `nub watch` and lifecycle scripts need no wrapping code.** Their `node`
   resolves through nub's PATH shim, re-enters nub, and gets wrapped there. This deleted the
   marker-stamping-across-five-launch-paths problem wholesale.
2. **`__NUB_ENV_OWNER_WRAPPED` must be nub's OWN marker, never `__VARLOCK_RUN`.** Keying the guard
   off varlock's variable would make a user's `varlock run -- nub` in a *different* directory
   silently serve the outer project's environment. Measured; see §"varlock's env vars".
3. **`--path` is load-bearing.** Without it a workspace member dies with `No .env files found in
   …/pkgs/web` against a schema nub had just found at the root. nub walked up to decide the loader
   owns the project; withholding where it walked to is incoherent. Also fixes `cd src && nub app.js`.
...
Varlock does not work that way. The schema's `@currentEnv=$X` root decorator names *which variable* selects the environment, and it is authoritative — `varlock run` has no `--env` flag at all, and where `--env` exists (`load`, `explain`, `reveal`) it is only a fallback that `@currentEnv` overrides.
...
nub keys on `APP_ENV`, never sees `MY_STAGE`, and so never loads `.env.production`. It fails silently. Any design where nub keeps resolving env files itself reproduces this class of bug, which is exactly the author's point: the schema describes a *graph* with its own resolution rules, not a file list.
...
It is also load-bearing in the other direction, so it cannot simply be removed. `nub run <script>` launches a shell, which launches Node; the preload reaches that Node *only* through `NODE_OPTIONS`. Switching `preload` to argv would fix the fork bomb but break `nub run` script coverage — verified working today via the inheritance path. Any fix has to keep script coverage.

### Hijacked `node` descendants are NOT run in compat mode

A natural assumption, and it is wrong — worth stating because it changes the fork-bomb analysis. A descendant that resolves `node` through nub's PATH shim is **fully augmented**, not compat. Measured, under `nub probe.mjs` with `preload: ["./noop.mjs"]`, spawning bare `node` (so PATH resolves the shim):

```json
{"viaPathShim":true,"nubVersion":"0.6.0","noopRan":true,"hasNubPreload":true,"hasUserPreload":true}
```

The child reports `process.versions.nub`, ran the user preload, and carries both preload tokens. What re-entrancy detection suppresses is a **second augmentation pass** (`is_reentrant` → skip re-injecting), not augmentation itself — the child stays augmented by inheritance. Compat mode is entered only by explicit `--node` / `NODE_COMPAT=1`, which does `restore_compat_environment` ([`spawn.rs:816`](../../crates/nub-core/src/node/spawn.rs)) and genuinely un-augments.

### nub's env loading builds a CHILD env map — there is nothing to "undo"

`merge_child_env` ([`cli.rs:242`](../../crates/nub-cli/src/cli.rs)) returns a `HashMap` that is applied to the spawned command's environment. nub's `.env` cascade **never mutates nub's own process env**.

This matters for the design: detecting `.env.schema` is a `stat` at the project root, and it can run *before* the cascade. So nub can simply decline to load, rather than loading and then reversing it. The "undo what we already did" problem only exists if the decision has to be made in JS, after the child has started with values already in `process.env`.

## Defect 2 (varlock): `execSyncVarlock` does not scrub `NODE_OPTIONS`

`varlock/auto-load` is a synchronous *subprocess wrapper* by design — the graph resolves asynchronously, and top-level await causes ESM hoisting-order problems, so it is forced through `execSync` (`src/auto-load.ts:13-16`, `:63`):

```ts
const { stdout } = execSyncVarlock('load --format json-full --compact', { … });
```

`lib/exec-sync-varlock.ts` passes the parent environment through wholesale and never strips `NODE_OPTIONS`. The spawned CLI is a Node process, so it inherits `--import varlock/auto-load` and recurses. Nothing breaks the cycle: the `__VARLOCK_ENV` reuse fast-path is only populated *after* `execSync` returns, so every level re-resolves from scratch.
...
Worth reporting upstream. It will bite any tool that does env-based preloading.

### There are TWO recursion channels, not one

This is the part that makes user-space workarounds fragile, and it is easy to miss.
...
Maintainer's call, and it is the right shape. nub does **not** load varlock, vendor it, or resolve the graph. It does two things:
...
Rationale for keeping it this thin: nub is expected to grow its own schema equivalent, so deep varlock coupling is a liability. This design couples nub to exactly two strings — the filename `.env.schema` and the sentinel `__VARLOCK_ENV` — and to zero varlock APIs.
...
Measured startup order is `--require` → each `--import` in argv order → entry module. nub's own preload is the fast-tier `--require`, so it runs **before** any user `--import`. Three placements and their outcomes:
...
nub controls `NODE_OPTIONS` token order, so appending its check token after the user's `preload` entries is straightforward and deterministic.
...
Same detection, but instead of only warning, nub appends its own preload module that calls varlock's in-process API. The user's only obligation is `npm install varlock` — nub vendors nothing and calls two public functions.
...
**This variant fixes the workspace case, which no other design does.** Because nub owns the load, it can work around varlock's cwd-only discovery by hopping cwd for the duration of the call. Verified: a member directory correctly picks up the root `.env.schema`, and cwd is restored before user code runs.
...
nub is expected to grow its own schema-driven env layer, so this integration should be built assuming the loader gets swapped. Keep the split explicit:
...
- The preload slot is a **pluggable loader module**. Which module fills it is a detail.
- The post-preload verification step — *did anything actually load?* — is loader-agnostic.
...
Note also that a native nub equivalent needs a Rust `@env-spec` parser, and none exists today — varlock's is Peggy/JS, and the monogram branch is a draft that emits TypeScript. That gap, not the integration work, is what gates a replacement.
...
`auto-load` installs the console / ServerResponse / Response guards, applies `@encryptInjectedEnv`, and strips `@internal` keys — all gated on the schema's own settings. nub cannot preload it alone, because it resolves its graph through a CLI subprocess whose `#!/usr/bin/env node` shebang re-enters nub (**measured: 541 spawns from one plain-node run**). But it reuses an already-populated `__VARLOCK_ENV` and then skips the CLI entirely, which is exactly what the in-process `load()` sets up.
...
- **Homebrew / curl** — a standalone executable with no module behind it. Nothing to import; the CLI path is the only option and redaction is genuinely unavailable.
...
Maintainer's call, taken deliberately because stamping the markers on the lifecycle overlay made the adapter effective inside third-party build scripts — `node-gyp` and `prebuild-install` reach Node directly rather than through the shim.

The reasoning is consistency, not appetite for exposure. nub's own `.env` cascade **already** reaches those scripts, because a lifecycle script's `node` goes through nub's PATH shim, re-enters nub, and resolves for itself. So this is a *widening of the non-shim route to match the shim route*, not a new class of access. Withholding the schema would have meant a project's environment behaving one way for its own code and another for a dependency's build, differing only by which loader answered.
...
1. **Detection was never really filename-driven** — `suppresses_env_files()` is false for `Missing`, so nub only stands down when the `varlock` package actually resolves. The coupling sits on the dependency, not the filename. That was already right.
...
2. **Resolvability is not evidence — it is a requirement.** A `.env.schema` with no rival declared and no resolvable varlock is an ERROR, not a fall-back to `.env*`. The old non-fatal "loaded .env files instead" hint is deleted. The reasoning: falling back is a DIFFERENT answer wearing the same shape (no defaults, no validation, no providers, and for a schema-only project nothing at all), and it was silent. `SchemaProblem::is_fatal` is gone with it — both variants now bail, differing only in the fix they recommend (`nub install` vs `nub add -D varlock`).
...
Reproduced against shipped behavior, and **not caused by this PR** — nub's `.env` cascade doing exactly what it is told:
...
`dotenvx run -- node app.js` returns `hello-plain`; bare `nub app.js` returns the ciphertext. That is the same silent-wrong-answer class this whole PR exists to close, at roughly **57×** the population.

It composes correctly both ways already — `dotenvx run -- nub app.js` is right, and `node --require @dotenvx/dotenvx/config app.js` decrypts in **0.19 s** with no subprocess. So the defect is confined to the bare invocation.

The broad fix is **tool-agnostic and needs no adapter**: refuse to inject a value that obviously is not one (`encrypted:` alongside `DOTENV_PUBLIC_KEY`; `ENC[AES256_GCM`). It requires zero knowledge of any tool's API and covers tools nub has never heard of. Tracked as a separate decision — it changes nub's core env path for everyone, so it is not being slipped into this PR.
...
### Known divergence: type generation does not run

`@generateTsTypes` writes its file when the **CLI** resolves the environment. nub resolves in-process, so nothing is written. Users run `varlock codegen`; documented on the env-files page.
...
**This supersedes the `varlock load` subprocess design below.** Varlock's root export has a genuinely in-process async loader, `load()` (`src/index.ts:19-30`). Because Node fully evaluates `--import` modules — top-level await included — before the entry module runs, a preload can `await load()` and have `process.env` populated before any user code. The redaction patches are on the same root export.
...
Two real limitations, both measured:

- **`load()` takes no arguments** (`load.length === 0`, and the source carries a literal `// TODO: add some options`). So there is **no path override**, and varlock's cwd-only discovery cannot be redirected — **the workspace case is not solvable on this path today.**
...
The obvious upstream ask, and a small one: **`load({ path })`**. That single addition would let the fast in-process path serve monorepos too, collapsing the two designs into one.
...
`varlock run` hands its child two things — confirmed by dumping the child's environment:

- `__VARLOCK_ENV` — the full serialized graph JSON (same shape as `load --format json-full`)
- `__VARLOCK_RUN=1`

This is a first-class, documented handoff, not an implementation detail: `varlock run -i/--inject` takes `all` (default) / `vars` (individual vars only) / `blob` (only `__VARLOCK_ENV`).

So nub should do exactly what `varlock run` does:
...
`varlock/env` purely consumes the handoff — with `__VARLOCK_ENV` unset it produces nulls rather than resolving. `init-server` consumes it *and* installs the console/Response/ServerResponse patches. Because it never spawns, it is safe to inherit through `NODE_OPTIONS`, which is what keeps `nub run <script>` coverage working.
...
Verified working on **both varlock 1.9.0 and 1.16.0**: `varlock run` sets `__VARLOCK_ENV` + `__VARLOCK_RUN=1`, `./init-server` is in the exports map, and blob + `--import varlock/init-server` redacts correctly with zero spawns. The exact floor below 1.9.0 is untested. So the design does not pin users to a bleeding-edge varlock.

## Redaction: nub does NOT match `varlock run` (measured)

**Correction.** This document previously claimed the preload path was "byte-identical to `varlock run`, redaction included". That was measured with a `console.log` probe only and does not generalize. The two use different mechanisms and the coverage genuinely differs:
...
1. **The gap is widest exactly where redaction matters most.** Piped output is CI logs and log files — the leak path redaction exists for — and there `varlock run` covers everything while nub covers only `console.*`. A logger writing to `process.stdout` directly (pino and friends), or any script that shells out, leaks under nub and does not under `varlock run`.
...
Nub will not add stream-level redaction. The adapter installs exactly the protections the loader installs for itself — its own `auto-load` calls these three, in this order, and nub matches it:
...
Each is internally gated on the schema's own settings, so a project that set `@redactLogs=false` or `@preventLeaks=false` gets what it asked for. Nub makes no judgement about which to run and adds nothing of its own.

**Behavioral note worth knowing: in-process leak prevention THROWS where `varlock run` redacts.** Verified on the same fixture, putting a `@sensitive` value into `Response.json`:
...
That difference is not nub's doing. Under `varlock run` the child has no loader in it at all, so `Response.json` is never patched and the parent's stream redactor masks the bytes on the way out; the in-process path patches the real constructor and can therefore catch the leak at its source. It is the same difference a user would get from `varlock/auto-load` directly, and the schema controls it.

If you need stream-level coverage — raw `process.stdout.write`, or anything a subprocess prints — the answer is `varlock run -- nub …`, which composes fine (verified). Nub does not reimplement it.
...
- **Stream redaction for subprocesses.** A nub-run script that shells out to `sh -c 'echo $SECRET'` is redacted by `varlock run` and not by a preload. Worth stating plainly rather than claiming parity.
...
`varlock load` is not free. Measured on a contended host (load ~33, so directional only):
...
That is too much to add unconditionally to every `nub` command. **Auto-detection needs a resolved-graph cache** keyed on the schema and env-file mtimes, the current-env value, and the varlock version — or the integration has to be opt-in. This is the main open question below.

`varlock load` is also not read-only: with a `@generate*` decorator it writes generated files (`runCodeGeneratorsIfNeeded` runs on the `load` path). Measured: `env.d.ts` is written on the first load and **not** re-touched on an unchanged second load, so it is not a write-storm — but nub cannot treat the call as pure.
...
`envFile: false` is the load-bearing half — it turns off nub's cascade so varlock is the only thing resolving env, which is the whole point. Verified to match `varlock run -- node app.js` on values and `@currentEnv` selection, and it covers `nub run <script>` too. Redaction coverage is NARROWER than `varlock run` — see the redaction section above.

The fragility of both footguns is itself the argument for solving this in nub rather than in userland.

**Blob handoff** — if you already have a resolved graph, `preload: ["varlock/init-server"]` plus `__VARLOCK_ENV` needs no shim and spawns nothing. This is the pattern nub should adopt internally.

## Answering the author's concerns

| Concern | Resolution |
|---|---|
| "It's loading a graph, resolvable different ways" | Conceded entirely — nub defers, does no resolution of its own. Verified that nub's cascade is silently wrong under `@currentEnv`. |
...
3. **Fixing nub's `preload` leak generally**, without losing `nub run <script>` coverage. The integration no longer depends on it once the blob handoff lands, but it remains a real bug.
...
- `origin/main` still ships peggy 5.x. `git grep -i monogram origin/main` returns **no hits**. The real parser is `packages/env-spec-parser/` — a PEG.js (`grammar.peggy`) grammar compiled to JS/TS. No Rust, no WASM, no tree-sitter anywhere in the tree.
...
The "monogram gives us a Rust parser" expectation needs tempering against what monogram actually is: a **grammar-definition tool**, not a parser library. You write the grammar once in TypeScript combinators and it derives a lexer/CST parser, a TextMate grammar, a **tree-sitter** grammar, a Monaco tokenizer, and language-config files. So the realistic artifact for a Rust consumer is a tree-sitter grammar — which is C, reachable from Rust via the `tree-sitter` crate's bindings — not a bespoke native Rust `@env-spec` parser. That is a materially different (and heavier) integration than "just use their Rust crate".

One thing not to be confused by: **varlock already ships Rust**, at `packages/encryption-binary-rust/` (`varlock-local-encrypt`). That is the local-encryption helper — ECIES plus platform key protection (DPAPI/Windows Hello, TPM2/secret-service), spawned as a helper binary over JSON IPC. It is the same binary the load path probes on every run (see Cost). It has nothing to do with parsing `@env-spec`.

So a Rust-native `@env-spec` parser is not available, and reimplementing `@env-spec` in nub is rejected for now — it would also re-introduce exactly the "nub resolves it itself" failure mode this document argues against. Revisit only if #744 lands *and* a usable Rust parsing path materializes.

## Reproduction

Fixtures used: `/tmp/vlk` (single package), `/tmp/vlkmono` (workspace). Both are throwaway; recreate with `npm install varlock@1.16.0` plus a `.env.schema`. The full experiment matrix is in the thread scratchpad at `.fray/threads/a0c5fba3-72dd-404a-a7a3-f2d56df4408c/scratch.md`.

Care is needed reproducing the fork bomb: bound it with a short `timeout` and clean up with `pkill -f "varlock load"`. Do **not** try to bound it with `ulimit -u` — that limit counts all of the user's existing processes, so on a busy host it fires before anything runs and produces a false positive.
...
Every name in the source, for reference: `_VARLOCK_ENV_KEY`, `_VARLOCK_FILTER`,
`_VARLOCK_REDACT_STDOUT`, `_VARLOCK_CACHE_KEY`, `__VARLOCK_INTEGRATION`,
`__VARLOCK_EXECUTION_PHASE`, `_VARLOCK_USE_INJECTED_ENV`, `_VARLOCK_THROW_ON_LOAD_ERROR`,
`_VARLOCK_FORCE_KILL_TIMEOUT_MS`, `__VARLOCK_SEA_BUILD__`, `__VARLOCK_BUILD_TYPE__`,
`_VARLOCK_DYNAMIC_BUILD_ACCESS_MODE`, `_VARLOCK_FORCE_FILE_ENCRYPTION_FALLBACK`, and a proxy-session
family. Convention (`auto-load.ts:124-127`): `__` = varlock-set, `_` = user-controllable.

**DECIDED (2026-08-02): nub does not use these to skip re-invoking varlock.** Measured the nested
case — project A's `varlock run` wrapping a nub that runs in project B:
...
Double resolution is **idempotent for schema-declared values** — B's schema correctly wins. (`ONLY_A`
rides along as ambient, which is varlock's own passthrough behaviour and identical to
`varlock run -- varlock run -- node`.) Skipping on `__VARLOCK_RUN` alone would have given B **A's**
`GREETING`. A `basePath` comparison would catch that, but it is necessary and not sufficient:
...
Each failure mode is a silent wrong or incomplete environment, to save ~0.15 s on a deliberate and
uncommon invocation. Not worth it.
