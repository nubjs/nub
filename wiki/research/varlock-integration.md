...
Detecting `.env.schema` is therefore a short walk of `stat`s from the project root up to the workspace root, and it can run *before* the cascade, so Nub declines to load rather than loading and reversing. The "undo what we already did" problem exists only if the decision has to be made in JS, after the child has started with values already in `process.env`.
...
- 2026-08-28 — **Schema discovery is now a walk from the project root to the workspace root, and a schema is only read at a PACKAGE root.** It checked exactly `project_root` and `workspace_root`, which covered a standalone project, a workspace root and a member, and silently missed a schema at a package that CONTAINS the package being run (`apps/web/.env.schema`, run from `apps/web/functions/`, which has its own manifest and is therefore the project root). That fell back to nub's own `.env*` cascade with no diagnostic — the exact silent substitution `SchemaProblem` exists to refuse. `search_roots` now yields the project root and each parent up to and including the workspace root, and the loader lookup — which already walked with that same ceiling, for the same "nothing above the workspace is this project's" reason — shares it, so the two cannot disagree about reach. The ceiling is bounded by `Path::starts_with` rather than an equality test, so a `workspace_root` that is not an ancestor of `project_root` yields the project root alone instead of a walk to `/`; component-wise comparison also makes it immune to a trailing slash, a `.` component, and the `/a/bb` vs `/a/b` prefix trap (probed). **Requiring a `package.json` beside the schema was tried and REVERTED within the same effort.** It was added on maintainer instruction and reverted on maintainer challenge, and the challenge was right. One of the three reasons given for it was simply false, found by reading the code rather than recalling it: `root` is handed to the loader as `--path`, and that branch returns early and *explicitly ignores* the `varlock.loadPath` it would otherwise read from a `package.json` (`loadFromPaths` requires only that the path exists), so the loader is perfectly happy pointed at a manifest-less directory. What remained was a tidiness argument that cost something real — a schema deliberately placed inside the ceiling would be ignored SILENTLY, the one failure mode this module refuses. The bound doing the safety work is the ceiling, not the manifest: within the project root through the workspace root, usually one to three directories, a `.env.schema` is deliberate. The rival carve-out and the diagnostic are unaffected either way, since both read the running package's manifest rather than the schema directory's. Grounded in what varlock itself does: **it performs NO ancestor walk at all** — `loadGraph` resolves `--path`, else `varlock.loadPath` read from the `package.json` in cwd (`readVarlockPackageJsonConfig` joins cwd exactly once), else cwd; confirmed in the published docs ("It does not walk up to a parent directory automatically") and in source at varlock 1.9.0. nub walks only because it resolves the project root rather than cwd, then passes `--path` — which is the loader's own answer for that gap, so supplying it is not a second opinion layered over the loader's but the input the loader leaves to its caller. Fallout fixed in the same change: the manifest checks (`loader_declared`, `rival_schema_tool_declared`) take DIFFERENT reaches, which review proved load-bearing. Scanning the whole chain for both looked cautious and was a regression: with `dotenv-extended` hoisted at a workspace root for a sibling's benefit, a member that ships its own schema, declares the loader and has it installed lost the hand-over, and lost it SILENTLY, because the missing-loader diagnostic is gated on the same `is_ours` that had just flipped (measured against the pre-change binary, which hands over correctly). So `rival_schema_tool_declared` reads only what the schema GOVERNS — the running package, anything between it and the schema, and the schema's directory — because a directory above the schema answers to a different schema or to none; `loader_declared` reads the whole chain, because a devDependency at a workspace root is how a monorepo declares the loader for every member, and it only chooses between two fatal messages rather than changing what runs. The general lesson: standing down is not the free cautious option it resembles, since it substitutes a different environment with no diagnostic. Manifests are read through `strip_utf8_bom` now, matching every other manifest reader in the tree. **Known limit, open:** the ceiling is `workspace_root.unwrap_or(project_root)`, so a repo with NO declared workspace has a one-entry chain and a nested `package.json` (Firebase `functions/`, a Vercel edge bundle) still cannot see an enclosing schema; covering that needs a stopping rule other than the workspace root, which is a separate decision. Stated in the Varlock docs rather than left implicit.
- 2026-08-17 — **REVERSAL of the global-scope carve-out shipped three days ago: a `.env.schema` is a DEFAULT, and a declared `envFile` at ANY scope beats it.** The carve-out held that a global `envFile` must not displace a project's schema, so that `nub config set --global envFile false` could not empty a schema project's environment far from the config that caused it. Maintainer's call to reverse, on two grounds, and both are about coherence rather than the failure mode. A schema is not a config field and does not sit in the precedence chain, so exempting one layer put a non-config signal at a novel position INSIDE that chain — beating `global`, losing to `project` — which no other key does and which cannot be stated without describing the wedge. And the change it was part of exists to stop an explicit `envFile` being silently ignored, so carving out one layer where it still is contradicts the premise. The schema now resolves at the `Defaults` layer and `CLI > environment > project > global > defaults` runs unchanged. A consequence worth recording: `envFile: "varlock"` was purely declarative under the carve-out (an absent `envFile` reached the same place) and is now load-bearing, because it is how a project overrides a machine-wide `envFile: false`. Accepted cost: a global `false` empties a schema project, which fails loudly at an undefined variable rather than silently, and one project line fixes it.
- 2026-08-17 — **Relative paths in config files resolve against the DECLARING file's directory — settled, not merely current.** Surveyed after the question was raised against a global `envFile`. Three models exist in the wild: declaring-file (uv, `options.relative_to(path.parent())`; Cargo, which uses the parent of the config's directory so `.cargo/config.toml` means the project root; ESLint; tsconfig's default), project-root-always (pnpm's `store-dir` — `pathAbsolute(storePath, pkgRoot)`, issue #3976), and an opt-in token (tsconfig's `${configDir}`, added in TS 5.5 precisely because paths in a shared base config were useless in derived projects). nub keeps the majority model. The known consequence — a relative path in the GLOBAL file points inside `~/.config/nub/`, which is nearly never meant — is documented rather than special-cased, the way Cargo documents its own footgun. Rejected: resolving global paths against the project root (inconsistent with how a project file's own paths resolve), a `${projectDir}` token (speculative surface; the grammar already exists via `${VAR}` expansion if it is ever wanted), and refusing relative paths in the global file outright. Only three keys are path-valued — `preload`, `envFile`, `tsconfig` — so the policy stays cheap to revisit.
