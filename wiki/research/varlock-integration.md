...
- 2026-08-28 — **Schema discovery is now a walk from the project root to the workspace root, not two fixed directories.** It checked exactly `project_root` and `workspace_root`, which covered a standalone project, a workspace root and a member, and silently missed everything between them. Two shapes fell through to nub's own `.env*` cascade with no diagnostic: a schema at a package that CONTAINS the package being run (`apps/web/.env.schema`, run from `apps/web/functions/`, which has its own manifest and is therefore the project root), and a schema parked at a directory with no manifest at all. That silent substitution is the exact failure `SchemaProblem` exists to refuse, so the gap was a hole in the guarantee rather than a missing convenience. `search_roots` now yields the project root and each parent up to and including the workspace root, and the loader lookup — which already walked with that same ceiling, for the same "nothing above the workspace is this project's" reason — shares it, so the two can no longer disagree about reach. The ceiling is bounded by `Path::starts_with` rather than an equality test, so a `workspace_root` that is not an ancestor of `project_root` yields the project root alone instead of a walk to `/`; component-wise comparison also makes it immune to a trailing slash, a `.` component, and the `/a/bb` vs `/a/b` prefix trap (probed). Fallout fixed in the same change: the manifest checks (`loader_declared`, `rival_schema_tool_declared`) read the schema's own directory, which need no longer carry a `package.json`, so they now scan the whole walked chain — the running package plus each enclosing directory, never a sibling and never anything outside the workspace. That matters most for the rival carve-out, where reading the wrong manifest routes a run through `varlock run` against a schema written in another format. Rejected: stopping the walk at package boundaries only, which would close the nested-package gap but keep ignoring a manifest-less schema directory for no stated reason. Maintainer's call, on the grounds that discovery should be workspace-aware.
