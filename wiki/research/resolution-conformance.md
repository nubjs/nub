...
- **Apples-to-apples baseline.** The baseline is not "some Node on `$PATH`" — it is `nub node which`, the exact Node binary Nub itself spawns, so the only variable between the two columns is Nub's augmentation (resolution hooks + injected flags), never the Node version. Same dual-mode idea as [[research/node-test-suite-leverage]] (`nub --node` passthrough vs augmented `nub`), narrowed to the resolution corpus.
