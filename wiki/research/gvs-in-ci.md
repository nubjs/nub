...
Two independent triggers keep the store project-local; both are DEFAULTS an explicit user setting overrides:

- **Env-based (`CI` set):** Nub fills `enableGlobalVirtualStore=true` only where no source set it, the layout is isolated, no framework that cannot reach a shared store is declared, and `CI` is unset (`fill_install_defaults` in `crates/nub-cli/src/pm_engine/host_settings.rs`). Presence-based: `CI=false` and `CI=""` also count (see §6).
- **Verb-based (`nub ci`):** the same default is withheld for `nub ci` under any of its names (`clean-install`, `ic`, `install-clean`), whatever the environment (#241, #261). `nub ci` is the frozen, deploy-oriented install whose output is copied with `COPY --from`, so it is self-contained regardless of env.

**What actually changes**, read from `node_modules/.modules.yaml` (2026-09-14): with the global virtual store on, `virtualStoreDir` is the machine-global `~/.cache/nub/store/v11/links` and `node_modules` links into it; with it off, `virtualStoreDir` is `.store` and the packages are real directories under `node_modules/.store/`, cloned from the content-addressable store. **The isolated layout — and its phantom-dep protection — is identical in both modes**; only the store's location moves. The content-addressable store stays machine-global in both modes.

**Override precedence:** an explicit `enableGlobalVirtualStore` (`.npmrc`, `install.settings` in `nub.jsonc`, `npm_config_enable_global_virtual_store`) beats both the `CI` gate and `nub ci`. The auto-disable is a default, not a force.
