---
**Scope:** This doc captures the executable-hijack flag-compatibility contract. Under the hijack-by-default PATH shim, Nub's binary sits in front of EVERY `node` invocation in a Nub-orchestrated process tree: it inspects `basename(argv[0])`, and when that is `node` it dispatches into an augmented top-level that auto-injects flags (per auto-flag-injection.md) and ultimately spawns the REAL node. That makes Nub-as-node a full `node [node-flags] <script> [script-args]` argv proxy. This doc answers, for each flag category: as an `argv0=node` proxy, how must Nub faithfully forward/intercept each flag, locate the script boundary, inject its own flags strictly before the script, and reproduce node's early-exit / identity / value-form semantics byte-for-byte. Getting the boundary, a value-form (`--opt=val` vs `--opt val`), or an early-exit flag wrong silently breaks drop-in compat for the user AND for every descendant `node` invocation.

**Relationship to [`node-flag-interactions.md`](node-flag-interactions.md):** that doc asks the INVERSE question — "from Nub's augmentation surface (registerHooks, `--import` preload, env loading, auto-flag-injection), do we BREAK this flag?" — and produces a per-flag A–F verdict table (no-interaction / plays-nicely / conflict / plausibly-broken / subtly-broken / mooted). This doc asks "as an argv0=node proxy, how must we faithfully PARSE and FORWARD this flag, and where exactly do we inject?" It supersedes nothing. Where `node-flag-interactions.md` already has a per-flag augmentation verdict (e.g. `--permission`, `--watch`, `--env-file`, `--conditions`, `--require`-vs-`--import`, `--inspect-brk` source maps), this doc references it rather than re-deriving — the value added here is the boundary-parser contract, the value-form / arity table, the injected-flag collision matrix, and the early-exit/identity rules that govern Nub-as-node's argv rewrite.

**Verification posture:** Findings marked **confirmed** were reproduced by running real `node` (primarily v26.2.0, the development host's Node) with the cited flags, and cross-checked against `node/src/node_options.cc`, `node_options-inl.h`, and `node/lib/internal/main/`. Findings marked **design-inferred** are read from source / behavior on the dev-box version but were NOT reproduced on the 18.19–22.14 / 22.15 augmentation floor; those carry an explicit re-verify note. Node's augmentation floor is 18.19.0 (sync `registerHooks` on 22.15+, async `module.register` on 18.19–22.14) per `auto-flag-injection.md`; several findings are tier-dependent and say so.
...
Nub-as-node must replicate node's tokenizer EXACTLY, because three things ride on getting the parse right: (1) the script boundary — where node-flags end and the script + its argv begin; (2) the injection point — Nub must insert its auto-flags (`--import` preload, `--enable-source-maps`, `--disable-warning=ExperimentalWarning`, version-gated `--experimental-*`) STRICTLY before the script and, for `--import` specifically, before any user `--import`; (3) early-exit / identity fidelity — `--version`, `--help`, `--v8-options`, `--completion-bash` must print the REAL node's output and exit, never Nub's identity. A naive "first non-dash token is the script" scan is wrong because value-consuming flags (`-e CODE`, `--require MOD`, `--env-file PATH`) legitimately place a non-dash token immediately after a flag that is NOT the script, and because `-e`/`-p`/`-i`/`--`/`-`/early-exit flags each bend the rules.

The single most important parser fact (`node_options-inl.h:337-338`): node's option loop `break`s the instant `args.first().size() <= 1 || args.first()[0] != '-'`. The first token that is not `-…` (or is a bare single `-`) terminates node-flag parsing; that token and everything after become `process.argv`. Everything in this doc is downstream of correctly locating that break point AFTER honoring value consumption, `--`, and the eval/REPL/early-exit modes.
...
Per-flag augmentation verdicts (does the loader break) live in [`node-flag-interactions.md` §3 / §4.6 / §4.7](node-flag-interactions.md). Here: how Nub-as-node must order/forward them around its injected `--import`.
...
- `node --no-import main.mjs` → `node: --no-import is an invalid negation because it is not a boolean option` (confirmed). String-list flags (`--import`/`--require`/`--loader`/`--conditions`) have NO `--no-X` form. The `--no-experimental-*` subtract-merge opt-out in `auto-flag-injection.md` does NOT cover the injected `--import`; the only escape is `nub run --node` / `nubx --node`. The opt-out doc must state this exclusion explicitly.
...
The category's dominant trap is VALUE-FORM ASYMMETRY: the TOGGLES take no value; their SETTINGS take a space-or-equals value. One mis-classification swallows the script.
...
| `--trace-*` (deprecation, warnings, exit, sync-io, tls, uncaught, env, …) | none (bool) | passthrough | Valueless. `--trace-sync-io` will fire constantly under Nub's sync registerHooks — expected; see [`node-flag-interactions.md`](node-flag-interactions.md). | low |
...
Nub injects ZERO flags in this category, so it is pure passthrough — the entire risk is the splitter's value-form classification. The category splits into TWO disjoint, superficially-identical parse behaviors.
...
The classification MUST come from node's actual option-type table (typed-field binding / kUInteger / kInteger / kString vs `V8Option{}`), version-pinned, NOT from name shape. The safe DEFAULT for any unknown `--flag` the version-pinned table doesn't recognize is "V8 passthrough, consume no following token" (matches `node_options-inl.h:441-444`).

## 6. Boundary parser contract (the load-bearing section)

This is the single algorithm Nub-as-node lives or dies by. State it precisely and pin it to the detected node version.
...
1. Walk tokens left to right. For each token `t`:
   ...
   - `=`-splitting is `--`-prefix ONLY. `-e=x` / `-p=x` are REJECTED (`node: bad option`). For short flags the value is the next whole token.
...
Nub injects, per `auto-flag-injection.md`: `--import <preload>` (always), `--enable-source-maps` (universal), `--disable-warning=ExperimentalWarning` (universal), and version-gated `--experimental-sqlite` / `--experimental-websocket` (only on node versions where still flagged). All are kAllowedInEnvvar. The collisions, with the merge rule:
...
**Identity mode is decided by `argv[0]`, not by a heuristic.** Nub dispatches on `basename(argv[0])` (`hijack-by-default.md`), which cleanly splits the two cases that would otherwise conflict on `--version`/`--help`: (1) **node-identity mode** — Nub was reached THROUGH the hijack shim (a descendant process invoked `node`, so `argv[0]` is `node`/`node.exe`). Here every identity surface below MUST be the real spawned node's, byte-for-byte, because the caller is build tooling (node-gyp, npm, `child_process.spawn('node', …)`) that parses it. (2) **nub-identity mode** — the user typed `nub`/`nubx` directly (`argv[0]` is `nub`), so `nub --version` prints NUB's version and `nub --help` prints Nub's help; printing node's identity here would be the wrong, confusing answer. The discriminator is deterministic and unspoofable (node cannot rewrite the `argv[0]` Nub sees), so there is no "detect if hijacked" guesswork — the rules in this section govern node-identity mode; the direct-`nub` surface is Nub's own CLI. One corollary: in node-identity mode `--version` must reflect whatever node Nub actually spawns (forward the flag or shell `node --version`), never a string baked into Nub, so it tracks the user's installed/selected Node.

Early-exit flags fire ONLY when parsed as a node OPTION (before the script boundary / before `--` / not bound as a preceding option's value). But — load-bearing correction to a tempting simplification — they ALSO fire after `-e`/`-p`/`--eval`/`--print`, which do NOT establish a boundary: node keeps parsing options after the eval value.
...
These are the `isReal: true` risks from verification, ranked.
...
14. **Permission-tier breakage (high, tier-dependent).** Under `--permission`: Nub's preload read needs `--allow-fs-read=<canonicalized-install-dir>` (a bare install-dir grant FAILS under a symlinked Homebrew install — node realpath's the `--import` path; confirmed); its N-API transpiler addon needs `--allow-addons` (refuse-early, don't auto-inject — widening); and on the async tier (18.19–22.14) `module.register` spawns an INTERNAL worker that throws "Use --allow-worker" (confirmed on node 26 that the check fires before is_internal is read). The sync registerHooks tier (22.15+) is clean. **Mitigation:** match node's FULL permission-active set (`--permission` AND `--permission-audit`, argv + NODE_OPTIONS); canonicalize the install dir via `current_exe()->canonicalize()` for the fs-read grant; refuse-early on missing `--allow-addons`; on the async tier under `--permission`, refuse-early or force the sync tier. Full plan: `permission-passthrough.md`; per-flag augmentation verdicts: [`node-flag-interactions.md` §4.1](node-flag-interactions.md).

## 10. Open questions

- **Version-pinned arity table source of truth.** The §6 table is read off node v26.2.0 source + behavior. Nub augments node 18.19+. New V8 flags appear and some flags migrate type across majors. Should Nub bake a per-major option-type table, or derive it at runtime from `node --v8-options` + a node-owned-flag allowlist? The default-safe "unknown `--flag` consumes nothing" rule covers the long tail but not node-owned value flags added in a version Nub's table predates. (design-inferred; not yet decided)
- **TLA-of-ambiguous-`.js`-entry under the injected `--import` on the 18.19 floor.** The entry-loader flip is benign on node 26; not reproduced on the floor where detect-module differs. Needs a `node:18.19-slim` Docker fixture before claiming full parity. (design-inferred)
- **`--inspect-brk` first-frame.** Should Nub ignore-list its preload frames via the V8 inspector skip-list, defer the break, or just document that the first paused frame is the loader shim? A user who passes their own `--import` already sees this, so it is augmentation-induced, not non-conformant vs `node + --import`. (design-inferred from `node.cc:312-319`; not reproduced live)
- **Async-tier `--permission` decision.** Refuse-early vs force-sync-tier vs document-`--allow-worker`-requirement is undecided in `permission-passthrough.md` and needs a decision record.
- **Signal relay completeness.** Half of §4's flags depend on the resident parent forwarding SIGUSR1/SIGUSR2/SIGINT to the child. Is that relay implemented and tested? Not a flag in this audit, but it gates the observable effect of the whole diagnostic category.
