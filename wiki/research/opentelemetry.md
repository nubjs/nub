...
**Status:** 2026-08-03. Survey and measurement pass, commissioned to find out what a launcher that computes a child process's flags and environment can do for OpenTelemetry that an in-process SDK cannot. Companion to the [preload ecosystem survey](preload-ecosystem.md), which measures the same attach path from the `NODE_OPTIONS` side, and to [monkey-patch prevalence](monkey-patch-prevalence.md), which covers the interception layer underneath it.
...
Node 26.5.0, macOS arm64, `@opentelemetry/auto-instrumentations-node` with its 80 transitive OpenTelemetry packages, minimum of 3 runs, on a contended host. Every cell asserts exit 0 **and** a positive control — the probe reports whether `Symbol.for('opentelemetry.js.api.1')` was actually registered — so a crash or a silently-absent preload cannot be recorded as a fast run. The OTLP endpoint is pinned to an unused port rather than the default 4318, which was contended.
...
The collector-listening column is the useful control: standing a collector up removes the 7.1 s term completely and leaves the detectors, which confirms the two are unrelated causes rather than one effect measured twice.
...
`diagnostics_channel` appears in exactly one OpenTelemetry instrumentation (undici), against 192 files in `dd-trace-js`.
...
One notable extension: `OTEL_EXPORTER_OTLP_PROTOCOL=console` pretty-prints spans, metrics and logs to stderr. It is a non-spec fourth protocol value beside `http/protobuf`, `http/json` and `grpc`, and it answers the question of how a user sees their traces without first standing up a collector.

**The global-registry mechanism, and its consequence.** Deno writes `globalThis[Symbol.for("opentelemetry.js.api.1")]` with version `1.999.999` and fills the `trace`/`context`/`metrics`/`propagation` slots with native providers. That version passes `@opentelemetry/api`'s read check for every 1.x, so user code calling `trace.getTracer()` transparently gets Deno's tracer with no setup. It also permanently blocks **writes**: `registerGlobal` requires an exact version-string match, so no user copy of `@opentelemetry/api` can register anything afterwards. Upstream declined to change this ([opentelemetry-js#5454](https://github.com/open-telemetry/opentelemetry-js/issues/5454), closed as not planned). Vendors have responded by deleting the registry outright — Sentry's Deno SDK clears any pre-existing registration before installing its own. A related exact-match failure bit the Node flavour too, where a patch-level mismatch silently disabled all tracing ([sentry-javascript#22338](https://github.com/getsentry/sentry-javascript/issues/22338)).

**Operational findings from the implementation**, each traceable to a public thread, and each a constraint on anyone building the same thing:
...
An earlier run of this measurement was invalid and is recorded here because the failure is easy to repeat: the baseline's HTTP code was declared but never called in the telemetry variants, so link-time optimization dead-stripped it and the instrumented binaries measured *smaller* than the baseline. The table above uses an identical live workload in every variant, confirmed by checking that OTLP field names are present in the hand-rolled binary and absent from the baseline.
...
Turborepo is the closest analogue to a package manager emitting its own telemetry, and three of its choices are instructive. It emits **metrics rather than spans**. It treats **cardinality as a first-class design axis**, attaching bounded attributes always and gating unbounded ones (run id, revision, task id, task hash) behind opt-in flags, on the stated grounds that backends charge per unique series. And it restricts endpoints to `https://` while rejecting private, loopback, link-local and cloud-metadata addresses as SSRF targets — including the IPv4-mapped-IPv6 bypass — but still permits the hostname `localhost` for local collectors. It also locks credentials to the endpoint that introduced them, so a higher-priority configuration source changing the endpoint drops lower-priority auth headers rather than forwarding them elsewhere.
...
Implementations to model: BuildKit extracts the variables at startup and injects the current span when spawning a child, in about 40 lines total. The Jenkins OpenTelemetry plugin exports them into every build step's environment so downstream tools join the pipeline trace. `otel-cli` picks up an ambient `TRACEPARENT`, injects one into child processes, and can substitute it into an arbitrary argument so it can be passed as an HTTP header. The Gradle plugin passes it to exec tasks behind a setting.
...
Two plausible explanations were tested and ruled out, recorded here as current verdicts because both are the sort of thing that would otherwise be re-derived. The version band that governs whether Nub downgrades to its asynchronous tier is **not** the cause: a plain-Node synchronous `registerHooks` pass-through composed with the same asynchronous loader on Node 26.5.0 exits 0, so hook composition is not broken at that version. And the internal environment variable that forces the asynchronous tier is **not** a workaround, nor can it be tested as one, because Nub strips the variable before the child process reads it — it is set by the launcher only.

## Reproduction

- Attach-cost matrix: install `@opentelemetry/auto-instrumentations-node` into an empty project, then time `node` running a trivial script with `NODE_OPTIONS="--require @opentelemetry/auto-instrumentations-node/register"` across the env-var combinations in the table. Pin `OTEL_EXPORTER_OTLP_ENDPOINT` to an unused port — the default 4318 is a commonly-occupied port and a stray listener silently removes the 7.1 s term. Assert a positive control in every cell: a probe script reporting whether `globalThis[Symbol.for('opentelemetry.js.api.1')]` is set, so an unattached run cannot be recorded as a fast one. Note that `--require` of an absolute path to the `register` subpath does not resolve, because it is an exports subpath rather than a file; use the bare specifier with the project as the working directory.
