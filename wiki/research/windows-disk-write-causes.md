...
This tracer demonstrated the point against itself. The `@aws-amplify/cli` run hit a dependency-install timeout, so `lib/install.js` started against an incomplete `node_modules` and died on `Cannot find module 'tar-stream'`. The visible result was `exit=0`, the echo's text on stdout, and **zero write paths** — which reads exactly like a package that needs nothing. It is not a measurement of anything, and it is excluded from the table above rather than reported as a narrow result. A script that bails for a missing precondition is untested, not passing, and this shape makes the two indistinguishable without reading stderr.
...
- **Six packages traced, not nine.** `jpegtran-bin` and `mozjpeg` were still running when this was written, and `@aws-amplify/cli@12.9.0` produced an UNUSABLE result — see the timeout case above. The latter two are the same `bin-wrapper` cohort as `gifsicle` and are expected to share mechanism 1; that expectation is not a measurement.
...
**Building nub on that box needs two things that are not obvious.** Wipe `~/.cargo/registry/src` and `~/.cargo/registry/cache` first — a corrupted registry copy produces 41 errors confined to `dashmap`, mixing `E0412` with `E0514`, which reads like a compiler-version problem and is not. Then put `cmake` on `PATH`: `libz-ng-sys` builds through it, VS BuildTools ships it at `C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\Common7\IDE\CommonExtensions\Microsoft\CMake\CMake\bin`, and nothing puts it there. Then put the toolchain's own `bin` directory first on `PATH` as well, because **invoking `cargo.exe` by absolute path out of a toolchain directory does not pin `rustc`** — cargo resolves `rustc` through `PATH`, so a box carrying a second, older toolchain silently pairs a new cargo with an old compiler. The symptom names neither: `error: the -Z unstable-options flag must also be passed to enable the flag check-cfg`, attributed to whichever third-party crate happened to compile first, with no `-->` source location anywhere in the log. That missing location is the tell that the failure is a rejected command line rather than bad code. Have the build write its own `EXIT=<n>` status file: a scheduled task's `LastTaskResult` is the shell's exit code, not cargo's, and reports 0 over a failed build.
