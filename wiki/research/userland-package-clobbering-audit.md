...
- **Recommended Nub v0.1 clobber set: empty.** No default specifier intercepts. Userland packages like `ws` and `node-fetch` already work on the user's Node — they're not load-failures the way `node-fetch`/`undici` are inside Bun's WebKit/JSC runtime. The install-size win is real but small; the API-parity-bug surface (server-side `ws`, off-spec callbacks on `eventsource`, `Database` shape on `better-sqlite3` vs `node:sqlite`) is large. Reverse the polarity from Bun's default-clobber posture to **opt-in only**.
- **Load-bearing safety concern: `ws` has a server.** Bun's `ws` shim is a 1,582-line file that reimplements `WebSocketServer` from scratch on top of `Bun.serve`, stubs `Receiver`/`Sender`/`createWebSocketStream` to throw `"Not supported yet in Bun"`, and has needed multi-year patch-work to add `upgrade`/`unexpected-response` events ([oven-sh/bun#28114](https://github.com/oven-sh/bun/pull/28114), [#4568](https://github.com/oven-sh/bun/issues/4568), [withastro/astro#15926](https://github.com/withastro/astro/issues/15926)). Any clobber of `ws` inherits that maintenance liability. Nub has no reason to take it on — plain Node + the user's installed `ws` works.
- **Meta-rec: defer all default clobbers to v0.x; ship two opt-in clobbers in v0.x once the resolve-hook is stable.** The two genuinely safe candidates are `@js-temporal/polyfill` and `urlpattern-polyfill` — both are pure spec-shim packages whose entire purpose is `globalThis.Temporal = …` / `globalThis.URLPattern = …`. Intercepting and serving Nub's native is functionally identical; the install-size win is the whole point. Everything else either fails the parity test or doesn't earn the install-cost saving.
- **Most surprising finding:** Bun does *not* clobber `better-sqlite3`, despite `bun:sqlite` being explicitly `better-sqlite3`-inspired. Loading instead fails with `NODE_MODULE_VERSION` ABI errors ([oven-sh/bun#16050](https://github.com/oven-sh/bun/issues/16050)). The reason is exactly the parity hazard this audit names: `bun:sqlite` is `better-sqlite3`-*shaped*, not `better-sqlite3`-*identical* (no `.aggregate`, no `.backup`, different `pragma` return shape), and Bun doesn't want ownership of those divergences. That conservatism is the model Nub should adopt across the board.
...
Brand-boundary-clean intercept rides a resolve hook installed via Node's [`module.registerHooks()`](https://nodejs.org/api/module.html#moduleregisterhookspreoptions), registered from Nub's standard `--import` preload. On each resolution the hook checks the specifier against the clobber table; if matched and enabled for the project, it short-circuits to a synthesized module that exports the wrapper. Pass-through otherwise. The same hook already exists for tsconfig-path rewriting and extensionless probing, so the marginal cost is one table plus the wrappers — no new mechanism. The augmenter-vs-fork test passes: a user on plain Node could ship the same `module.register()` hook. Compat mode (`--node` / `NODE_COMPAT=1`) skips the hook; the real npm package resolves.

## Recommendation

**Per-package decisions:** the table above is the full set. The column collapses to: opt-in for `@js-temporal/polyfill` and `urlpattern-polyfill`; do not clobber for everything else.

**Meta-call: do not ship any default clobbering in v0.1.** Bun's seven clobbers exist because Bun is a *replacement* runtime where `node-fetch`, `ws`, and `undici` either won't load (`undici` depends on Node internals JSC doesn't have) or are redundant with native primitives. Nub *augments* the user's actual Node; those packages load and run. Bun pays the parity cost because it must; Nub would pay it for an install-size lottery ticket. The recurring `ws`-shim parity bugs ([#4568](https://github.com/oven-sh/bun/issues/4568), [#28114](https://github.com/oven-sh/bun/pull/28114), [withastro/astro#15926](https://github.com/withastro/astro/issues/15926)) are existence proof that this is hard to get right.

**In v0.x, ship opt-in clobbering for Temporal and URLPattern.** Both exist solely to install the spec global. User opts in via a brand-clean config file (no `"nub"` field, no `NUB_*` env, no `nub:*` specifier). Opt-out is the absence of an opt-in. Removing the entry restores the real resolve.

**General rule:** Nub clobbers a userland package iff (a) it's a pure spec-shim with no non-spec surface, (b) it's in the user's opt-in list, and (c) Nub's native passes a parity test gating the clobber. Default-clobbering — Bun's posture — is rejected.
...
- `better-sqlite3` → `node:sqlite` redirect already rejected: `../runtime/sqlite-unflag.md` §"Open questions"
