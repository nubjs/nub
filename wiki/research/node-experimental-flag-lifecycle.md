...
Nub injects `--experimental-*` flags to unflag features early, through the feature-matrix `Unflag` bands — `--experimental-import-text` on `[24.19.0, 25.0.0) ∪ [26.5.0, ∞)`, for instance. Several bands are **open-ended** (`hi = None`). If Node eventually **removes** a flag once its feature is default-on, does injecting it crash Node, and if so, how can a launcher stay robust without paying node-invocation latency on every run?
