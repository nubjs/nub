...
- **An explicit binary that is really Nub is refused.** Nub's own `node` shim resolves a version by re-entering discovery, which reads the same setting again, so naming it would recurse until the process died — a hang with no output. The PATH scan has always skipped the shim directory for this reason; an explicit override is the one route that still reaches it, and asking a shell for `which node` is the way it happens, because a user's `which` does not share that skip list.
