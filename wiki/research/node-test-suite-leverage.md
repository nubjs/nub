...
- **Recommended: dual-mode + Deno-shape vendoring.** Two pass-rates per release. The `--node` number is the integrity audit ("our passthrough is byte-for-byte Node"); the augmented number is the additivity audit ("our augmentations don't break Node semantics").
...
The proposed shape: a shallow submodule of `nodejs/node`, a Deno-schema JSONC allowlist, a Rust dispatcher running each test in both modes, and two published pass rates.
...
- **Compat-mode pass rate.** Expected to pass essentially the whole corpus; the residue is tests that depend on a flag the passthrough does not propagate, and anything else is a passthrough bug.
- **Augmented-mode pass rate.** Carries the documented divergences: cases (e.g. tests asserting `import './file'` resolves _without_ extension when our extensionless-resolver _does_ add one) are listed with `expect: fail-known` and counted alongside the score.
...
The harness in `tests/cross-runtime/` runs the whole Node 26.7.0 test suite from a full checkout against node, nub, bun and deno under named scoring lenses; the Rust gate over `tests/node-compat-config.jsonc` covers 5,279 tests in six directories.

The headline facts, with the lens named each time because the lens is most of the number:

- Node-relative to Node 26.7.0, under Deno's own directory set and skip list: nub 98.4%, deno 72.4%, bun 68.5%; under the bun.com universe (`parallel/` + `sequential/`, nothing skipped): nub 98.1%, deno 69.8%, bun 70.3%; over every executable directory with nothing skipped, including `pseudo-tty/` under a real pty, `ffi/` with its fixture compiled and Node's `wpt/` wrappers: nub 97.3%, deno 66.2%, bun 63.9%. A test Node itself skips (`common.skip()` exits 0; every `quic/` test on the official build, which has QUIC compiled out) is outside every node-relative denominator, since Node did not run it. Dropping the 718-test engine-specific class (V8 internals, natives syntax, snapshots, inspector protocol, tracing, V8 error text — `tests/cross-runtime/engine-specific.txt`) moves bun and deno up by 4–7 points and nub by none.
- Node itself passes 99.1% of its own suite on the measuring host once the harness runs it the way Node's runner does (pseudo-terminal + `.out` comparison for `pseudo-tty/`, the compiled `ffi/` fixture, a full checkout so `doc/`, `deps/npm` and `benchmark/` resolve); the 49 residual failures are the system CA store, the network, reporter snapshots and load.
...
- 2026-08-30 — Refreshed the headline lens figures from the regenerated results: a `node:test` file now runs under `bun test` (mirroring the `deno test` treatment, and bun's own runner), so bun's numbers rose across every lens (68.1 → 70.1 on Deno's lens); nub's reflect the intervening resolver fixes.
- 2026-09-08 — Refreshed every lens figure: a test the reference Node skips is no longer counted as a Node pass, so it leaves the node-relative denominator for every runtime. The 245 `quic/` tests were the bulk of it (the official Node build has QUIC compiled out; Bun ships `node:quic` and had been running them), and every runtime's rate moved by one to two points.
