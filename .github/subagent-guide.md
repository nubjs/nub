...
- **You own the thread's `status:` field — set it yourself.** The orchestrator does not clean up after you. The only valid values are exactly: `planning · planned · enqueued · active · blocked · done · dismissed` (legacy `todo`/`plan`/`needs-decision` are still accepted as read-aliases, but write only the canonical words). Any other word (`ready`, `landing`, `investigated`, `complete`, …) is invalid and breaks the board. When your work is genuinely finished, set `done` (or `dismissed` if the decision was not-to-pursue); if it now needs a human call, `blocked`; if it is blocked on another in-flight thread, `enqueued` plus a `depends_on:` entry. **Never leave the thread `active` when you have finished** — that strands it. Also update the one-line `status_text:` to the current truth.
