...
# ── nub's DEFAULT `Isolated` linker layout, so the wrong-version hazard is measurable HERE ──
# `preserve_symlinks_isolated_layout` measures it on a host where realpath works; the same fixture
# under the real AppContainer is what says whether a realpath repair PRESERVED resolution or
# merely stopped calling realpath. `foo`'s private dependency is `bar@2.0.0`, a sibling inside its
# own store cell; an unrelated `bar@1.0.0` sits at the layout's top level, which is where a
# link-path walk lands. Junctions rather than symlinks: a directory symlink needs Developer Mode
# or elevation, a junction needs neither, and Node's realpath resolves both.
$isoRoot = Join-Path $dataDir 'iso'
$isoNm = Join-Path $isoRoot 'node_modules'
$isoStore = Join-Path $isoNm '.aube'
foreach ($pair in @(, @('bar@2.0.0', '2.0.0')) + @(, @('bar@1.0.0', '1.0.0'))) {
  $cell = Join-Path $isoStore "$($pair[0])\node_modules\bar"
  New-Item -ItemType Directory -Force -Path $cell | Out-Null
  "module.exports={v:'$($pair[1])'};" |
    Set-Content -LiteralPath (Join-Path $cell 'index.js') -Encoding ASCII
  "{`"name`":`"bar`",`"version`":`"$($pair[1])`"}" |
    Set-Content -LiteralPath (Join-Path $cell 'package.json') -Encoding ASCII
}
$isoFooCell = Join-Path $isoStore 'foo@1.0.0\node_modules\foo'
New-Item -ItemType Directory -Force -Path $isoFooCell | Out-Null
"module.exports={barVersion:require('bar').v};" |
  Set-Content -LiteralPath (Join-Path $isoFooCell 'index.js') -Encoding ASCII
'{"name":"foo","version":"1.0.0","main":"index.js"}' |
  Set-Content -LiteralPath (Join-Path $isoFooCell 'package.json') -Encoding ASCII
function New-Junction([string]$link, [string]$target) {
  $null = & cmd /c mklink /J "$link" "$target" 2>&1
}
New-Junction (Join-Path $isoStore 'foo@1.0.0\node_modules\bar') (Join-Path $isoStore 'bar@2.0.0\node_modules\bar')
New-Junction (Join-Path $isoNm 'foo') $isoFooCell
New-Junction (Join-Path $isoNm 'bar') (Join-Path $isoStore 'bar@1.0.0\node_modules\bar')
'{"name":"iso"}' | Set-Content -LiteralPath (Join-Path $isoRoot 'package.json') -Encoding ASCII
$env:BT_ISO_FOO = Join-Path $isoNm 'foo'
Fact 'iso-fixture-foo' $env:BT_ISO_FOO
# The links must actually BE links, or the version cell measures a plain-directory copy and
# "resolved bar@2.0.0" would be true for a reason that has nothing to do with symlink resolution.
$isoLinks = @((Join-Path $isoNm 'foo'), (Join-Path $isoNm 'bar'),
              (Join-Path $isoStore 'foo@1.0.0\node_modules\bar'))
$isoLinkTypes = @($isoLinks | ForEach-Object {
  "$(Split-Path $_ -Leaf)=$((Get-Item -LiteralPath $_ -Force -ErrorAction SilentlyContinue).LinkType)"
})
Fact 'iso-fixture-link-types' ($isoLinkTypes -join ' ')
Prop 'iso-fixture-is-really-linked' (@($isoLinkTypes | Where-Object { $_ -notmatch '=Junction$' }).Count -eq 0) `
  "all three store links must be junctions or the wrong-version control is vacuous: $($isoLinkTypes -join ' ')"
...
function Invoke-Arm {
  param(
    ...
    # `NODE_OPTIONS` rather than argv, because that is how nub actually delivers a preload
    # (`windows_build_jail_node_options`) — an argv-only measurement would leave the delivery
    # channel itself untested. Set for the arm's launch and cleared after, so no arm inherits
    # another's.
    [string]$NodeOptions = '',
  ...
  try {
    ...
    if ($NodeOptions) { $env:NODE_OPTIONS = $NodeOptions } else { Remove-Item Env:NODE_OPTIONS -ErrorAction SilentlyContinue }
    Fact "$Name/node-options-len" $(if ($NodeOptions) { $NodeOptions.Length } else { 0 })
...
# ────────────── the realpath shim's NODE_OPTIONS, built the way nub builds it ──────────────
# Read from the SHIPPING source file rather than restated here, so the arm cannot go green on a
# copy that has drifted from what the product delivers. Base64 for the same reason nub uses it:
# `NODE_OPTIONS` is split on whitespace and base64's alphabet contains none, so the payload cannot
# be truncated into a half-loaded preload. The roots substituted here are the jail's granted
# anchors, which is exactly what `windows_realpath_node_options` passes.
$shimSrc = Join-Path $PSScriptRoot '..\..\crates\nub-sandbox\src\backend\windows_realpath_shim.js'
$shimOptions = ''
if (Test-Path -LiteralPath $shimSrc) {
  $rootsJson = (@($runtimeDir, $dataDir, $root) | ForEach-Object { $_ -replace '\\', '\\' } |
    ForEach-Object { '"' + $_ + '"' }) -join ','
  $js = (Get-Content -LiteralPath $shimSrc -Raw).Replace('__NUB_REALPATH_ROOTS_JSON__', "[$rootsJson]")
  $b64 = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($js))
  $shimOptions = "--preserve-symlinks-main --import data:text/javascript;base64,$b64"
  Fact 'shim-source' $shimSrc
  Fact 'shim-roots' $rootsJson
  Fact 'shim-payload-chars' $b64.Length
} else {
  Fact 'shim-source' "MISSING $shimSrc"
}
Prop 'shim-source-present' ($shimOptions -ne '') `
  "the shipping shim must be readable from the checkout or the shim arm measures nothing: $shimSrc"
...
# 1b. THE RESOLUTION TRUTH BASELINE. Unconfined AND unflagged, so `isolated-layout-version`
#     reports what Node resolves when nothing is interfering — the answer every repair must match.
#     Separate from `plain` because `plain` carries the two flags for one-variable comparability,
#     and those flags are themselves what mis-resolves.
Invoke-Arm -Name 'plain-noflags' -AppContainer $false -Cwd $runtimeDir -NodeFlags @()
...
# 9. THE REALPATH SHIM — the candidate repair, delivered the way nub delivers a preload.
#    Grants byte-identical to arm 3; the ONLY differences are the flags on argv (withheld, so
#    nothing skips realpath tree-wide) and `NODE_OPTIONS` carrying `--preserve-symlinks-main` plus
#    the `data:` `--import`. Read against arm 3 it says whether the repair PRESERVES resolution;
#    read against `ac-noflags` it says whether it repairs anything at all.
if ($shimOptions) {
  Invoke-Arm -Name 'ac-shim' -AppContainer $true -GrantRX @($runtimeDir) `
    -GrantModify @($dataDir) -Cwd $runtimeDir -NodeFlags @() -NodeOptions $shimOptions
}

# 10. THE SHIM AS ENTRY-POINT ARM. `node <deep file>` with the shim, no tree-wide flag: proves the
#     entry point survives on `--preserve-symlinks-main` alone while dependency resolution is
#     carried by the shim rather than by the disqualified flag.
if ($shimOptions) {
  Invoke-Arm -Name 'ac-shim-entry-deep' -AppContainer $true -GrantRX @($runtimeDir) `
    -GrantModify @($dataDir) -Cwd $runtimeDir -EntryFile $deep -NodeFlags @() -NodeOptions $shimOptions
}
