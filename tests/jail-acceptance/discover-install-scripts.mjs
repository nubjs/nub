...
const CANDIDATES = [
  ...
  //
  // The list is now the union of the former hand-curated set with every carrier found over npm ranks
  // 1-25,500 — the band above 100,000 weekly downloads — by a scan using the shipped rule. That rule
  // reached its current form through the counts it produced over this same band: a top-3 window found
  // 244 carriers, a top-5 window 362, no window 403, and summing each package's carrying versions
  // rather than taking its best one 420. `pickInstalledVersion` carries the reasoning.
  //
  ...
  // nothing rechecks.
  '@andrewstory18/is-real-odd', '@angular/cli', '@anthropic-ai/claude-code', '@apollo/protobufjs',
  '@apollo/rover', '@ast-grep/cli', '@astryxdesign/core', '@aws-amplify/cli', '@azure/mcp',
  ...
  '@coreui/coreui', '@coreui/react', '@coze/cli', '@databricks/sql',
  '@datadog/mobile-react-native', '@datadog/mobile-react-native-session-replay',
  '@datadog/native-appsec', '@datadog/native-iast-taint-tracking', '@datadog/native-metrics',
  '@datadog/pprof', '@deepseek-ai/dsh-subprocess-local', '@depot/cli', '@discordjs/opus',
  '@elastic/eui', '@embedded-postgres/linux-x64', '@evilmartians/lefthook',
  '@ffmpeg-installer/linux-arm64', '@ffmpeg-installer/linux-x64', '@ffprobe-installer/linux-x64',
  '@firebase/util', '@fission-ai/openspec', '@fortawesome/fontawesome-common-types',
  ...
  '@fortawesome/free-solid-svg-icons', '@github/keytar', '@google/genai', '@googleworkspace/cli',
  '@hatchet-dev/typescript-sdk', '@heroui/shared-utils', '@hyperjump/json-pointer',
  '@hyperjump/json-schema', '@hyperjump/json-schema-core', '@ibm/plex', '@ibm/plex-sans',
  '@ibm/plex-sans-hebrew', '@ibm/plex-sans-thai', '@ibm/plex-sans-thai-looped', '@ibm/plex-serif',
  '@infisical/cli', '@journeyapps/wa-sqlite', '@larksuite/cli', '@launchql/protobufjs',
  '@lavamoat/preinstall-always-fail', '@matrix-org/matrix-sdk-crypto-nodejs',
  '@maxmind/geoip2-node', '@medusajs/telemetry', '@memlab/cli', '@microsoft/m365agentstoolkit-cli',
  '@modelcontextprotocol/ext-apps', '@modelcontextprotocol/inspector', '@mongodb-js/zstd',
  '@moonrepo/cli', '@mui/x-telemetry', '@napi-rs/simple-git-linux-x64-gnu',
  '@napi-rs/simple-git-linux-x64-musl', '@nestjs/core', '@newrelic/fn-inspect',
  '@newrelic/native-metrics', '@nrwl/angular', '@openapitools/openapi-generator-cli',
  '@opencode-ai/cli', '@openrouter/sdk', '@openuidev/lang-core', '@pact-foundation/pact-core',
  '@parcel/watcher', '@percy/core', '@playwright/browser-chromium', '@playwright/browser-firefox',
  '@playwright/browser-webkit', '@pnpm/exe', '@posthog/cli', '@prisma/client', '@prisma/engines',
  '@progress/kendo-licensing', '@pulumi/aws', '@pulumi/aws-native', '@pulumi/awsx',
  '@pulumi/azure-native', '@pulumi/command', '@pulumi/datadog', '@pulumi/docker',
  '@pulumi/docker-build', '@pulumi/gcp', '@pulumi/kubernetes', '@pulumi/random', '@railway/cli',
  '@react-hookz/deep-equal', '@react-spring/core', '@reown/appkit', '@salesforce/cli',
  '@sap/hana-client', '@scarf/scarf', '@sentry-internal/node-cpu-profiler',
  '@sentry-internal/node-native-stacktrace', '@sentry/cli', '@sentry/node-cpu-profiler',
  '@sentry/profiling-node', '@shopify/react-native-skia', '@stacksjs/ts-webp',
  '@stdlib/math-base-assert-is-integer', '@stdlib/math-base-special-exp',
  '@stdlib/math-base-special-kernel-cos', '@stdlib/math-base-special-kernel-sin',
  '@stdlib/math-base-special-ldexp', '@stdlib/math-base-special-ln',
  '@stdlib/number-float64-base-exponent', '@stdlib/number-float64-base-normalize',
  '@stellar/stellar-sdk', '@stoprocent/noble', '@strapi/strapi', '@sveltejs/kit', '@swc/core',
  '@tailwindcss/oxide', '@temporalio/core-bridge', '@tensorflow/tfjs-node', '@tloncorp/tlon-skill',
  '@toruslabs/eccrypto', '@tree-sitter-grammars/tree-sitter-yaml', '@trufflesuite/bigint-buffer',
  '@tsparticles/engine', '@turbodocx/html-to-docx', '@uirouter/core',
  '@vaadin/vaadin-usage-statistics', '@vercel/speed-insights', '@vscode/ripgrep',
  '@vscode/sqlite3', '@vscode/vsce-sign', '@vscode/windows-registry', '@whiskeysockets/baileys',
  '@zowe/secrets-for-zowe-sdk', 'admin-lte', 'agent-browser', 'agentdb', 'altcha',
  'ant-design-vue', 'appium', 'appium-chromedriver', 'appium-ios-tuntap',
  'applicationinsights-native-metrics', 'appmetrics', 'argon2', 'autoevals', 'aws-crt', 'aws-sdk',
  'azure-functions-core-tools', 'backport', 'baileys', 'bcrypt', 'better-sqlite3', 'bigint-buffer',
  'bignum', 'blake-hash', 'blake3', 'bluetooth-hci-socket', 'bootstrap-vue', 'braintrust',
  'browser-tabs-lock', 'btch-downloader', 'bufferutil', 'bun', 'canvas', 'cbor-extract', 'ccxt',
  'chrome-local-mcp', 'chromedriver', 'chromium', 'classic-level', 'cline', 'cloudflared',
  'comfyui-mcp', 'console-stamp', 'contentful', 'core-js', 'core-js-pure', 'cpu-features',
  'cucumber-expressions', 'cwebp-bin', 'cy2', 'cypress', 'dd-trace', 'deasync', 'deno', 'detox',
  'dprint', 'dtrace-provider', 'duckdb', 'edgedriver', 'ejs', 'electron', 'electron-winstaller',
  'epoll', 'es5-ext', 'esbuild', 'estimo', 'exifreader', 'faiss-node', 'fallow', 'farmhash',
  'fetch-mock', 'ffi-napi', 'ffmpeg-static', 'final-form', 'flag-icon-css', 'flow-bin',
  'free-email-domains', 'fs-xattr', 'fsevents', 'full-icu', 'gatsby', 'gatsby-cli',
  ...
  'ip-num', 'isolated-vm', 'javascript-obfuscator', 'jest-preview', 'jpegtran-bin', 'jsonpath',
  'jss', 'keccak', 'kerberos', 'keytar', 'koffi', 'lefthook', 'less', 'level', 'leveldown',
  'libpg-query', 'libpq', 'libxmljs', 'libxmljs2', 'llnode', 'lmdb', 'lz4', 'lzo', 'maplibre-gl',
  'memlab', 'microtime', 'mongodb-client-encryption', 'mongodb-memory-server', 'mozjpeg',
  'msgpackr-extract', 'msw', 'n8n-nodes-evolution-api', 'native-keymap', 'nestjs-pino', 'netlify',
  'netlify-cli', 'ngrok', 'ngx-infinite-scroll', 'nice-napi', 'node', 'node-expat', 'node-hid',
  'node-jq', 'node-libcurl', 'node-liblzma', 'node-llama-cpp', 'node-pty', 'node-report',
  'node-sass', 'nodejieba', 'nodemon', 'nodent-runtime', 'nuxt', 'nx', 'odbc', 'odiff-bin',
  'onnxruntime-node', 'openclaw', 'opencode-ai', 'optipng-bin', 'oracledb', 'oxc-resolver',
  'paper', 'parcel', 'parse-domain', 'phantomjs-prebuilt', 'playwright-chromium',
  'playwright-webkit', 'pngquant-bin', 'pnpm', 'postinstall-postinstall',
  'postman-code-generators', 'pprof', 'pre-commit', 'prisma', 'protobufjs', 'puppeteer', 'radium',
  're2', 'react-ace', 'react-final-form', 'react-grab', 'react-jsx-parser',
  ...
  'react-native-video', 'react-native-webrtc', 'react-table', 'realm', 'redis-memory-server',
  'ref-napi', 'robotjs', 'rocksdb', 'scrypt', 'secp256k1', 'segfault-handler', 'serialport',
  'serverless', 'sha3', 'sharp', 'simple-git-hooks', 'sinon', 'skia-canvas', 'sleep', 'snappy',
  'snyk', 'sodium-native', 'sonar-scanner', 'spawn-sync', 'sqlite3', 'squawk-cli', 'sse4_crc32',
  'ssh2', 'storybook-addon-remix-react-router', 'stream-chat', 'stream-chat-react',
  ...
  'v8-profiler-next', 'vercel', 'vis-data', 'vis-network', 'vis-timeline', 'vnu-jar', 'vue-demi',
  'vue-echarts', 'wd', 'web3', 'web3-bzz', 'web3-shh', 'websocket', 'win-ca', 'wix-style-react',
  'workerd', 'wrtc', 'yarn', 'yo', 'yorkie', 'zeromq', 'zlib', 'zopflipng-bin', 'zstd-napi',
];
