...
**The population used to be incomplete inside that band as well, and closing that was worth far more than moving the gate.** Ranking each package's versions by actual weekly downloads over ranks 1-25,500 finds 244 install-script carriers; the population held **107**. The other 137 carried **286.7M weekly downloads - 30.8% of the population's own weight** - and are listed in [`uncovered-above-gate.tsv`](uncovered-above-gate.tsv). `sharp` at 83M and `protobufjs` at 75M were the two largest. Five were hand-verified against their manifests, including `tree-sitter-python` running `node-gyp-build`. Both causes are fixed and the population is regenerated; the paragraphs below record what the hole was, because the shape of it is what the discovery script now guards against.
...
⇒ **So it was a MEASUREMENT hole rather than a live breakage** - the 286.7M weekly downloads were never being broken by the jail, only invisible to the harness. That is what made fixing discovery worth more than widening the sweep by hand.

Two separate causes produced it, and only one was subtle. 56 of the 137 run their install script on a version that is *not* `latest` - `sharp`'s most-installed release, 0.34.5 at 30.7M weekly, runs one while `latest` does not - and every lookup judged a candidate by `latest`, so no candidate list could have surfaced them. The other 81 carry a script on `latest` and were simply absent from the `CANDIDATES` array, so its claim to union in every carrier in the top-downloaded set had stopped holding whatever was true when it was written. `discover-install-scripts.mjs` now ranks each candidate's versions by actual weekly downloads, and its array is regenerated from the scan that found the 244.
