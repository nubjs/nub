...
⛔ Every below-gate figure in this section was produced by a `latest`-only scan and is being re-measured with the version-aware one. Treat the counts and the 2.1% as floors until that lands; the 23% of above-gate carriers that only a version-aware scan can see suggests the correction is not small.
