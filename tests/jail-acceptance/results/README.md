...
⛔ **That rising density is itself worth watching.** Carriers thicken as rank falls - 1.63% in the nearer band against 2.31% in the deeper one - which is consistent with the migration the discovery script documents, where popular packages replace install scripts with prebuilt binaries shipped as platform `optionalDependencies` (`sharp`, `better-sqlite3`, `playwright`, `electron`). If install scripts are becoming a long-tail phenomenon, a download-threshold population drifts further from what it samples every year without anything reporting that it has. Re-running the scan periodically is the cheap way to notice.
