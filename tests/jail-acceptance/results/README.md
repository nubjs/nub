...
**The 1% floor is the rule's only remaining parameter, and it behaves nothing like the window did.** Sweeping it across a fifty-fold range over the same 25,421 packages: 507 carriers at 0.1%, 420 at 1%, 378 at 5%. So a 50x change in the floor moves the count by 1.34x, where a 1.67x change in the window moved it by 1.48x — the floor is a threshold with a shallow response, the window was a knob with a steep one. That is why one was removed and the other kept, and it is measured rather than argued. `scan-below-gate.mjs --min-share` reproduces it.
...
⭐ **176 of the 450 had never been measured by anything, and all three platforms have now swept them: zero `JAIL-CAUSED` everywhere.** They hold 523M weekly downloads between them — 30% of the population's weight — and include `pnpm`, `ejs`, `husky`, `fsevents`, `nodemon`, `@nestjs/core` and `sinon`. Per-platform results are in [`never-swept-sweep-linux.tsv`](never-swept-sweep-linux.tsv), [`-macos`](never-swept-sweep-macos.tsv) and [`-win`](never-swept-sweep-win.tsv).

| platform | OK | UPSTREAM | NO-SCRIPT-RAN | JAIL-CAUSED | confined spawns | rows engaged | jailed-arm |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| linux | 143 | 23 | 10 | **0** | 265 | 166 | 16.4 min |
| macos | 146 | 20 | 10 | **0** | 274 | 166 | 23.1 min |
| win | 164 | 2 | 10 | **0** | 270 | 166 | 145.4 min |

**The three agreeing on WHICH rows engaged is worth more than any one of them passing.** All three ran a confined script on exactly the same 166 of 176, and on every platform the ten that did not are precisely the ten `NO-SCRIPT-RAN` rows — so nothing passed by quietly never engaging the jail, which is the way a sweep like this reads clean while testing nothing. The OK/UPSTREAM split differs because old native packages fail to build differently per platform, not because the jail behaved differently.

⛔ **Windows is 9x slower per package, and that is the number to plan around.** 49.6s against Linux's 5.6s and macOS's 7.9s on the identical set — a grant costs a DACL walk per confined spawn there, and it dominates. It is also why the previous estimates in this file were wrong in both directions at once: they projected from the older population's mix, which overstated Linux and macOS by 2x and understated Windows by the same again. Per-package costs are a property of the package set, so measure them on the set you mean.
...
⛔ **Extending the population is a standing cost, not a one-off, and it differs enormously by platform.** The sweep runs packages serially (`for pkg in $PKGS`), and the per-package jailed-arm times these files record (`jail-s=`) put the current 450 at roughly 42 min on Linux, 1h on macOS and **6h on Windows** at the rates measured over the 176 new members. Treat those as an order of magnitude rather than a forecast: the mix drives them, and the 450 is a heavier mix than the 176. Those are the jailed arm alone; the two control arms run only for a row the jailed arm did not clear, which took one measured Windows sweep from 1.05h of jailed-arm time to 1h42m of wall clock, so budget wall clock roughly half again above them. Discovery is the cheap half — about 45 minutes for a whole band, of which 11 is the ranking sweep, so a re-run against a cached ranking costs about 35 minutes, on one machine rather than one per platform. Scan wide, sweep by weight.
