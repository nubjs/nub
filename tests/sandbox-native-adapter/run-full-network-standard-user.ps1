param(
    ...
    [Parameter(Mandatory=$true)][string]$FileBrokerFixtureBinary,
...
if (!(Test-Path -LiteralPath $FileBrokerFixtureBinary -PathType Leaf)) { throw "File-broker fixture is missing: $FileBrokerFixtureBinary" }
...
try {
    ...
    Copy-Item $FileBrokerFixtureBinary (Join-Path $stage 'file-broker-fixture.dll')
    Copy-Item $BinaryManifest (Join-Path $stage 'binary-sha256.json')
@'
param([string]$Stage, [ValidateSet('full', 'owner-pipe-diagnostic')][string]$Mode, [string]$ExpectedUser)
$ErrorActionPreference = 'Stop'
$profileRoot = [Environment]::GetFolderPath('UserProfile')
if (!$profileRoot -or $profileRoot -like '*systemprofile*') { throw 'A normal user profile is required' }
$identity = [Security.Principal.WindowsIdentity]::GetCurrent()
$principal = [Security.Principal.WindowsPrincipal]::new($identity)
if (!$ExpectedUser -or $identity.Name -ine "$env:COMPUTERNAME\$ExpectedUser") { throw "Unexpected ordinary-user identity: $($identity.Name)" }
if ($identity.User.Value -in @('S-1-5-18', 'S-1-5-19', 'S-1-5-20') -or $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw "Privileged identity is not an ordinary user: $($identity.Name) ($($identity.User.Value))" }
Write-Host "STANDARD_USER_FULL_NETWORK_IDENTITY=$($identity.Name);$($identity.User.Value)"
Write-Host 'STANDARD_USER_FULL_NETWORK_ORDINARY_USER=ok'
...
if (!(Test-Path -LiteralPath $fileBrokerFixture -PathType Leaf)) { throw 'Required file-broker fixture DLL is missing from the staged artifacts' }
...
$env:NUB_FILE_BROKER_TEST_DLL = $fileBrokerFixture
...
$runs = @(
    ...
    @{ file='nub_sandbox_lib.exe'; label='proxy-nat64'; filter='proxy::tests::nat64_range_checks_preserve_prefix_boundaries_and_private_matching'; summary=$one; exact=$true },
    @{ file='nub_sandbox_lib.exe'; label='proxy-metadata-link-local'; filter='proxy::tests::blocks_metadata_and_link_local_egress'; summary=$one; exact=$true },
    @{ file='nub_sandbox_lib.exe'; label='proxy-rfc1918-ula'; filter='proxy::tests::rfc1918_and_ula_blocked_by_default_but_opt_in_reachable'; summary=$one; exact=$true },
    @{ file='nub_sandbox_lib.exe'; label='proxy-connect-tier'; filter='proxy::tests::connect_upstream_blocks_by_tier_but_reaches_allowed_target'; summary=$one; exact=$true },
    @{ file='nub_sandbox_lib.exe'; label='windows-broad-write-roots'; filter='backend::windows::tests::explicit_broad_write_roots_are_granted'; summary=$one; exact=$true },
    @{ file='nub_sandbox_lib.exe'; label='drive-root-default-glob'; filter='compiler::defaults::tests::drive_root_subtree_globs_preserve_absolute_node'; summary=$one; exact=$true },
    @{ file='nub_sandbox_lib.exe'; label='drive-root-path-spelling'; filter='matcher::path::tests::drive_root_preserves_absolute_and_relative_spellings'; summary=$one; exact=$true },
    @{ file='nub_sandbox_lib.exe'; label='drive-root-path-descendants'; filter='matcher::path::tests::drive_root_descendant_twin_matches_descendants'; summary=$one; exact=$true }
)
$runs += @(
...
'@ | Set-Content -Encoding ascii (Join-Path $stage 'run-full-network.ps1')
    ...
    $process = Start-Process powershell.exe -Credential $credential -LoadUserProfile -ArgumentList @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', "$stage\run-full-network.ps1", $stage, $Mode, $name) -WorkingDirectory $stage -PassThru -RedirectStandardOutput "$ReportDirectory/standard-user.log" -RedirectStandardError "$ReportDirectory/standard-user-error.log"
    ...
    if ($exitCode -ne 0 -or !(Select-String -Path "$ReportDirectory/standard-user.log" -Pattern $successPattern) -or !(Select-String -Path "$ReportDirectory/standard-user.log" -Pattern '^STANDARD_USER_FULL_NETWORK_ORDINARY_USER=ok$')) { $exitCode = 1 }
} finally {
