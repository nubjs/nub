...
if (!$outer.Contains('[Parameter(Mandatory=$true)][string]$FileBrokerFixtureBinary')) { throw 'Expected a required file-broker fixture DLL' }
...
if (!$inner[0].Value.Contains("`$env:NUB_FILE_BROKER_TEST_DLL = `$fileBrokerFixture")) { throw 'Expected the ordinary-user file-broker DLL binding' }
if (!$inner[0].Value.Contains('STANDARD_USER_FULL_NETWORK_ORDINARY_USER=ok')) { throw 'Expected strict ordinary-user provenance marker' }
...
foreach ($filter in @(
    'proxy::tests::nat64_range_checks_preserve_prefix_boundaries_and_private_matching',
    'proxy::tests::blocks_metadata_and_link_local_egress',
    'proxy::tests::rfc1918_and_ula_blocked_by_default_but_opt_in_reachable',
    'proxy::tests::connect_upstream_blocks_by_tier_but_reaches_allowed_target',
    'backend::windows::tests::explicit_broad_write_roots_are_granted',
    'compiler::defaults::tests::drive_root_subtree_globs_preserve_absolute_node',
    'matcher::path::tests::drive_root_preserves_absolute_and_relative_spellings',
    'matcher::path::tests::drive_root_descendant_twin_matches_descendants'
)) {
    if (!$inner[0].Value.Contains("filter='$filter'")) { throw "Expected full replay filter: $filter" }
}
Write-Host 'FULL_NETWORK_REPLAY_FULL_FILTERS=8'
