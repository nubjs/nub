...
```text
WARN store <HOME>/.local/share/nub/store/v1/files is not writable (inside the Codex sandbox); new packages go to the project-local store <TMP> for this run code=WARN_NUB_STORE_FALLBACK
