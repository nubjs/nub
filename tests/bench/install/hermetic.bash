...
#                           ~/.cache/nub-bench/registry). Holds the
...
BENCH_HERMETIC_CACHE="${BENCH_HERMETIC_CACHE:-$HOME/.cache/nub-bench/registry}"
...
#
# Warming runs one install per PM (nub, bun, pnpm, npm, yarn, deno)
# so the cache is the *union* of every tool's resolution set —
# each resolver picks its preferred versions independently (e.g. nub
...
#
# nub is skipped when `$NUB_BIN` isn't built yet, so warming is still
# bootstrap-safe for flows that warm before compiling nub.
_hermetic_warm() {
	...
	if [ "${BENCH_TOOLS:-nub,bun,pnpm,npm,yarn,deno}" != "nub,bun,pnpm,npm,yarn,deno" ]; then
	...
	local warm_root
	warm_root=$(mktemp -d "${TMPDIR:-/tmp}/nub-bench-warm.XXXXXX")
	...
	_warm_one() {
		...
		*) return 0 ;;
		...
		# `npm_config_registry` — nub reads `.npmrc` and does not
		...
		# Without the `.npmrc` files nub's warm install silently hits
