...
const RUNTIME_DEFINE = runtimeConfig.define || {};
...
export function loadTranspile(url, ext) {
  ...
  // changes the output without changing `lang`, so the key must see it. `define`
  // rides along for the same reason — it substitutes into the emitted source.
  const runtimeHash = JSON.stringify({ define: RUNTIME_DEFINE, loader: RUNTIME_LOADER[ext] || null, tsconfig: RUNTIME_TSCONFIG || null });
