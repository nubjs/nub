...
# Usage: curl -fsSL https://raw.githubusercontent.com/colinhacks/nub/main/install.sh | bash

# Windows: delegate to PowerShell
if [[ ${OS:-} = Windows_NT ]]; then
    powershell -c "irm https://raw.githubusercontent.com/colinhacks/nub/main/install.ps1 | iex"
    exit $?
fi
...
if [[ "$version" == latest ]]; then
    version=$(curl -fsSL "https://api.github.com/repos/colinhacks/nub/releases/latest" | grep '"tag_name"' | sed -E 's/.*"v(.*)".*/\1/')
    if [[ -z "$version" ]]; then
...
url="https://github.com/colinhacks/nub/releases/download/v${version}/nub-${target}.tar.gz"
