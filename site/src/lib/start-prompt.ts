/* The exact text the homepage "Copy agent prompt" button copies. A short,
   paste-into-your-agent prompt modeled on Vite+'s: it orients the agent, points
   it at the full adoption guide (https://nubjs.com/start.md) and command
   reference (https://nubjs.com/llms-full.txt) to read, and lists the day-to-day
   commands — the detailed 7-step playbook lives in start.md, which the agent
   fetches and follows rather than having it pasted inline. Keep this in sync
   with the install commands in InstallTabs and the surface in start.md. */
export const START_PROMPT = `I want to use nub in my project. nub is a single Rust CLI that augments your installed Node.js — one tool that runs TypeScript and JSX files directly, runs your scripts and local CLIs, manages packages, and provisions Node versions, with no new runtime and no lock-in. First, read https://nubjs.com/start.md — the guide to adopting nub — and follow it; the full command reference is https://nubjs.com/llms-full.txt. Install the \`nub\` CLI:
