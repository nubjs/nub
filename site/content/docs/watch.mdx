...
For how `nub` runs files in the first place, see [Running files](/docs/runtime); for scripts, [Running scripts](/docs/run).
...
A `--watch` placed after a script name is forwarded *to the script* like any other argument — `nub run test --watch` runs your test tool's own watch mode, it does not invoke Nub's watcher. See [`nub run` → Forward arguments](/docs/run#argument-forwarding).
...
```console
$ nub watch src/server.ts
Completed running 'src/server.ts'. Waiting for file changes before
restarting...
...
- [Script runner](/docs/run) — `nub run` and its workspace surface.
- [Bin runner](/docs/nubx) — `nubx`, running local CLIs from `node_modules/.bin`.
