...
This page is a cookbook. For how `nub` runs files in the first place, see [Running files](/docs/runtime); for scripts, [Running scripts](/docs/runner/run).
...
A project [`nub.jsonc`](/docs/config) applies the same way: its runtime settings — `preload`, `envFile`, `define`, `loader`, `conditions`, `tsconfig` — are in force on every restart.
