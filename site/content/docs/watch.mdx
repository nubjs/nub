...
This restarts on source changes the same way the file form does, while still going through `nub run` — so your `npm_*` environment variables, `pre`/`post` lifecycle hooks, and `node_modules/.bin` PATH chain are all in place. See [Running scripts](/docs/run) for the full `nub run` surface.
