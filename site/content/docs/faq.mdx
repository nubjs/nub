...
The short, indexed answers to the common questions about Nub — what it is, how it works, and what it deliberately is not.
...
No. Nub does not patch Node source, ship a custom-built Node binary, or embed `libnode`. Every augmentation rides on Node's public extension surfaces. The "would a user on plain Node, plus the corresponding `module.register()` / `--import` / npm addon, get the same result?" question always answers yes. See `architecture.md#augmenter-not-fork` for the load-bearing version of this distinction.

## How is it different from plain `node`?

`nub <file>` is flag-for-flag compatible with `node <file>` — same argv shape, same flag set, same behavior — and on top of that adds:
...
## Does it support all Node core modules (`node:fs`, `node:http`, `node:test`, etc.)?
...
**Augmented modes require Node 22.15+.** That's the minimum version that ships synchronous `module.registerHooks()` — Nub's load-bearing primitive for the transpile-on-import path. Node 22.15 was released April 2025 and Node 22 LTS runs through April 2027, so the floor sits inside the current LTS window.
...
On Node older than 22.15, augmented commands emit a tagged error telling you to upgrade Node or use compat mode. Full details: `runtime/target-version.md`.
...
Yes. `nub run` reads workspace topology from `package.json`'s `"workspaces"` field (`npm`, `yarn`) and `pnpm-workspace.yaml` (`pnpm`), and supports the `-r` / `--filter` semantics every workspace user already knows from `pnpm`:
...
Topological ordering, parallelism, and `--workspace-root` are honored. The point is what *isn't* honored: the ~150 ms `pnpm` bootstrap, paid per package. In a 30-package monorepo, `pnpm -r run build` pays that bootstrap 30 times before any of your code runs. `nub -r run build` pays it zero.

## Does it replace `npx`?

Yes — `nubx` (also available as `nub exec`) is the equivalent. Locally installed CLIs live in a standard place: `node_modules/.bin/`, with the resolution algorithm well-defined (walk up the directory tree, check `.bin/` at each level). `nubx` is that lookup, in Rust. Local `node_modules/.bin` resolution returns in single-digit milliseconds. For binaries that aren't installed, Nub detects the package manager your project uses and delegates the fetch — so your lockfile stays consistent.

## Does it replace `tsx` / `ts-node`?

Yes. `nub script.ts` runs TypeScript directly, with the full TS surface (not just type stripping), respecting your `tsconfig.json`, with inline source maps so stack traces point at your `.ts` source. Full details: `runtime/ts-transpilation.md`.

## Does it replace `nodemon` / `tsx watch`?

Partially in v0.1, fully in v0.x. `nub script.ts --watch` and `nub run dev --watch` use Node's built-in `--watch` flag, which works fine for many use cases. A first-class hot-reload story (preserving in-memory state across reloads, HMR-style module replacement) is on the v0.x roadmap; see `runtime/hot-mode.md`.

## What about `dotenv-cli`?

Not needed. Nub loads `.env`, `.env.local`, `.env.<NODE_ENV>`, and `.env.<NODE_ENV>.local` automatically — with the Vite/Bun-style precedence rules — and injects them into the process environment *before* Node starts. `${VAR}` and `$VAR` expansion is supported, including nested references and cycle detection. `.env.local` is intentionally skipped under `NODE_ENV=test`, following the Next.js convention — local secrets shouldn't leak into your test suite. Full details: `runtime/env-loading.md`.
...
- **`module.registerHooks()`** (Node 22.15, April 2025) — synchronous loader hooks. Async hooks add per-import latency that makes TypeScript-on-import unusable for cold start; the sync API is what makes the transpiler viable.
...
Two buckets. **Nub replaces directly:** `dotenv`, `cross-env`, `tsx` / `ts-node`, `nodemon`, `tsconfig-paths`, `npx` (replaced by `nubx` / `nub exec`). **Modern Node already obsoletes; Nub's Node 22.15+ floor lets you inherit:** `node-fetch` / `cross-fetch` / `whatwg-fetch` (native `fetch` since 18), `abort-controller` (native since 15), `uuid` for the v4 case (`crypto.randomUUID` since 14.17), `rimraf` (`fs.rmSync({ recursive: true, force: true })` since 14.14), `mkdirp` (`fs.mkdirSync({ recursive: true })` since 10.12), `glob` / `globby` for basic cases (`fs.glob` since 22), `form-data` (native `FormData` since 18), `ws` client (native `WebSocket` since 22.5, Nub polyfills on older). The second bucket's credit is Node's — running on a modern-Node floor is what makes the inheritance practical without per-project Node-version verification. Full catalog with weekly download counts, Node-version notes, and the explicit "we don't replace this" list: `runtime/polyfill-obsoletion-catalog.md`.

## What's `nub-resolver`?

`nub-resolver` is the one Rust crate Nub ships of its own: a Rust implementation of Node's ESM resolution algorithm that layers in `tsconfig.json` `paths` and extensionless `.ts` import probing on top of vanilla Node's behavior. It builds on `oxc_resolver` for the primitives, is validated against a conformance suite ported from Node's own ESM resolver specification, and is also what powers the resolver inside `nubx` and `nub run`.
...
## Are there Nub-specific APIs or globals?

No. See above — the brand-boundary rule is absolute. The one structural exception is `@types/nub` on DefinitelyTyped, which is the TypeScript-ecosystem convention for ambient type declarations (alongside `@types/node`, `@types/bun`, `@types/deno`). That's a single package, convention-driven, not a runtime-vendor namespace claim.

## What's the `--node` flag?

`--node` is a compat flag on `nub run` and `nubx`. It disables Nub's runtime augmentation for that orchestration call while keeping the CLI work (workspace, scripts, `npm_*` env, lifecycle hooks).
...
There's no `NODE_COMPAT=1` env var (dismissed 2026-05-25) and no `nub node` verb (also dismissed). Per-invocation flag on the two orchestration verbs is the one mechanism. See `architecture.md#compat-mode`.
...
Yes. On macOS and Linux, `curl -fsSL https://nub.sh/install.sh | bash`; on Windows, `powershell -c "irm https://nub.sh/install.ps1 | iex"`. Both drop a native binary into `~/.nub` and put it on your `PATH`. If you'd rather go through your package manager, `npm install -g @nubjs/nub` (or `pnpm add -g` / `yarn global add`) does the same thing.

## Can I install via `npm install -g`?

Yes — that's the primary distribution channel. `pnpm add -g @nubjs/nub` and `yarn global add @nubjs/nub` work equivalently.
...
No. `node:test` already owns this — and `vitest` and `jest` if you prefer those. Nub runs whichever you've chosen. Workers spawned by your test runner inherit Nub's augmentation, so TypeScript test files Just Work.
...
In v0.1, `nub --watch` uses Node's built-in `--watch` flag, which restarts the process when files change. That's good enough for many use cases.

A first-class hot-mode primitive — preserving in-memory state across reloads, HMR-style module replacement, sharing protocol with Vite's `import.meta.hot` — is on the v0.x roadmap. See `runtime/hot-mode.md` for the current thinking.

## When will it ship a package manager?

No current plan. As covered in [Does it replace my package manager?](#does-it-replace-my-package-manager), staying out of package management is a deliberate scope choice, not a postponement. The right answer for the foreseeable future is to be the best possible script runner sitting on top of `pnpm` / `npm` / `yarn` / `bun`, not to ship a fifth installer that competes with the four your project's tooling already understands.

## Will there be an `export default { fetch }` server primitive?

Not in v0.1. The standards-aligned `export default { fetch(req) {...} }` pattern — used by Bun, Deno, Cloudflare Workers, Vercel Edge, and Fastly — is on the v0.x roadmap, tracked at `runtime/fetch-handler-server.md`. For v0.1, use Hono, Fastify, Express, or `node:http` directly; `nub run dev` runs whatever your project's `dev` script invokes.
