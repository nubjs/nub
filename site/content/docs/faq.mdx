...
Canary may break, and each build replaces the last. A canary is at most a day behind `main`, so a fix that just landed reaches the channel on the next nightly rather than immediately. Homebrew and winget carry only stable releases.
