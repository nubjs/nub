...
rimraf                                  # fs.rmSync recursive+force, 14.14
mkdirp                                  # fs.mkdirSync recursive, 10.12
...
nub run --node test          # Nub's CLI orchestration, no augmentation
nubx --node prisma generate  # Nub's bin resolution, no augmentation
```
...
curl -fsSL https://nubjs.com/install.sh \\
  | NUB_INSTALL_DIR="$HOME/.local/nub" NUB_NO_MODIFY_PATH=1 bash
```
