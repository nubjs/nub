...
It can. The `nub install` command is a full package manager with pnpm's command-line interface. In a pnpm project it installs exactly as pnpm 12 does, from `pnpm-lock.yaml`; every other project gets `nub.lock`.

| The project has | `nub install` |
| --- | --- |
| `pnpm-lock.yaml`, `pnpm-workspace.yaml`, or a pnpm `packageManager` pin | Installs as pnpm 12, from `pnpm-lock.yaml` |
| `nub.lock`, or no lockfile | Installs from `nub.lock` |
| `package-lock.json`, `yarn.lock`, or `bun.lock` | Does not read it, and writes `nub.lock` — convert it once with [`nub pm migrate`](/docs/install/migrate) |

## Does it work with `pnpm` / `npm` / `yarn` / `bun`?

Yes. Whatever installed the project, workspace topology (`pnpm-workspace.yaml`, `npm` workspaces, `yarn` workspaces) is honored by `nub run` and `nubx` directly. The sources of truth:
...
The one behavioral difference from `npx` is that the fetch is not silent: the first fetch of a tool prompts, and CI fails closed unless you pass `-y`. See [the runner docs](/docs/nubx#registry-fetches) for the consent model.
...
corepack            # → nub pm
the PM CLI itself   # nub install / nub run
```
...
Yes. A nightly build of `main` publishes a full 8-platform build under the rolling [canary release](https://github.com/nubjs/nub/releases/tag/canary) and to npm under the `canary` dist-tag. Nights when `main` gained no code change are skipped. Install it with the install script:
