...
Yes — `nub script.ts` runs TypeScript directly, with the full TS surface (not just type stripping), respecting your `tsconfig.json`, with inline source maps so stack traces point at your `.ts` source. Full details: [File runner → TypeScript](/docs/runtime#typescript).
...
Full details: [File runner → Environment variables](/docs/runtime#environment-variables).
