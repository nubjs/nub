...
For restart-on-change, yes: `nub script.ts --watch` and `nub run dev --watch` restart the process when anything in the resolved dependency graph (plus your `.env*` files and `tsconfig.json`) changes — no glob list to maintain. Full details: [Watch mode](/docs/watch).
