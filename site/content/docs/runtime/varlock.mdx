...
The `--env-file` and `--no-env-file` flags do the same for one run. Either also clears the missing-Varlock error above, which is the way to run a schema project on a machine where Varlock will not install.

Going the other way, `"envFile": "varlock"` selects Varlock explicitly. It matters when a global `envFile` is set: a global value never takes precedence over a project's `.env.schema`, but naming Varlock in the project states the intent outright.
