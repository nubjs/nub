...
| --- | --- |
| A project dependency, or `npm i -g` | Loads it in-process before your code, with secret redaction. This is the path to want, and either install shape reaches it. |
| The standalone binary, from Homebrew or the install script | Runs it and injects the values. Redaction is unavailable — it needs Varlock inside the process, and a standalone binary has no module to import — so Nub warns once. |
