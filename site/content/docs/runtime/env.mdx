...
Secrets are redacted from the whole output stream — `console` methods, raw `process.stdout.write`, and anything a subprocess prints — because Varlock is piping it. A schema with `@generateTypes` regenerates its file on every run.
...
A dependency's `postinstall` or build script gets the schema-resolved environment too, the same as your own code. That matches what those scripts already got from Nub's `.env` loading.

One consequence is worth knowing: if your schema fails validation, the failure appears while a dependency is building rather than when you run your app. To keep the schema out of the install entirely, `nub install --ignore-scripts` runs no dependency scripts at all.
