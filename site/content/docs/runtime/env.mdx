...
The filename is the reason. Other tools, [dotenv-extended](https://www.npmjs.com/package/dotenv-extended) among them, have used `.env.schema` for their own incompatible formats for years, so Nub reads the file before saying anything and stays quiet unless it is actually `@env-spec`.
