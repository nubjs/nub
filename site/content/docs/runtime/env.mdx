...
An `@env-spec` schema — conventionally `.env.schema` — describes your environment as types, validation, which variables are secret, and where their values come from. Nub integrates with [Varlock](https://varlock.dev), which implements that format and resolves and validates the whole set before your code runs.

The filename is not exclusive to that format: other tools, [dotenv-extended](https://www.npmjs.com/package/dotenv-extended) among them, have used `.env.schema` for their own incompatible formats for years. Nub keys on the Varlock dependency being installed, not on the filename, so a `.env.schema` belonging to something else is left alone.
...
| How Varlock is installed | What Nub does |
| --- | --- |
| A project dependency, or `npm i -g` on macOS or Linux | Loads it in-process before your code, with secret redaction. This is the path to want. |
| The standalone binary from Homebrew or the install script, or `npm i -g` on Windows | Runs it and injects the values. Redaction is unavailable — it needs Varlock inside the process, which Nub cannot reach from either of those installs — so Nub warns once. |
| Not installed | Keeps loading `.env*` as usual and warns that the schema was not applied. A `.env.schema` committed for some other tool will not leave you with no environment. |
...
Type generation is the one thing Nub does not trigger. A schema with `@generateTsTypes` regenerates its file when the Varlock CLI resolves your environment, and Nub resolves in-process instead, so nothing is written. Run it explicitly when the schema changes:
...
A dependency's `postinstall` or build script sees the same schema-resolved environment your own code does. That matches what those scripts already got from Nub's `.env` loading, so the schema does not create a second, quieter set of rules for them.

One consequence is worth knowing: if your schema fails validation, the failure appears while a dependency is building rather than when you run your app.

```bash
nub install   # a dependency's build script resolves your schema too
```

If you would rather they did not, keep the schema out of the install: `nub install --ignore-scripts` runs no dependency scripts at all.
