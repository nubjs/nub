...
Files named with `--env-file` are the exception — they are never expanded. See [explicit files](#explicit-files).
...
Values from these files arrive verbatim — no `${VAR}` expansion, matching Node's `--env-file`. A password holding a literal dollar sign survives intact, so `PASSWORD=foo$bar` stays `foo$bar` and needs no escaping.
