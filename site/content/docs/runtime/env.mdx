...
The same choices live in a [nub.jsonc](/docs/config#env) at the project root, so every run in the project makes them without a flag.

```jsonc title="nub.jsonc"
{
  // ...
  "env": ["./env/base.env", "./env/local.env"]  // later files win
}
```
...
"./env/local.env"        one file, like --env-file
["./a.env", "./b.env"]   several files, in order; later files win
```

Paths resolve from the directory holding the file that supplied them, and they expand `${VAR}` and `$VAR` against the environment, because a config file is never shell-expanded. A path of `"./env/.env.${APP_ENV}"` reads `./env/.env.staging` under `APP_ENV=staging`.
