...
Nub has first-party support for [Varlock](/docs/runtime/varlock). When a project carries a `.env.schema` and Varlock is installed, Nub's own environment loading switches off entirely and Varlock owns the environment — nothing on this page applies.
</Callout>
