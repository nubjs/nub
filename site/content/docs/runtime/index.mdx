...
```jsonc title="nub.jsonc"
{
  ...
  "define": { "APP_VERSION": "\"1.4.0\"" }        // compile-time replacement
}
```

A preload entry is either a path, resolved from the directory holding the file that supplied it, or a bare package specifier resolved the way an import in your project would be. Each `define` value is JavaScript source, so a string constant carries its own quotes. V8 flags travel through `NODE_OPTIONS`, so only the flags Node accepts there work — Nub rejects the rest before Node starts.
