...
A dash in the fast-tier column means that line has no fast-tier build — every patch runs in the compatibility tier. A ✓ marks the fast-tier versions, recommended for the lowest startup overhead — the latest (Node 26) is the default.
...
- Importing a file as text — `import readme from "./README.md" with { type: "text" }` — needs Node 18.20, the first version whose parser accepts the `with` import-attribute syntax. Node added native text imports at 26.5 and backported them to 24.19, keeping them behind `--experimental-import-text`; Nub injects that flag and steps aside on every version that has it, and polyfills the feature everywhere else.
...
Nub still resolves and provisions the project's pinned Node. Nothing else on this page applies — no load hook, no preload, no `.env` loading, and none of the other `nub.jsonc` runtime fields.
