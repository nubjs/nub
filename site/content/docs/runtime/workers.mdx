...
```ts
const url = import.meta.resolve("./worker.ts");
...
```ts
// resolves relative to this module; a .ts entry is fine
...
```ts
// traced by bundlers; also correct at runtime
...
```ts
// cwd-relative — avoid unless the launch dir is known
...
```ts
worker.postMessage({ n: 41 });
// MessageEvent — payload on .data
worker.onmessage = (ev) => console.log(ev.data);
// the same event, in EventTarget form
