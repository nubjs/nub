...
<Callout title="Delivery">
Version-gated V8 feature available on Node 26.4+. Nub turns on V8's `--js-defer-import-eval` flag inside the process the first time it loads a module that uses the syntax, so a program that never uses it runs with V8's default flags. A subprocess launched as `node` gets the same treatment through Nub's shim; one launched by absolute executable path does not.
</Callout>
