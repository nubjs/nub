...
<Callout title="Not in the Homebrew core build">The formula in Homebrew's core tap builds Nub without this command, so that binary answers with `nub: "compile" is not a nub command`. The install script, npm, and `nubjs/tap` all ship it.</Callout>
