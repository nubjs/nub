...
Building for a platform other than the host also needs that platform's launcher, and an install ships only its own. Nub fetches the one it needs from the release it came from, checks it against the published checksum, and caches it — so the first cross-compile to a given platform downloads a few hundred kilobytes and later ones do not. Setting `NUB_LAUNCHER_TEMPLATE` to a launcher on disk skips that entirely, which is the way to cross-compile with no network at all.
