...
The two shapes treat the version differently. The default shape bakes one **exact** version into the binary — a range resolves to the newest match at compile time. With `--smol`, an exact version such as `26.0.0` requires that exact Node at runtime, and an explicit semver range is enforced in full. A major/minor pin or alias resolves to a **floor** and accepts any Node at or above it. When no installed Node qualifies, Nub provisions the newest matching release resolved when the executable was compiled.
