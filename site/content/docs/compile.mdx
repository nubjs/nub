...
By default, the entry's dependencies are bundled or embedded in the executable and need no separate installation on the target. TypeScript, JSX, and non-erasable syntax are transpiled at compile time. Builds using `--external` or `--allow-dynamic-import` can depend on files available on the target machine.
...
| | Default | `--smol` |
|---|---|---|
| Runtime in the executable | Embedded Node | Launcher and application only |
| Node at runtime | Runs directly as a Node SEA, or is extracted by the launcher | Found on the machine, else fetched from nodejs.org |
| Offline machine | No Node download needed | Requires a usable installed or cached Node |

```bash
nub compile app.ts           # self-contained for this target: embeds Node
nub compile app.ts --smol    # tiny: finds or provisions Node at first run
```
...
### Single-executable applications and launchers

Nub automatically chooses a **Node single-executable application (SEA)** when the target Node version and bundled program support it. Node and the application run from the executable without extraction, including statically bundled workers.

| Shape | Runtime and application delivery |
| --- | --- |
| Node SEA | Node and bundled JavaScript run from the executable |
| Launcher with embedded Node | Compressed Node and app files are extracted to a cache |
| `--smol` launcher | Node is found or provisioned; eligible bundles run from memory, other app files are extracted |

- Programs needing real files or a separate Node process use the launcher: native packages, included files, external dependencies, linked source maps, and child-process or cluster use. The build reports which shape it chose and why extraction is needed.
- A launcher compresses its embedded Node; a SEA carries Node directly. Executable size depends on the shape, Node version and application. Matching launcher-built binaries share one extracted Node. See [cache locations](#where-the-runtime-is-extracted).

### Build-time bytecode

Large extracted programs built for the host platform on Node 24 or later also carry precompiled ESM bytecode. Compilation does not run application code.

- The bytecode increases executable and cache sizes to reduce repeated compilation.
- Node validates the bytecode before use and falls back to source when it cannot use the cache.
...
```bash
nub compile app.ts --smol                  # finds or fetches Node
nub compile app.ts --smol --target ">=22 <23" # a host Node in the range
nub compile app.ts --smol --target 26.0.0     # exactly Node 26.0.0
```
...
nub compile app.ts --target latest    # latest major ({{NODE_MAJOR}}), not LTS
```
...
```bash
nub compile app.ts --target ">=22"            # bakes the newest >=22 now
nub compile app.ts --smol --target ">=22"     # smol: any host Node >= 22
nub compile app.ts --smol --target ">=22 <23" # smol: rejects 23 and newer
nub compile app.ts --smol --target 26.0.0     # smol: exactly Node 26.0.0
```
...
```bash
nub compile app.ts --sourcemap=linked    # a .map inside the binary
nub compile app.ts --sourcemap=inline    # the map inside the bundle
nub compile app.ts --sourcemap=external  # a .map beside it, not shipped
```
...
```bash
nub compile app.ts --define 'API_URL="https://api.example.com"' \
  --define BUILD=42
```
...
const icons = createReadStream(join(here, "..", "assets", "icons.xml"));
```
...
\e[31m\e[1merror\e[22m\e[39m  --include "data/missing.json" matched no files
       (the path does not exist)
```

Embedded files retain their original bytes when extracted. They are compressed inside the executable, so its size increase depends on how well those files compress.
...
'/…/compile-app/64119dd9/table.txt'
```
...
```bash
nub compile app.ts --include=data --exclude=data/fixtures \
  --exclude='**/*.tmp'
```
...
new URL("./cache.db", import.meta.url)       // no such file — left alone
```
...
join(import.meta.dirname, "..", "assets", "icons.xml")       // from entry
join(import.meta.dirname, "..", "..", "assets", "icons.xml") // from helper
```
...
```console
$ nub compile src/index.ts                   # entry at the root, assets
                                             # beside it
$ nub compile src/index.ts --include assets  # `assets` is a sibling of
                                             # `src`, so the root holds
                                             # both, and the entry sits
                                             # in `src/` inside it
```
...
If the dependency stays bundled, reading that path gives `ENOENT`. Nub detects many packages that read adjacent files and ships them unbundled, preserving their installed layout inside the executable. If detection misses one, use [`--unbundled`](#--unbundled); unlike `--external`, it keeps the package in the binary. The `--include` flag does not move a dependency's file beside the bundled entry.
...
const Spawn = globalThis.Worker ?? NodeWorker; // every arm is a Worker
```
...
```ts
new Worker("./worker.js");                    // resolved against the cwd
// a child process, not an import — never bundled
fork(new URL("./child.js", import.meta.url));
```
...
Shipping 1 package unbundled, in its own installed layout:
  @napi-rs/nice — 16 per-platform binary packages (napi-rs layout)
...
error  the bundler failed:
  app.ts:1:19
    Could not load node_modules/local-addon/addon.node (imported by
    app.ts) - plugin `nub:native-addons` threw an error.

this native addon is built for macOS, but the binary targets linux-x64:
node_modules/local-addon/addon.node
  A native addon is machine code for one platform, and a compiled binary
  loads it from a real file at run time — there is no later step that
  could translate it. Install this dependency for the target (its
  platform package) and compile again, or drop --platform to build for
  this machine.
```
...
Static addon imports can be bundled directly. Computed paths need the package's installed layout:

```js
require(`pkg-${process.platform}`)             // statically followed

const name = `pkg-${process.platform}`;
require(name);                                 // computed lookup

require("bindings")("better_sqlite3.node")  // searches the disk at run
```

Nub detects native packages through manifest signals such as a dependency on `bindings` or `node-gyp-build`. It ships those packages unbundled inside the executable, so their runtime lookups still work. For example, `better-sqlite3` is detected automatically and needs no `--external` flag.

Detection cannot identify every computed dependency. Use [`--unbundled`](#--unbundled) when a package needs its installed layout preserved. Use [`--external`](#--external) only when the package should stay outside the executable and be installed on the target machine. An arbitrary computed lookup in application code still needs its target included explicitly.
...
A cross-build that selects the launcher needs the target platform's launcher template; an install ships only its own. Nub fetches the launcher for its exact version, checks it against the published checksum, and caches it. An offline launcher cross-build needs that template already cached. SEA builds do not use a launcher template.
...
error  1 import could not be resolved at build time:
  ...
  A compiled binary carries no node_modules, so an unresolved import
  fails at runtime on the machine you ship to. Make the specifier a
  static string so the bundler can follow it.
  A specifier your program computes at run time — a plugin path, a
  config module — cannot be made static. Pass --allow-dynamic-import to
  keep it, and the binary will resolve it from the directory it is
  started in. What it loads then depends on the machine you ship to.
```
...
error  1 import could not be resolved at build time:
  ...
  fails at runtime on the machine you ship to. Make the specifier a
  static string so the bundler can follow it.
  A require() whose `require` is a local binding (a UMD factory
  parameter) is an ordinary call the bundler cannot rewrite. Depend on
  the package's ESM build, or alias the specifier to it with --alias.
```
...
error  the bundler emitted 1 chunk that is not valid JavaScript:
  ...
  The compiled binary would throw a SyntaxError on startup. This
  usually means a source language survived the bundle untransformed —
  most often JSX, from a tsconfig whose compilerOptions.jsx is
  "preserve".
```
...
```bash
nub compile app.ts --platform win32-x64 \
  --metadata "CompanyName=Acme Inc." \
  --metadata "LegalCopyright=Copyright 2026 Acme"
```
...
SEA executables do not extract Node or the bundled application. A launcher with embedded Node extracts it under `compile-node/<version>-<hash>`, keyed by the runtime's content hash. Matching compiled binaries share that extraction; the launcher can also adopt a matching Node from Nub's ordinary store.

Cache locations are tried in order:

- The `nub` subdirectory of `XDG_CACHE_HOME`, when set.
- The user cache: `$HOME/.cache/nub` on Unix; the Windows user profile's `.cache\nub` for a normal profile, or `%LOCALAPPDATA%\nub` for a missing or system profile.
- A private directory under the platform's temporary directory: `nub-<uid>` on Unix, or `nub` on Windows. The temp root is typically `$TMPDIR` or `/tmp` on Unix and `%LOCALAPPDATA%\Temp` on Windows.

On Windows, the user-cache choice also honors `XDG_CACHE_HOME`. If that override is unusable, the next distinct location is the temporary directory.
...
The app's extracted files are validated before publication, not scanned on every launch. Later runs check the private cache directory, completion marker, entry file, and bootstrap. If other extracted files are modified or deleted afterward, remove that app's extraction directory to restore them on the next run.
...
RUN ./app --version || true   # the exit code is irrelevant; the
                              # extraction is the point
```

Extracted app files live under `compile-app/<hash>`, separately from Node. Identical payloads reuse the same directory; changed payloads get a new one. The launcher performs best-effort cleanup of other app and Node extractions older than 30 days. Delete the app cache to reclaim space immediately; affected executables extract again on their next run:
...
```bash
xattr -d com.apple.quarantine ./app   # or right-click the app → Open
```
...
The embedded or provisioned runtime is official, unpatched Node. Both SEA and launcher executables install the compiled bootstrap, version-appropriate flags, and bundled preamble. The inherited `NODE_OPTIONS` value is passed to Node unchanged, so supported Node flags keep their normal behavior.
...
process.argv[1]   // extracted entry, or outer executable for SEA/inline
