...
The application's own files land beside it under `compile-app/<hash>`, keyed by the payload's content. That key changes with every rebuild, so a machine that compiles the same program repeatedly accumulates one extraction per build — for a large program embedding a Node, hundreds of megabytes each. Nothing evicts them. Delete the directory to reclaim the space; the next run extracts again:
