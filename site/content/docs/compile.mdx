...
A compiled binary runs bundled, minified code, so an unmapped stack trace points at a column in one long line. Nub ships a source map alongside the bundle by default and the embedded Node consumes it, so traces name your original TypeScript:
...
`inline` embeds the map in the bundle, so the runtime reads and parses it on every launch. The default keeps the map in its own file, which Node reads only when it renders a stack trace — same traces, faster start.
