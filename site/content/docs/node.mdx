...
Like [`nub pm shim`](/docs/pm/pm-shim), it is opt-in and reversible. It installs a `node` link in `~/.nub/node-shim` and adds that directory to your PATH; because that comes first, a Node you install later (`brew install node`) is shadowed until you unshim. Re-run `nub node shim` after `nub upgrade` to re-link. On Unix, a fresh install with `XDG_DATA_HOME` set uses `$XDG_DATA_HOME/nub/node-shim` instead; an existing `~/.nub/node-shim` always wins.
