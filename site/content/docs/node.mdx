...
With no pin the explainer reads `» resolved from node on PATH`. An alias pin (`lts/*`, `lts/<codename>`, `node`) resolves against the release index that the last `nub node install` fetched, so `which` and every run work offline once the version is in the cache. Bare `nub node` prints a status block instead — the version, path, and resolution source on separate lines.
...
node shim in /Users/you/.local/share/nub/node-shim (created)
  `node` now resolves through nub (version management only — no
  augmentation; run `nub` for that)
