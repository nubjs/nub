...
2. **`package.json#/devEngines/runtime`** — the `node` entry
3. **`.node-version`**
4. **`.nvmrc`**
5. **`.tool-versions`** — the asdf/mise file; its `nodejs` or `node` line
6. **`package.json#/engines/node`** — a range or exact version
