...
Whichever manager the project ends up on, [Nub's own installer](/docs/install) mirrors that manager on version resolution and module resolution and respects its lockfile. It does not take `node_modules` layout from that manager's config — layout is Nub's own axis, set in [`nub.jsonc`](/docs/config#installlinker) or the neutral `.npmrc` keys.
