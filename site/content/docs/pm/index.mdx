...
Whichever manager the project ends up on, [Nub's own installer](/docs/install) mirrors that manager's supported resolution settings and respects its lockfile. Layout is Nub's own axis: the [`nub.jsonc`](/docs/config#installlinker) layout fields apply in any project, while its resolution fields wait until Nub is the manager.
