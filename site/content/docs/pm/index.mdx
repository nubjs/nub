...
Whichever manager the project ends up on, [Nub's own installer](/docs/install) mirrors that manager's supported resolution and layout settings and respects its lockfile. A project pinned to Nub reads the install block in [`nub.jsonc`](/docs/config#installlinker); a project pinned to npm, pnpm, Yarn, or Bun keeps that manager's own configuration.

Every file written or removed is named in the output; rerunning is a no-op. Four cases stop the command before anything is written:
