...
Whichever manager the project ends up on, [Nub's own installer](/docs/install) mirrors that manager's supported resolution and layout settings and respects its lockfile. A Nub-identity project reads the [`nub.jsonc`](/docs/config#installlinker) install block; a compat project keeps its incumbent configuration.
