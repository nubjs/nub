...
Whichever manager the project ends up on, [Nub's own installer](/docs/install) mirrors that manager on version resolution and module resolution and respects its lockfile. How packages get linked into `node_modules` is a separate setting, taken only from [`nub.jsonc`](/docs/config#installlinker) or the neutral `.npmrc` keys, never from that manager's config.
...
Only the install verbs hand off. Everything else — the file runner, script running, [`nubx`](/docs/runner), version management, self-update — never reads the pin, so `nub script.ts` stays fast. A non-exact pin runs in place with a notice: like corepack, the `packageManager` field is exact-only.
