...
A routed install runs every lifecycle script, as npm does, under the `node` on PATH (the project's Node pin is not consulted and nothing is provisioned, as under npm), and hands them `NODE_ENV=production` exactly when dev dependencies are omitted. An `ignore-scripts=true` in the project or user `.npmrc`, or `npm_config_ignore_scripts` in the environment, is honored, and `--ignore-scripts=false` on the command line outranks it. The default trust list still applies to `nub install` typed directly.
