...
Nub refuses a key it would never read back rather than writing a line nothing consults, and names the surface that works instead. Three kinds are refused: a setting only the underlying engine's own CLI acts on, a setting `.npmrc` has no spelling for, and a key under one of this file's sections (`install.`, `dlx.`, `runtime.`) that names no field, such as `dlx.implicit`. That last one is a typo for a field of this file; written to `.npmrc` it would only make npm warn about an unknown key on every run.
