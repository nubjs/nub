...
- **Global `nub.jsonc`** — the same field, for personal defaults everywhere. This file is treated leniently, because it applies to every project on your machine: a bad value is reported and that setting skipped, and an unrecognized top-level section is ignored outright — so one typo, or a key left behind by a newer Nub, never takes the rest of your defaults down with it. The strict behavior above applies to the project file, which is checked in and shared. Read from the first of:
...
The install block configures Nub's package manager. Its fields fall into two groups, scoped differently.
