...
Hand the environment to Varlock explicitly with `"varlock"`. A project with a `.env.schema` and no `envFile` already does this, so the value is there to state it in the file rather than leave it to be inferred from a missing field.
...
A global `envFile` never takes precedence over a project's `.env.schema` — only a project file, an environment value, or a flag does. A machine-wide default set with `nub config set --global envFile false` would otherwise empty the environment of every schema project on the machine.
