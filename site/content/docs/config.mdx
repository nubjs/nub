...
```jsonc title="nub.jsonc" full
{
  ...
  // ── installs — Nub projects only; an error in a pnpm project ──
  "install": {
    ...
    "settings": { "savePrefix": "~" }      // pnpm-workspace.yaml key names
  }
}
```
...
The install block takes four fields: `linker` and `publicHoist` for `node_modules` layout, and `minimumReleaseAge` with `minimumReleaseAgeExclude` for the version cooling window. Any other install setting goes in [`settings`](#installsettings).

The block applies when Nub is the project's package manager. A project that already belongs to pnpm takes its install settings from pnpm's own configuration, so an `install` block there is a contradiction rather than a setting to skip, and Nub refuses the command instead of picking one of the two:

```text wide
Error: /path/to/nub.jsonc sets an `install` block, but this project already belongs to pnpm.
A pnpm project takes its install settings from pnpm's own configuration, so the block would be ignored. Move those settings into pnpm-workspace.yaml, or remove pnpm's files to make this a nub project.
```

The refusal covers the whole block, layout fields included.
...
Set any other install setting by the name `pnpm-workspace.yaml` gives it. Keys and values are spelled exactly as pnpm spells them.
...
A key that `pnpm-workspace.yaml` does not accept is an error:

```text wide
Error: install.settings.savePrefx is not a setting pnpm-workspace.yaml accepts
```

The four fields above also have a pnpm spelling, such as `nodeLinker` for `linker`. Setting one both ways is an error:

```text wide
Error: `install.settings.nodeLinker` in /app/nub.jsonc: sets the same thing as
`install.linker`; keep one of the two
```

A few pnpm settings are refused, because a Nub project reads that setting from somewhere else or does not read it at all. The error names the replacement:

```text
# pnpm's own files and directories
pnpmfile
ignorePnpmfile
...
# the Node version, which Nub picks
nodeVersion
executionEnv
useNodeVersion

# build lists; use allowScripts in package.json
onlyBuiltDependencies
onlyBuiltDependenciesFile
neverBuiltDependencies
ignoredBuiltDependencies

# workspace members; use workspaces in package.json
packages
```

```text
Error: `install.settings.onlyBuiltDependencies` in /app/nub.jsonc: is a
deprecated build list; approve dependency scripts with `allowScripts` in
package.json
```
...
A key that is not a field of this file reads and writes `.npmrc`, so the value stays visible to npm, Yarn, and pnpm.

In a pnpm project, that key goes to pnpm's own `config` command instead. It reads and writes pnpm's files, takes pnpm's `--location` flag, and prints what `pnpm config` prints. The fields of this file, `config init`, and `config path` behave the same in both kinds of project.
