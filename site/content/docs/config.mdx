...
Put `nub.jsonc` at the project root. Nub finds the nearest file by walking up from the command's working directory. The file uses JSON with comments and trailing commas, and an unknown key or invalid value stops the command with an error.
...
- **Global `nub.jsonc`** — the same field, for personal defaults everywhere. Read from the first of:
