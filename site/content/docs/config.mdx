...
Replace identifiers at compile time. Each value is a JavaScript source string, so a string literal needs its own quotes inside the JSON one.
...
Materialize selected packages inside the project instead of linking them from the shared store. Reach for it when a package resolves paths relative to its own real location and a symlink puts that somewhere it does not expect.

```jsonc title="nub.jsonc"
{
  // ...
  "install": {
    "symlinkDisablePattern": ["electron", "@company/native-*"]
  }
}
```
