...
Every field is addressable by its dotted path. Writes go to the project file, and `--global` sends them to your global one instead. The `nub global config` prefix is equivalent.
...
The install block splits across the two axes. Layout is Nub's own, so those fields apply whoever owns the project; resolution follows the incumbent, so those apply once Nub is the project's manager.

| what it controls | fields | applies |
| --- | --- | --- |
| layout | `linker`, `publicHoist` | every project |
| resolution | `minimumReleaseAge`, `minimumReleaseAgeExclude` | Nub identity |

Under an npm, pnpm, Yarn, or Bun identity, Nub names each resolution field it skipped.
...
Create a commented project file with every supported project field. Only the schema URL is active, so initialization changes no runtime or install behavior.
...
The command refuses to replace an existing file. Read and write individual fields with the remaining config commands, which validate values and preserve the file's comments.
...
Writes go to the project file, and `--global` sends them to the global one instead. The equivalent `nub global config` prefix is useful when several operations should clearly read as user-level configuration. Fields under [`dlx`](#dlx) always go there, with or without the selector.
