...
The install block takes four fields: `linker` and `publicHoist` for `node_modules` layout, and `minimumReleaseAge` with `minimumReleaseAgeExclude` for the version cooling window.
