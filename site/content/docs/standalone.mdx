...
## From the nub installer

The `nub` binary answers to the name `nubr` too. Installing it puts the command on your `PATH` with no npm step, and with no Node either: the binary provisions one when none is installed.
...
Both installs run the same runner, so a file behaves the same under either.
...
```sh
node --import @nubjs/runner app.ts      # ESM hooks + CJS require()
node --require @nubjs/runner app.ts     # same, as a CJS preload (20.19+)
