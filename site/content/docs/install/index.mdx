...
Refusing a version is not the same as failing the install. Nub keeps walking down the versions the range admits and takes the newest one that still carries its evidence, naming what it skipped and why:
...
The substitution never reaches outside the declared range, and never climbs above the refused version. When nothing the range admits clears the check, the install stops with `ERR_NUB_TRUST_DOWNGRADE`.
