...
The tables above cover resolution and the supported layout settings for each incumbent. A Nub-identity project reads its [`nub.jsonc`](/docs/config#installlinker) install block. A compat project keeps its incumbent settings instead; the neutral `node-linker` key in `.npmrc` remains available where that package-manager version reads it.
...
A fresh `nub install` records Nub as the manager with a non-locking range — `devEngines.packageManager` set to `{ name: "nub", version: "^<version>", onFail: "ignore" }`, never an exact `packageManager` pin. Tools that read `devEngines` by name see the signal, and the caret is a floor, so upgrading Nub just works. The entry is a signal to be read, not a rule to enforce: `onFail: "ignore"` keeps npm from printing a mismatch warning on every command it runs in the project, including ones like `npx` that install no dependencies at all. To freeze the project at an exact Nub version — the corepack-visible hard pin — opt in with `nub pm use nub@<version>`; the bare `nub pm use nub` writes only the range.
...
Layout settings live in the same block; see [the virtual store](/docs/install/virtual-store#project-configuration).
...
The duration takes a unit — `s`, `m`, `h`, `d`, or `w` — and a bare number means minutes, matching pnpm. Sub-minute values round up, so a short window can never round down to zero and switch the gate off by accident. Both flags work on every command that resolves from the registry, including the remote bin runner.

The exclude flag *replaces* the configured `minimumReleaseAgeExclude` for that run rather than adding to it, matching pnpm. In a project that already exempts `@internal/*`, passing one more package on the command line drops that exemption for the run, so name every package you still need:
...
When nothing satisfying the range is old enough, the install fails — Nub does not quietly take a version that missed the cutoff, and no flag relaxes that. Setting the window to `0` turns it off outright, which is the honest way to say you don't want one. An organization-managed config sets a floor no flag can lower. This is where Nub departs from pnpm, whose `minimumReleaseAgeStrict` is off unless you set it, leaving its default window advisory.
</Callout>
