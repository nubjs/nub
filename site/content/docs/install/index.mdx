...
A project [`nub.jsonc`](/docs/config) carries an [`install`](/docs/config#install) block. It applies only when Nub is the project's package manager — a project with an npm, pnpm, Yarn, or Bun lockfile keeps that tool's own configuration.
