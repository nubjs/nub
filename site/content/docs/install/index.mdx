...
description: Nub's install engine — the neutral package.json fields and config keys that shape dependency resolution, plus lockfile compatibility with npm, pnpm, Bun, and Yarn.
---
...
The lockfile is `nub.lock`. Every field Nub reads to build the graph is one npm, pnpm, Yarn, or Bun already reads — Nub adds no config field of its own, so there is no new syntax here to learn. Running Nub inside a repo that already belongs to one of those tools works too: see [lockfile compatibility](#lockfile-compatibility).

## Resolution

Resolution decides which version of which package ends up in the graph. These are the `package.json` fields that shape it:

| Field | What it does |
|---|---|
| `overrides` | Force one version of a package everywhere in the graph, whoever asked for it |
| `resolutions` | The same, in Yarn's spelling — both are read, and `overrides` wins a conflict |
| `packageExtensions` | Fix dependency metadata a published package got wrong, at resolve time |
| `patchedDependencies` | Apply a patch file to a package's contents, keyed by name and version. Written for you by `nub patch` |
| `allowUnusedPatches`, `allowNonAppliedPatches` | Downgrade a patch that matched nothing from an error to a warning |
| `workspaces` | Monorepo member globs. Members resolve to each other through `workspace:` specifiers |
| `workspaces.catalog`, `workspaces.catalogs` | Named version sets that `catalog:` specifiers resolve against |
| `allowBuilds` | Which dependencies may run install scripts — see [lifecycle scripts](#lifecycle-scripts) |
| `dependenciesMeta` | Per-dependency flags: `injected` hard-copies a workspace dependency instead of linking it, `built: false` denies its scripts |
| `peerDependenciesMeta` | Mark one of your own peer dependencies optional, so a missing peer is not an error |
| `allowedDeprecatedVersions` | Which deprecated versions may resolve without a warning |
| `engines` | The Node and package-manager versions the project requires |

### Overrides

An override forces a version regardless of what any package in the graph asked for. The selector is a package name on its own, a name with a range to match, or a path that scopes the pin to one parent:

```json
// package.json
{
  "overrides": {
    "is-number": "7.0.0",              // everywhere in the graph
    "is-number@^6.0.0": "7.0.0",       // only where the request matches ^6.0.0
    "is-odd>is-number": "7.0.0",       // only under is-odd
    "semver": "$semver"                // match this project's own dependency range
  }
}
```

Yarn's `resolutions` field takes the same selectors and is read the same way. Declaring a pin in both is fine and common in a repo that several tools install — Nub applies `overrides` on a conflict and stays quiet when the two agree.

### Package extensions

A published package sometimes declares a peer dependency it doesn't actually need, or is missing one it does — and its `package.json` isn't yours to edit. A package extension patches a dependency's manifest at resolve time, adding to its `dependencies`, `optionalDependencies`, `peerDependencies`, or `peerDependenciesMeta`. The selector is a package name, optionally pinned to a version range:
...
An extension only *adds* what's missing — it never overrides a range the package already declares. To change the version of a dependency a package does declare, use an override instead. The semantics match pnpm's `packageExtensions`.

### Catalogs

A catalog names a version once and lets every package in a monorepo reference it, so a bump is a one-line edit. Definitions live beside the workspace globs, and members refer to them with a `catalog:` specifier:
...
// package.json — the workspace root
{
  "workspaces": {
    "packages": ["packages/*"],
    "catalog": { "react": "19.2.0" },              // the default catalog
    "catalogs": { "testing": { "vitest": "3.2.4" } }  // named catalogs
  }
}
```
...
{
  "dependencies": { "react": "catalog:" },       // the default catalog
  "devDependencies": { "vitest": "catalog:testing" }
}
```

A `catalog:` reference that names no defined entry is an error rather than a fallback, so a typo fails the install instead of silently resolving to something else. Adding a dependency straight into the catalog is `nub add --save-catalog <pkg>`.
...
```ini
# .npmrc
resolutionMode=highest              # or lowest-direct
autoInstallPeers=true
strictPeerDependencies=false
dedupePeerDependents=true
resolvePeersFromWorkspaceRoot=true
linkWorkspacePackages=true          # prefer a local member over the registry
saveWorkspaceProtocol=rolling
savePrefix=^
saveExact=false
supportedArchitectures.os=linux     # which optional-dependency variants to install
engineStrict=false
```

Four further install fields live in `nub.jsonc`, Nub's own config file. Two set the [cooling window](#cooling-window) — `minimumReleaseAge` and `minimumReleaseAgeExclude` — and two set [layout](#layout), `linker` and `publicHoist`. That is the entire block; see the [config reference](/docs/config#install) for each field's shape.

## Layout

Resolution decides *which* packages you get; layout decides how they are arranged on disk. Layout is Nub's own axis, so `nub.jsonc` sets it whichever package manager owns the project:
...
The full set is also readable from `.npmrc`, under every incumbent:
...
The `--node-linker` flag does the same for one command. No package manager's own config file directs the tree — see [layout settings](#layout-settings). See [the flat opt-out](/docs/install/virtual-store#the-flat-opt-out) for choosing between the layouts.
  ...
  rows={[
    ...
    },
    {
      ...
      items: [
        ...
        { code: 'pnpm-workspace.yaml', ok: true, note: 'Pnpm 11+ resolution settings; layout keys are not read from it.' },
...
Layout is the one axis Nub does not mirror. A branded layout setting in the incumbent's own config file is read for nothing; the rest of that file — registries, auth, resolution settings, overrides — is read normally.

| Package manager | Its own layout settings | Under Nub |
|---|---|---|
| **npm** | `install-strategy`, `global-style`, `legacy-bundling` | Not read |
| **pnpm** | `nodeLinker`, `hoist`, `hoistPattern`, `publicHoistPattern`, `shamefullyHoist`, `hoistWorkspacePackages`, `hoistingLimits`, `modulesDir`, `virtualStoreDir` — in `pnpm-workspace.yaml` | Not read. The same keys in `.npmrc` are Nub's own surface and do apply |
...
Mirroring an incumbent's layout would only read coherently if its *default* were mirrored too — npm and Bun default to hoisted, so honoring that would end Nub's isolated default — and honoring a written key while ignoring the identical unwritten intent is a seam you cannot predict.

The [`nub.jsonc`](/docs/config#install) layout fields apply whoever owns the project. Only the resolution fields in that block are Nub-identity-only, and Nub names those if a project sets them under another manager.
