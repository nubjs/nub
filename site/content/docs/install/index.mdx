...
The OS-level build jail — a filesystem-scoped sandbox around every build script, with egress narrowed to the handful of hosts install scripts download their own binaries from — is compiled in but **off by default**. Opt in with the neutral `paranoid` / `npm_config_paranoid` setting, which also flips the advisory gate to fail-closed. The jail covers macOS and Linux; Windows is a passthrough.
