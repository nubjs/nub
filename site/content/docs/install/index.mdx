...
A project [`nub.jsonc`](/docs/config) carries an [`install`](/docs/config#install) block. It applies only when Nub is the project's package manager — a project with an npm, pnpm, Yarn, or Bun lockfile keeps that tool's own configuration, and the block is ignored.

```jsonc title="nub.jsonc"
{
  "install": {
    "minimumReleaseAge": "3d",                   // cooling window
    "minimumReleaseAgeExclude": ["@company/*"],  // exempt from the window
    "nodeOptions": ["--max-old-space-size=8192"] // for dependency build scripts
  }
}
```

Durations are an integer plus one of `s`, `m`, `h`, `d`, or `w` — a bare number is rejected. The top-level `nodeOptions` field is a separate setting that covers runtime commands, not dependency builds. Layout settings live in the same block; see [the virtual store](/docs/install/virtual-store#project-configuration).
