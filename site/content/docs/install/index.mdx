  ...
  rows={[
    ...
    },
    {
      ...
      items: [
        ...
        { code: 'pnpm-workspace.yaml', ok: true, note: 'Workspace, resolution, and layout settings.' },
...
A pnpm project takes its layout from pnpm's own configuration, as pnpm 12 does. A Nub project never reads another package manager's layout settings:

| Package manager | Its own layout settings | In a Nub project |
|---|---|---|
...
In a Nub project, set layout with [`install.linker` and `install.publicHoist` in `nub.jsonc`](/docs/config#install), their neutral `.npmrc` spellings, or command-line flags. Nub reports npm's layout keys when it finds them in `.npmrc`, so the ignored request is not silent.
