...
Dependency build scripts take their own Node options:
...
The top-level `nodeOptions` field is separate — it covers runtime commands, not dependency builds. Layout settings live in the same block; see [the virtual store](/docs/install/virtual-store#project-configuration).
