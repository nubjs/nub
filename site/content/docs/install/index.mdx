  ...
  rows={[
    ...
    },
    {
      ...
      items: [
        ...
        { code: 'pnpm-workspace.yaml', ok: true, partial: true, note: 'Layout keys are not read — layout is configured through nub.jsonc or .npmrc.' },
    ...
    },
    {
      ...
      items: [
        ...
        { code: 'bun.lockb', ok: false, note: 'Binary lockfile rejected — convert to bun.lock text first.' },
        {
          ...
          ok: true,
          partial: true,
          note: '[install] section only, minus linker — layout is configured through nub.jsonc or .npmrc.',
        },
...
What the tables above guarantee is **resolution**: which version a range resolves to, and which file a specifier resolves to. Nub mirrors the incumbent on both, and respects the lockfile it finds. What they do not cover is **layout** — how those resolved packages get linked into `node_modules`. Nub takes that setting only from [`nub.jsonc`](/docs/config#installlinker) or the neutral `.npmrc` keys, never from another manager's config, and defaults to [the virtual store](/docs/install/virtual-store). No lockfile format records how `node_modules` was arranged, so the round-trip holds either way.

So a layout setting in another manager's config file has no effect, whatever it asks for:
...
| **pnpm** | `nodeLinker` and the hoisting keys in `pnpm-workspace.yaml` | No effect |
...
A project [`nub.jsonc`](/docs/config) carries an [`install`](/docs/config#install) block. The layout fields shape `node_modules` and apply in every project; the version-selection fields below apply only when Nub is the project's package manager, and a project with an npm, pnpm, Yarn, or Bun lockfile resolves the way that tool would.
