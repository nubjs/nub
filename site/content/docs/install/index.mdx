...
Egress is decided by package identity rather than by destination: a package that a curated catalog does not name reaches no host at all, and one that genuinely downloads its own binary is added to that catalog by pull request. Confinement is all-or-nothing for a project — there is no per-package exemption, and `install.buildJail: false` in `nub.jsonc` is the only way off. The [sandbox page](/docs/sandbox#dependency-build-scripts) carries the detail.
