...
The lockfile is `nub.lock`, and everything Nub reads to build the graph is a neutral field that any tool could implement — there is no Nub config namespace to learn. Running Nub inside a repo that already belongs to npm, pnpm, Yarn, or Bun works too: see [lockfile compatibility](#lockfile-compatibility).
...
Registries, auth, peer-dependency behavior, and save behavior come from the `.npmrc` cascade, which Nub reads under every identity, along with `npm_config_*` in the environment. Setting names are pnpm's:
...
Typed project settings live in the [`nub.jsonc`](/docs/config#install) install block instead. It takes four fields: the cooling window (`minimumReleaseAge`, `minimumReleaseAgeExclude`) covered under [lifecycle scripts](#cooling-window), and the two layout fields below.

## Layout

Resolution decides *which* packages you get; layout decides how they are arranged on disk. Layout is Nub's own axis, set on a neutral surface:
...
The `--node-linker` flag does the same for one command. In `nub.jsonc` the same axis is two typed fields:
...
| **pnpm** | the keys above, plus `hoist` and `symlink` | Read, because they already live on the neutral surface. Under pnpm 11+ Nub follows pnpm's own move to `pnpm-workspace.yaml`, since pnpm 11 stopped reading them from `.npmrc` |
...
A project owned by another manager also keeps that manager's install settings, and Nub reports any [`nub.jsonc`](/docs/config#install) install fields it therefore ignored.
