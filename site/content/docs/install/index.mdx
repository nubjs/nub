...
A lockfile names one exact version rather than a range, so there is nothing to walk. Frozen reinstalls still abort on a pin the check refuses, which is why a fresh `nub install` can succeed by substituting while `nub ci` against a lockfile written before the substitution still fails. The remedy is a fresh resolve.
