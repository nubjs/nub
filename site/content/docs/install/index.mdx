...
A dependency that compiles a native addon builds offline as well. Nub points `node-gyp` at the headers shipped with the project's Node instead of letting it download them from nodejs.org, on macOS and Linux; on Windows the download stays, because the official Node zip carries no headers.
