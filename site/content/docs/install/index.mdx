...
What the tables above guarantee is **resolution**: which version a range resolves to, and which file a specifier resolves to. Nub mirrors the incumbent on both, and respects the lockfile it finds. It does not guarantee **layout** — how the resolved packages are arranged on disk. Layout is Nub's own axis, defaulting to [the virtual store](/docs/install/virtual-store) and configured through [`nub.jsonc`](/docs/config#installlinker) or the neutral `.npmrc` keys. No lockfile format records a layout, so the round-trip holds either way.
