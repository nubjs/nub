...
<CompatTable
  ...
  rows={[
    ...
    // ── Workspaces ────────────────────────────────────────────────────
    { feature: <><code>workspaces</code> / <code>pnpm-workspace.yaml</code></>, status: 'yes', note: 'Pnpm 11+ resolution settings; layout keys are not read from it.' },
    ...
    // ── Linking / hoisting ────────────────────────────────────────────
    { feature: <code>nodeLinker</code>, status: 'partial', note: 'Layout is nub’s own axis: set it in nub.jsonc, .npmrc, or --node-linker. No PnP.' },
  ]}
/>
...
Layout is Nub's own axis, so `nodeLinker` and the hoisting keys are not read from `pnpm-workspace.yaml` under any major. Set the layout in `nub.jsonc`, in `.npmrc`, or with the CLI flag — all three work while pnpm owns the project.
