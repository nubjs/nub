...
Nub mirrors the incumbent major: pnpm 11 and later read `nodeLinker` and the hoisting keys from `pnpm-workspace.yaml`, while pnpm 10 and below read their `.npmrc` spellings. The CLI flag works under every major. A `nub.jsonc` install block is Nub-native and is ignored while pnpm owns the project.
