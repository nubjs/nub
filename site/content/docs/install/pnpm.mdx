...
<CompatTable
  ...
  rows={[
    ...
    // ── Workspaces ────────────────────────────────────────────────────
    { feature: <><code>workspaces</code> / <code>pnpm-workspace.yaml</code></>, status: 'partial', note: 'Layout keys are not read — layout is configured through nub.jsonc or .npmrc.' },
    ...
    // ── Linking / hoisting ────────────────────────────────────────────
    { feature: <code>nodeLinker</code>, status: 'partial', note: 'From .npmrc or the flag, not pnpm-workspace.yaml. No PnP.' },
  ]}
/>
...
Layout is Nub's own axis rather than something it mirrors, so it comes from `.npmrc`, the flag, or [`nub.jsonc`](/docs/config#installlinker) — a `nodeLinker` or hoisting key in `pnpm-workspace.yaml` has no effect. Everything else in that file, resolution settings included, is read as usual. Because the layout keys stay on the neutral surface, `.npmrc` keeps setting them even under a pnpm 11 project, where pnpm itself no longer reads behavior settings from `.npmrc`.
