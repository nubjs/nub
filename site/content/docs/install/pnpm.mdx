...
<CompatTable
  ...
  rows={[
    ...
    // ── Workspaces ────────────────────────────────────────────────────
    { feature: <><code>workspaces</code> / <code>pnpm-workspace.yaml</code></>, status: 'yes', note: 'Pnpm 11+ layout and resolution settings.' },
    ...
    // ── Linking / hoisting ────────────────────────────────────────────
    { feature: <code>nodeLinker</code>, status: 'partial', note: 'From pnpm-workspace.yaml on pnpm 11+, or .npmrc on earlier majors. No PnP.' },
  ]}
/>
...
Nub mirrors the incumbent major: pnpm 11 and later read `nodeLinker` and the hoisting keys from `pnpm-workspace.yaml`, pnpm 10 and below read their `.npmrc` spellings. The `--node-linker` flag works under every major.

<Callout>
Read the [full docs](https://pnpm.io/settings/node-modules#nodelinker) on pnpm.io.
</Callout>

There is no `pnp` linker. A `pnp` value in `pnpm-workspace.yaml` or `.npmrc`, and a `--node-linker pnp` flag, are refused rather than silently downgraded:
