...
A project [`nub.jsonc`](/docs/config) picks the layout with one [`linker`](/docs/config#installlinker) field — `global-virtual-store` for the shared store (the default), `isolated` for a project-local one, or `hoisted` for a flat, real-directory tree. The block applies only when Nub is the project's package manager; under an npm, pnpm, Yarn, or Bun lockfile the neutral `.npmrc` keys above are the lever.
