...
A project [`nub.jsonc`](/docs/config) picks the layout with one [`nodeLinker`](/docs/config#installnodelinker) field — `symlink` for the shared symlink layout (the default), `isolated` for a project-local store, or `hoisted` for a flat, real-directory tree. The block applies only when Nub is the project's package manager; under an npm, pnpm, Yarn, or Bun lockfile the neutral `.npmrc` keys above are the lever.

```jsonc title="nub.jsonc"
{
  // ...
  "install": {
    "nodeLinker": "isolated",          // hoist applies to this layout only
    "hoist": ["*eslint*", "@types/*"]  // the hidden fallback tree
  }
}
```

The [`hoist`](/docs/config#installhoist) field fills the hidden fallback tree that packages reach for an undeclared import, and takes `true`, `false`, or a pattern list. Nub rejects it under any other layout rather than accept a setting it would ignore. Under the default `symlink` layout, [`symlinkDisablePattern`](/docs/config#installsymlinkdisablepattern) materializes matching packages inside the project instead of linking them from the shared store:
