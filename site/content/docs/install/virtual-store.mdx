...
Nub reads `node-linker` from `.npmrc` whatever package manager owns the project, so the same key works under npm, pnpm, Yarn, Bun, and Nub-identity projects alike. Files still materialize from the global content store by reflink or hardlink, so a flat tree costs little extra disk.
...
Nub mirrors the incumbent on version resolution, module resolution, and the lockfile. Layout is not on that list, so a package manager's own layout setting has no effect under Nub — set the layout on the neutral surface instead, in any project:

| Incumbent | Its own flat-layout config | Under Nub |
|---|---|---|
| **pnpm** | `nodeLinker: hoisted` in `pnpm-workspace.yaml` | No effect — set `node-linker=hoisted` |
...
The same holds for the hoisting keys, the modules directory, and the virtual-store paths: every one of them reads from `.npmrc`, its `npm_config_*` environment alias, and [`nub.jsonc`](/docs/config#installlinker) — never from another tool's config file. Resolution config in those same files is unaffected, and no lockfile format records a layout, so the round-trip with the incumbent holds either way.

Yarn Plug'n'Play is the one exception, and it is a refusal rather than an ignored preference. Nub installs a `node_modules` tree, so a project asking for PnP gets an error before anything is written:
