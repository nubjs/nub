...
```console
$ nub install                                                    # ❌
Error: nub: this project is configured for Yarn Plug'n'Play (nodeLinker:
pnp, or Yarn Berry's default) — nub installs a node_modules tree and
doesn't support PnP yet, so the result would diverge from yarn's. Install
with yarn, or set `nodeLinker: node-modules` in .yarnrc.yml.
[ERR_NUB_PNP_UNSUPPORTED]
```
...
```
Holding 30029 entries of the extracted-tree cache unreferenced for 30 days
before removal. Install in any project that still needs them and they are
kept.
```
...
```
No projects are registered against the store; skipping the extracted-tree
cache. Run an install in each project you want kept, then prune again.
```
