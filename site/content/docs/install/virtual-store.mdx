...
A Nub project links an **isolated virtual store** by default: direct dependencies sit at the top of `node_modules`, transitive packages link into a per-machine store, and undeclared (phantom) dependencies fail instead of resolving by accident. A [pnpm project](/docs/install/pnpm) keeps pnpm's defaults.

## One symlink per package

Other package managers build `node_modules` file by file: npm copies every file into every project, and pnpm and Bun hardlink every file from a machine cache — faster, but still one syscall per file, tens of thousands for a typical app.
...
my-app/
└── node_modules/
    ├── ms     →  ~/.cache/nub/store/…/ms@2.1.3
    └── react  →  ~/.cache/nub/store/…/react@19.2.0
```
...
Detection and ejection are on for every install, with no configuration. Three toolchains whose resolvers cannot follow symlinks out of the project — Next.js, whose Turbopack canonicalizes paths and confines the module graph to a single project root; bare React Native, whose Metro config crawls by real path; and Remix 3, whose asset server serves npm packages to the browser only from inside the project's `node_modules` — automatically get a **project-local** store instead: the same isolated layout, with every link staying inside `node_modules`. See [shared vs per-project store](/docs/install#shared-vs-per-project-store).
...
A Nub project reads `node-linker` from `.npmrc`. Files still materialize from the global content store by reflink or hardlink, so a flat tree costs little extra disk.

<Callout title="Layout versus hoist patterns">
...
A pnpm project takes its layout from `pnpm-workspace.yaml`, as pnpm 12 does, and `node-linker` in `.npmrc` has no effect there. The global virtual store is off unless the project turns it on:

```yaml title="pnpm-workspace.yaml"
nodeLinker: hoisted             # a flat tree
enableGlobalVirtualStore: true  # or: link from pnpm's shared store
```

See [pnpm](/docs/install/pnpm) for the rest of a pnpm project's settings.
...
```
Pruned 4 packages (0.1 MB) from the global virtual store
Pruned 1 stale project record from the global virtual store registry
Pruned 4 files (0.0 MB) from the store
```

Nub records every project that installs against the shared store. A prune keeps the entries those projects still link to — directly, or through another entry's own dependencies — and removes the rest. A registered project whose directory is gone stops counting as a store user; the prune drops its record and says so, and whatever only that project reached goes with it. The next install in a project that needs an entry extracts it again.

`--dry-run` prints the same plan without removing anything, and `--dry-run --json` prints it as JSON.

The extracted-tree cache, the copy of each package that the linker clones into projects, is held rather than collected. The first prune to find an entry unreferenced reports it; only a prune that still finds it unreferenced 30 days later removes it. Installing in a project that needs the entry clears the hold.

```
Holding 30029 entries of the extracted-tree cache unreferenced for 30 days before removal.
...
With nothing registered at all, a prune skips that cache rather than treating an empty registry as an empty store:
...
Earlier versions of Nub kept the shared store in a different layout. A prune removes what that layout left behind once no project links into it, on the same 30-day hold.

## Project configuration

A Nub project can pick the layout with one [`linker`](/docs/config#installlinker) field — `global-virtual-store` for the shared store (the default), `isolated` for a project-local one, or `hoisted` for a flat, real-directory tree.
