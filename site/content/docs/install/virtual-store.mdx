...
Deleting a project takes longer to reclaim than you might expect. A registered project that nub cannot find might be deleted, or might be on a disk that is not mounted, and those look identical — so the prune stops rather than guess. The record is dropped after 30 days, and the entries it was holding then need their own 30 days.
