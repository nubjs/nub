...
The hoisting keys, modules directory, and virtual-store paths follow the same per-major source — see [layout](/docs/install#layout) for the full field list. A `nub.jsonc` install block applies only under Nub identity; a project owned by npm, pnpm, Yarn, or Bun keeps its incumbent's settings.
