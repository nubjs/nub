...
The hoisting keys, modules directory, and virtual-store paths follow the same per-major source — see [resolution, not layout](/docs/install#resolution-not-layout) for the full field list. A `nub.jsonc` install block applies only under Nub identity; compat projects keep their incumbent package-manager settings.
