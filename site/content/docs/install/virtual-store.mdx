...
Nub repairs known manifest omissions with bundled **package extensions** before falling back to ejection. The engine's compatibility database is supplemented under Nub and pnpm identities by a vendored union of [`@yarnpkg/extensions`](https://github.com/yarnpkg/berry/tree/master/packages/yarnpkg-extensions) and pnpm's companion entries. User extensions take precedence over bundled entries, and existing dependency declarations are never overwritten. Bundled data stays outside the lockfile's `packageExtensionsChecksum`; the `ignore-compatibility-db` setting disables both bundled sources.
...
Nub reads `node-linker` from `.npmrc` under every incumbent, including pnpm 11 and later. Files still materialize from the global content store by reflink or hardlink, so a flat tree costs little extra disk.
...
The same rule covers hoisting keys, the modules directory, and virtual-store paths — see [layout settings](/docs/install#layout-settings) for the full field list. The layout fields in `nub.jsonc` apply under every incumbent.
...
Any project can pick the layout with one [`linker`](/docs/config#installlinker) field — `global-virtual-store` for the shared store (the default), `isolated` for a project-local one, or `hoisted` for a flat, real-directory tree.
