...
The hoisting keys, modules directory, and virtual-store paths follow the same per-major source. A `nub.jsonc` install block applies only under Nub identity; compat projects keep their incumbent package-manager settings.
