...
Nub records every project that installs against the shared store. A prune keeps the entries those projects still reach — directly, or through another entry's own dependencies — and removes the rest. Delete a project directory and its entries stop being reachable, so the next prune collects them.

Registration happens during an install, so a store built before this shipped has none. Nub says so rather than treating an empty registry as an empty store:
...
<Callout>
A project that has not installed since registration shipped is invisible to the prune, and its entries can be removed. Reinstall in that project to repair it.
</Callout>
