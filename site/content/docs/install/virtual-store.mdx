...
A project [`nub.jsonc`](/docs/config) picks the layout with one [`linker`](/docs/config#installlinker) field — `global` for the shared store (the default), `isolated` for a project-local one, or `hoisted` for a flat, real-directory tree. The block applies only when Nub is the project's package manager; under an npm, pnpm, Yarn, or Bun lockfile the neutral `.npmrc` keys above are the lever.
...
Under `global` the knob is `eject`, which writes matching packages into the project as real directories instead of linking them out of the shared store.
...
[`publicHoist`](/docs/config#installpublichoist) is the separate lever, and it sits outside `linker` because it applies under every strategy — it puts packages in the project's own top-level `node_modules` for tools that resolve from the project root, like TypeScript looking for `@types/*`.
