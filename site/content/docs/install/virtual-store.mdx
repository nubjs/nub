...
A project [`nub.jsonc`](/docs/config) picks the layout with one [`linker`](/docs/config#installlinker) field — `global-virtual-store` for the shared store (the default), `isolated` for a project-local one, or `hoisted` for a flat, real-directory tree. No lockfile records how `node_modules` is arranged, so this applies in every project, including one carrying an npm, pnpm, Yarn, or Bun lockfile.
