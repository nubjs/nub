...
Yarn's `nodeLinker` does not select the layout Nub installs. Layout is Nub's own axis, so a Yarn-owned project sets it in `nub.jsonc`, in `.npmrc`, or with the CLI flag.
