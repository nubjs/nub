...
<CompatTable
  ...
  rows={[
    ...
    { feature: <code>.yarnrc.yml</code>, status: 'partial', note: 'Per-host proxies and the layout keys are not read; PnP installs are refused.' },
    ...
    { feature: <code>nodeLinker</code>, status: 'no', note: 'Set the linker in nub.jsonc or .npmrc instead; pnp is refused outright.' },
...
Top-level `resolutions` in `package.json` is honored — Nub applies it the way Yarn does. Top-level `overrides` is npm and Bun's pin field; Yarn ignores it, and so does Nub under a Yarn incumbent — put pins in `resolutions`. The `packageManager` field is what tells Nub the project is a Yarn project.
...
Nub reads `.npmrc` for registry, auth, scopes, CA, and proxy settings. In a **Yarn-incumbent** project, Nub also reads a small `.yarnrc.yml` subset that maps cleanly onto its existing registry and resolution model:
...
Layout settings are not in that subset. Yarn's `nodeLinker`, `nmHoistingLimits`, and `nmMode` have no effect under Nub, and neither does `YARN_NODE_LINKER` — see [resolution versus layout](/docs/install#resolution-versus-layout).
...
Yarn's `nodeLinker` does not select the layout Nub installs. Nub's compatibility guarantee covers version resolution, module resolution, and the lockfile — not how packages get linked into `node_modules`, which is set only in [`nub.jsonc`](/docs/config#installlinker) or the neutral `.npmrc` keys. A Yarn project gets Nub's [virtual store](/docs/install/virtual-store) by default and picks another layout the way every other project does:

```ini
# .npmrc — flat, npm-style node_modules
node-linker=hoisted
```

Plug'n'Play is the exception, and it is a refusal rather than an ignored preference: a PnP project has no `node_modules` tree for Nub to install into. Yarn Berry defaults to PnP when `nodeLinker` is absent, so both `nub install` and `nub ci` abort before mutation with `ERR_NUB_PNP_UNSUPPORTED`. Set `nodeLinker: node-modules` to install with Nub.
...
- Layout settings — `nodeLinker`, `nmHoistingLimits`, `nmMode`, and `YARN_NODE_LINKER` have no effect. Pick a layout in `nub.jsonc` or `.npmrc` instead.
