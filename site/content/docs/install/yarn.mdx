...
<CompatTable
  ...
  rows={[
    ...
    { feature: <code>nodeLinker</code>, status: 'no', note: 'Layout is Nub\'s own axis; pnp is refused outright.' },
...
Yarn's `nodeLinker` does not select the layout Nub installs. Nub's compatibility guarantee covers version resolution, module resolution, and the lockfile — not how `node_modules` is arranged, which is Nub's own axis. A Yarn project gets Nub's [virtual store](/docs/install/virtual-store) by default and picks another layout the same way every other project does, through `nub.jsonc` or `.npmrc`:
