...
Layout settings are not in that subset. Yarn's `nodeLinker`, `nmHoistingLimits`, and `nmMode` have no effect under Nub, and neither does `YARN_NODE_LINKER` — see [layout settings](/docs/install#layout-settings).
