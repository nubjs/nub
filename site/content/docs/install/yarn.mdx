...
<CompatTable
  ...
  rows={[
    ...
    { feature: <code>nodeLinker</code>, status: 'no', note: 'Set node-linker in .npmrc instead; pnp is refused outright.' },
...
Yarn's `nodeLinker` does not select the layout Nub installs. A Yarn-owned project sets `node-linker` in `.npmrc` for a flat, npm-style tree, or passes `--node-linker hoisted` for one command.
...
- Layout settings — `nodeLinker`, `nmHoistingLimits`, `nmMode`, and `YARN_NODE_LINKER` have no effect. Set `node-linker` in `.npmrc` instead.
