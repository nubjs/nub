...
<CompatTable
  ...
  rows={[
    ...
    { feature: <code>nodeLinker</code>, status: 'no', note: 'Set the neutral .npmrc key instead; pnp is refused outright.' },
...
Yarn's `nodeLinker` does not select the layout Nub installs. A Yarn-owned project uses the neutral `.npmrc` key or the CLI flag instead; the `nub.jsonc` install block applies only under Nub identity.
...
- Layout settings — `nodeLinker`, `nmHoistingLimits`, `nmMode`, and `YARN_NODE_LINKER` have no effect. Pick a layout in `.npmrc` instead.
