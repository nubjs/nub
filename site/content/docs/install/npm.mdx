...
<CompatTable
  ...
  rows={[
    ...
    // ── Lockfiles ─────────────────────────────────────────────────────
    { feature: <><code>package-lock.json</code> v1 / v2 / v3</>, status: 'yes', note: 'v1 (npm 5/6) read; rewritten as v3 on the first mutating install.' },
    { feature: <><code>npm-shrinkwrap.json</code></>, status: 'yes', note: 'Includes pre-2017 (nested, no lockfileVersion).' },
  ]}
/>
...
<Callout>
Read the [full docs](https://docs.npmjs.com/cli/v11/configuring-npm/package-lock-json) on docs.npmjs.com.
</Callout>

```jsonc title="package-lock.json"
{
  "name": "app",
  "version": "1.0.0",
  "lockfileVersion": 3,
...
<Callout>
Read the [full docs](https://docs.npmjs.com/cli/v11/configuring-npm/npmrc) on docs.npmjs.com.
</Callout>
...
When npm is the incumbent, Nub honors npm's own dependency fields:

- `overrides` — npm's version-pin field (npm 8.3+), applied during resolution and written into the lockfile.
- `workspaces` — the membership glob array, plus npm's `--workspace` / `--workspaces` flags.
- `engines` / `os` / `cpu` / `libc` — platform fields enforced during resolution.

<Callout>
Read the [full docs](https://docs.npmjs.com/cli/v11/configuring-npm/package-json#overrides) on docs.npmjs.com.
</Callout>
...
The Yarn-style top-level `resolutions` field means nothing to npm, so Nub drops it with a warning under an npm incumbent. Put pins in `overrides`.

## Install behavior

The default `node_modules` layout is **isolated** — direct dependencies at the top level, transitive packages in a virtual store, and phantom (undeclared) dependencies that fail instead of resolving by accident. npm's `install-strategy`, `global-style`, and `legacy-bundling` settings do not change it. A project that relies on undeclared transitives can set [`install.linker` in `nub.jsonc`](/docs/config#installlinker) or put one line in `.npmrc`:
...
- **No drop-in `npm` CLI grammar.** Nub does not emulate npm-specific subcommand spellings and flags; the CLI is pnpm-shaped.
