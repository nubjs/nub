...
Not read: `[install].linker`. A Bun-owned project selects the layout through the neutral `.npmrc` key or the CLI flag; the `nub.jsonc` install block applies only under Nub identity. See [resolution, not layout](/docs/install#resolution-not-layout).
