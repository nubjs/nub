...
<CompatTable
  ...
  rows={[
    ...
    // ── Install behavior ──────────────────────────────────────────────
    { feature: <><code>[install].linker</code></>, status: 'no', note: 'Layout is Nub\'s own axis — set it in nub.jsonc or .npmrc.' },
...
Not read: `[install].linker`. That key picks a `node_modules` layout, and layout is Nub's own axis rather than something it mirrors from the incumbent — see [resolution versus layout](/docs/install#resolution-versus-layout).
