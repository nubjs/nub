...
<CompatTable
  ...
  rows={[
    ...
    // ── Install behavior ──────────────────────────────────────────────
    { feature: <><code>[install].linker</code></>, status: 'no', note: 'Set node-linker in .npmrc instead.' },
...
Not read: `[install].linker`. A Bun-owned project sets `node-linker` in `.npmrc`, or passes `--node-linker` for one command. See [layout settings](/docs/install#layout-settings).
