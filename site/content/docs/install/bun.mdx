...
Not read: `[install].linker`. Layout is Nub's own axis, so a Bun-owned project sets it in `nub.jsonc`, in `.npmrc`, or with the CLI flag. See [layout settings](/docs/install#layout-settings).
