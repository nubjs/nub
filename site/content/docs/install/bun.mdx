...
<CompatTable
  ...
  rows={[
    ...
    // ── Install behavior ──────────────────────────────────────────────
    { feature: <><code>[install].linker</code></>, status: 'no', note: 'Set the linker in nub.jsonc or .npmrc instead.' },
...
Not read: `[install].linker`. That key picks how packages are linked into `node_modules`; Nub takes that setting only from `nub.jsonc` or the neutral `.npmrc` keys — see [resolution versus layout](/docs/install#resolution-versus-layout).
