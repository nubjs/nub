...
The storage globals — `localStorage` / `sessionStorage` — also work without you setting `--localstorage-file`; Nub defaults the storage file under `~/.cache/nub` so persistence just works. And `reportError`, `vm.Module` (used by Jest/Vitest), and `node:sqlite` are likewise available where Node still gates them.
