...
```jsonc title="nub.jsonc"
{
  ...
  "envFile": "./env/local.env",          // read one env file instead of the default set
  "tsconfig": "./tsconfig.runtime.json", // the TypeScript config the runtime uses
  "define": { "__DEV__": "true" },       // replace identifiers at compile time
  ...
  "dlx": {
    "consent": "never"                   // never fetch an uninstalled CLI from the registry
  }
}
```

Unknown keys and invalid values stop the command with an error, so a typo never silently does nothing. See [nub.jsonc](/docs/config) for every field.
