...
Both `pnpm add -g @nubjs/nub` and `yarn global add @nubjs/nub` work too. That installs the Rust binary plus the platform-specific N-API addons, and puts `nub`, `nubx` and `nubr` on your `PATH`. On macOS and Linux, install with Homebrew:
...
A nightly build of `main` publishes a canary for all platforms, versioned `<version>-canary.<date>.<n>`, as the rolling [canary release](https://github.com/nubjs/nub/releases/tag/canary) on GitHub:

```bash
curl -fsSL https://nubjs.com/install.sh | bash -s canary
```

An existing install switches with `nub upgrade --canary`. Canary tracks `main` as of the last nightly and passes build and smoke checks only; `nub upgrade --stable` returns to the stable channel.
...
```bash
nub install              # resolve + link node_modules, existing lockfile
...
```jsonc title="nub.jsonc"
{
  ...
  "preload": ["./instrumentation.ts"],   // loaded before the entry file
  "envFile": [".env.local"],             // these instead of the defaults
  "tsconfig": "./tsconfig.runtime.json", // the runtime's TypeScript config

  "install": {
    "minimumReleaseAge": "3d"            // refuse deps newer than 3 days
  }
}
```
