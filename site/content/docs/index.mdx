...
That is the formula in Homebrew's own core tap. Nub also publishes its own tap, bumped during the release run rather than by a separate pull request, so it reaches a new version first. Install from it with `brew install nubjs/tap/nub`. Homebrew keeps the two apart by the tap each installation records, so an existing tap installation keeps upgrading from the tap and needs no migration.
