...
Nub is in Homebrew's core tap as well, so plain `brew install nub` works. The tap is still the better default, because it installs the same release binary as every other channel. The core formula builds from source instead, and that build has no [`nub compile`](/docs/compile). A core update lands later too, because Nub's own tap is bumped during the release run while core needs its own pull request.
