...
Nub is in Homebrew's core tap as well, so plain `brew install nub` works. The tap is still the better default, because it installs the same release binary every other channel does. Homebrew's core tap builds from source instead, and that build comes out without [`nub compile`](/docs/compile) and without the ranked package-popularity data that `nub add` uses to reject a name resembling a popular package — its other supply-chain checks, including the download-count threshold, are unaffected. A core update also lands later, since Nub's own tap is bumped during the release run while core needs its own pull request.
