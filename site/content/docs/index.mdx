...
Unknown keys and invalid values stop the command with an error, so a typo never silently does nothing. See [nub.jsonc](/docs/config) for every field, and for the block that lives in the global file instead.
