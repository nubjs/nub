...
Nub is in Homebrew's core tap as well, so plain `brew install nub` works. Two things make the tap the better default: the core formula builds without [`nub compile`](/docs/compile), and it reaches a new version later, because Nub's own tap is bumped during the release run while a core update needs its own pull request.
