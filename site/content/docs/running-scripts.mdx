...
`nub run <script>` is the only way to invoke a `package.json` script — barewords like `nub dev` do not fall through to scripts. (If you type `nub build` and `build` is a real script, Nub's error tells you to type `nub run build`.) See [Running files](/docs/running-files) for how `nub <file>` works.

## Forward arguments to a script without typing `--`
...
## Run pre and post lifecycle hooks
...
## Read npm_* env vars and node_modules/.bin from inside a script
...
The injected set includes `npm_lifecycle_event`, `npm_lifecycle_script`, `npm_execpath`, `npm_node_execpath`, `npm_command`, and the full `npm_config_*` and `npm_package_*` enumerations. `INIT_CWD` is set to the directory you invoked `nub run` from. Inside a script, spawning `node` (by shebang or `child_process.spawn("node", …)`) re-enters Nub through the PATH shim, so child processes stay transpiled and augmented — see [Subprocesses inherit augmentation](#run-a-script-without-runtime-augmentation) below.

## Honor `.npmrc`'s script-shell
...
## Target specific packages with pnpm-style filters

`--filter` (or `-F`) takes pnpm's filter grammar verbatim. Select by exact name, scope glob, name wildcard, or path glob.
...
## Select a package together with its dependencies or dependents
...
## Run only packages changed since a git ref
...
## Control how workspace runs parallelize
...
`--parallel` is the right choice for long-running dev servers where there is nothing to order; `--sequential` is for scripts that must not overlap.

## Stop on first failure, or push through
...
## Resume a workspace run after a failure
...
## Run a script at the workspace root

`-w` (pnpm's `--workspace-root`) targets only the root project, regardless of where you invoke it from. To run recursively across members and also include the root package, use the npm-style `--include-workspace-root`.
...
`--silent` (or `-s`) suppresses Nub's `$ <command>` preamble (it does not suppress the script's own stdout):

```sh
nub run --silent build
```

## Run a script without runtime augmentation
