...
```jsonc title="nub.jsonc"
{
  "dlx": {
    ...
    "env": "./env/tools.env"  // environment files for fetched tools
  }
}
```

Setting `consent` to `never` turns off [`nubx`](/docs/runner)'s implicit fallthrough fetch; an explicit `nub dlx` still runs, because typing it is the consent. The `env` key takes the same `true`, `false`, path, and path-array forms as the top-level `env` field.
