...
A project [`nub.jsonc`](/docs/config) carries a [`dlx`](/docs/config#dlx) block:
...
Fetched tools can also read their own environment files:
...
The `envFile` key takes the same `true`, `false`, path, and path-array forms as the top-level field.
