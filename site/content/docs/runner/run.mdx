...
Nub does not install on your behalf here, since it won't reshape a tree another package manager set up. Run `nub install` when you see the warning. pnpm's `install` and `prompt` values are read from `.npmrc` and treated as `warn`. The check also applies to `nub <file>`, `nub exec`, and `nubx`, and is skipped in [`--node` / compat mode](#--node) and inside an already-running script (so a script that spawns `node` doesn't re-check).
