...
A project [`nub.jsonc`](/docs/config) sets the same policy under [`verifyDepsBeforeRun`](/docs/config#verifydepsbeforerun), where skipping the check is spelled `false`:

```jsonc title="nub.jsonc"
{
  // ...
  "verifyDepsBeforeRun": "error"  // stop rather than run against a stale tree
}
```
...
Nub does not install on your behalf here — `install` and `prompt` are accepted as values but behave like `warn`, since Nub won't reshape a tree another package manager set up. Run `nub install` when you see the warning. The check also applies to `nub <file>`, `nub exec`, and `nubx`, and is skipped in [`--node` / compat mode](#--node) and inside an already-running script (so a script that spawns `node` doesn't re-check).
...
To pin a specific shell instead, set `script-shell` in `.npmrc` and Nub invokes script bodies with it on every platform. The rest of `.npmrc` (registry, auth, `node-linker`, `hoist-pattern`) is package-manager territory and is not read here.
