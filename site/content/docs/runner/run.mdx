...
Nub does not install on your behalf here, since it won't reshape a tree another package manager set up. Run `nub install` when you see the warning. pnpm's `install` and `prompt` values are read from `.npmrc` and treated as `warn`; in `nub.jsonc` they are rejected, because accepting them would name a behavior Nub does not implement. The check also applies to `nub <file>`, `nub exec`, and `nubx`, and is skipped in [`--node` / compat mode](#--node) and inside an already-running script (so a script that spawns `node` doesn't re-check).
...
The scan is deliberately conservative — it warns only on a package that is imported unguarded (a `try/catch` optional load is left alone), is absent from every dependency field, and is actually present in `node_modules`. It is warn-only and never changes the exit code.
