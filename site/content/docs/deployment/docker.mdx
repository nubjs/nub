...
In a pnpm project, copy `pnpm-lock.yaml` instead of `nub.lock`. Exclude `node_modules`, `.git`, and local `.env` files with `.dockerignore`.
...
<Callout>
Read the [full docs](https://docs.docker.com/build/building/multi-stage/) on docs.docker.com.
</Callout>
