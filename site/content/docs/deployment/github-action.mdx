...
[`nubjs/action/setup-node`](https://github.com/nubjs/action/tree/main/setup-node) takes the inputs and outputs of `actions/setup-node` and installs `nub` beside it:
...
```diff
  - uses: actions/setup-node@v4
    with:
      ...
      cache: npm
- - run: npm ci
+ - uses: nubjs/action/install@v0
  - run: npm test
```

Nub reads the lockfile unchanged, installs it into `node_modules`, and caches its store across runs, keyed on the lockfile. Node and the package managers on PATH are left alone. `working-directory` points the install at a subdirectory, `frozen-lockfile: false` lets it update the lockfile as `npm install` does, and `shim: true` adds the package-manager shims for later steps. The inputs are listed in [its README](https://github.com/nubjs/action/tree/main/install#inputs).
