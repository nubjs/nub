...
A frozen install reads `nub.lock`, not `package-lock.json`, so run [`nub pm migrate`](/docs/install/migrate) once and commit `nub.lock` first. The `npm test` step runs the real npm as before. Pass the same `node-version` to both actions: each fronts a Node on PATH, and without the input this action fronts the project's own pin. The `cache: npm` line goes because it restores npm's own cache, which Nub does not read; the action caches Nub's store by default.
