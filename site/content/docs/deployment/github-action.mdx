...
[`nubjs/nub/actions/setup-node`](https://github.com/nubjs/nub/tree/main/actions/setup-node) takes the inputs and outputs of `actions/setup-node` and installs `nub` beside it:
...
[`nubjs/nub/actions/install`](https://github.com/nubjs/nub/tree/main/actions/install) replaces the line that runs `npm ci`, `pnpm install --frozen-lockfile`, `yarn install --immutable` or `bun install --frozen-lockfile`, and changes nothing else:
...
```diff
  - uses: actions/setup-node@v4
    with:
      ...
      cache: npm
- - run: npm ci
+ - uses: nubjs/nub/actions/install@v0
  - run: npm test
```
