---
title: Cloudflare
description: Workers Builds preinstalls Nub. Cloudflare Pages uses the managed-builder devDependency pattern instead.
---

Cloudflare's [Workers Builds image](https://developers.cloudflare.com/workers/ci-cd/builds/build-image/) has included Nub since July 30, 2026. A package script can call `nub` or `nubx` without an install step:

```json title="package.json"
{
  "scripts": {
    "build": "nub run i18n:compile && nubx vite build"
  }
}
```

The binary is on `PATH` in Workers Builds. The change applies to the Workers image, not the separate Pages build image ([tracking issue](https://github.com/cloudflare/pages-build-image/issues/13)).

## Workers Builds

Cloudflare runs an optional build command and then a Wrangler deploy command. Point the build command at the package script that uses Nub; the preinstalled binary means no devDependency is required.

The image currently defaults to Node 24.18.0 and also preinstalls Node 22.23.2. Use `.node-version`, `.nvmrc`, or `.tool-versions` to select a version that both Cloudflare and Nub read.

<Callout>
Read the [full docs](https://developers.cloudflare.com/workers/ci-cd/builds/configuration/) on developers.cloudflare.com.
</Callout>

## Letting Nub run the install

Workers Builds installs dependencies itself before the build command runs, picking the package manager from the lockfile it finds. It does not yet recognize `nub.lock`, so in a Nub project it falls back to another manager, which installs the dependencies a second time and writes its own lockfile beside `nub.lock`.

Add a build variable so Cloudflare skips its install and Nub owns it:

```bash
SKIP_DEPENDENCY_INSTALL=true
```

Then install from the build command:

```bash
nub install && nub run build
```

A pnpm project is unaffected: Cloudflare installs with pnpm from `pnpm-lock.yaml`, and Nub reads the same file.

## HTTP endpoint

Workers use Cloudflare's `fetch` handler rather than a Node HTTP server. A minimal aliveness endpoint:

```ts title="src/index.ts"
export default {
  fetch() {
    return Response.json({
      alive: true,
      provider: "cloudflare",
      runtime: "cloudflare-workers",
    });
  },
};
```

Point Wrangler at the handler:

```json title="wrangler.jsonc"
{
  "$schema": "node_modules/wrangler/config-schema.json",
  "name": "nub-app",
  "main": "src/index.ts",
  "compatibility_date": "2026-07-30"
}
```

If the project defines a `build` package script, set **Settings → Build → Build command** to `nub run build`. Leave it empty for the standalone handler shown here. Workers Builds does not read custom-build settings from `wrangler.jsonc`; its default deploy command is `npx wrangler deploy`.

Wrangler bundles the TypeScript handler without a separate local build. Deploy it from a workstation with:

```bash
nubx -y wrangler deploy
curl --fail https://nub-app.ACCOUNT.workers.dev
# {"alive":true,"provider":"cloudflare","runtime":"cloudflare-workers"}
```

## Pages

Cloudflare Pages does not include Nub. Add the npm package as a devDependency so the install links both commands into `node_modules/.bin`:

```json title="package.json"
{
  "devDependencies": {
    "@nubjs/nub": "^{{NUB_VERSION}}"
  },
  "scripts": {
    "build": "nubx vite build"
  }
}
```

See [Managed builders](/docs/deployment#managed-builders) for the same pattern on other hosted build systems.

<Callout>
Read the [full docs](https://developers.cloudflare.com/pages/configuration/build-configuration/) on developers.cloudflare.com.
</Callout>

## Version pinning

Workers Builds provisions its tools with asdf, so a project-root `.tool-versions` pins Nub and Node together:

```text title=".tool-versions"
nodejs 24.18.0
nub {{NUB_VERSION}}
```

A pinned version the image doesn't already carry is downloaded during the build, so the pin holds even when it differs from the image default. Nub reads the same file when resolving its own Node, so one file pins both.

Adding `@nubjs/nub` as a devDependency also works, and pins the version in the lockfile alongside the project's other dependencies.

Nub only participates in the build. Deployed Workers execute on Cloudflare's runtime rather than Node with Nub augmentation.
