...
If the project has a build script such as the one above, set **Settings → Build → Build command** to `nub run build`. Leave it empty for the standalone handler shown here. Workers Builds does not read custom-build settings from `wrangler.jsonc`; its default deploy command is `npx wrangler deploy`.
