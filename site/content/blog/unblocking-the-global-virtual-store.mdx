...
my-app/
└── node_modules/
    ├── @firebase/database/          # ejected: hardlinked into the project
...
<Callout title="Two details make the eject sound">
  ...
  - **A fallback tree catches the rest.** The walk is back inside your project, but `@firebase/app` isn't at your top level either — *you* never declared it. So beside the ejected packages Nub writes one flat tree with an alias for every package in the graph, and the walk passes through it on the way up. Only ejected packages can reach it — a symlinked package's walk ascends the machine store, so the rest of your tree stays sealed.
</Callout>
