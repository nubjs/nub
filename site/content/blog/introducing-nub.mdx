...
description: The all-in-one Node.js toolkit — a Rust CLI that augments stock Node with TypeScript that just runs, a faster script runner, and a fast bin runner.
author: The Nub Team
date: 2026-06-15
---
...
Nub embraces it instead. It's a single Rust binary that transpiles your code and executes it on the stock `node` your project pins — so the runtime underneath *is* Node. On Node 26.7's own test suite, scored the way Deno's compatibility corpus scores it, it clears 98.4% of what real Node passes, versus 72.4% for Deno 2.9 and 68.5% for Bun 1.4. It combines:
...
One `paranoid: true` line turns every soft gate into a hard failure.
