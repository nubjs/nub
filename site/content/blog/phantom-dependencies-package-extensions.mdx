...
We've used Nub's built-in phantom detector to scan the 10,000 most downloaded packages on `npm` for undeclared dependencies. The resulting database covers 792 packages, including 650 missing from `@yarnpkg/extensions`, which undergirds the global virtual stores of pnpm v12 and Aube.
...
Here's the problem: it's woefully incomplete and minimally maintained. There have been just a handful of updates in the last three years. In our scan of the top 10,000 packages, we found **650 additional packages** with undeclared dependencies beyond the **142 already covered by Yarn's database**, bringing the total to **792 packages**.
