...
/// THE TABLE. Curated by nub; see the module docs for why a dependency cannot reach it.
///
/// Each entry records the package's OWN mechanism, because that is what bounds the grant:
/// a future version that writes somewhere else needs a new measurement, not a wider path.
