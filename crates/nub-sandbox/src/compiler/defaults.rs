...
mod tests {
    ...
    /// A cmd.exe ambient snapshot carries cmd's hidden `=C:`/`=ExitCode` per-drive entries.
    /// A `=`-named key cannot round-trip a `KEY=VALUE` block and the backend rejects one
    /// outright, so this pins that every posture builder DROPS them by allowlist rather than
    /// leaning on that reject.
    #[test]
    fn env_postures_drop_cmd_exe_shell_positional_keys() {
        ...
        let baseline = curated_baseline_env(&ambient);
        for posture in [&baseline, &strip_all_env(&ambient).constructed] {
            assert!(
