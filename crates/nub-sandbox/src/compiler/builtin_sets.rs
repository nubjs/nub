...
// THE HOST LIST IS DATA, NOT CODE. `DOWNLOAD_HOSTS` is the UNION of every catalog entry's
// `hosts`, generated by `build.rs` from `data/build-jail-catalog.json`. Per-package `hosts` is
// EVIDENCE rather than a per-package gate — egress itself is a boolean keyed on package
// identity (`compiler::package_network`) — so this union is what a policy author's
// `$downloads` expands to, and nothing narrower is enforced. `data/README.md` is the
// contributor-facing schema doc; the pre-migration per-host provenance is
// `wiki/research/build-jail-provenance.md`.
//
...
// far broader read-only surface, spanning every language ecosystem's package index. This one
// is the install-time artifact hosts a lifecycle script was actually observed to fetch.
...
mod tests {
    ...
    /// The four hosts that predate the catalog entirely, frozen as a literal rather than
    /// re-read from the JSON so both sides cannot agree on the same bad parse.
    ///
    /// A MEMBERSHIP check. The earlier form of this test pinned them as the first four
    /// entries in order, which was the right property while the catalog authored an ordered
    /// host array; the settled schema is a MAP keyed on package name, so there is no authored
    /// host order left to preserve and a prefix assertion would only be testing
    /// `BTreeSet`'s. Determinism — the thing that prefix check was actually protecting, since
    /// `download_net_rules` emits one rule per host in list order and the IR is compared
    /// byte-wise elsewhere — is now unconditional: the generator sorts.
    #[test]
    fn the_catalog_still_admits_the_pre_catalog_host_list() {
        for host in [
            "nodejs.org",
            "binaries.prisma.sh",
            "download.cypress.io",
            ...
            assert!(
                ...
                "`{host}` predates the catalog and was dropped by a schema round-trip"
            );
        }
    }

    /// The generator's ordering contract, asserted directly now that it is the ordering
    /// invariant rather than an authored sequence. Sorted-and-deduped is what keeps
    /// `download_net_rules`'s IR byte-stable across builds.
    #[test]
    fn the_download_host_set_is_sorted_and_deduplicated() {
        let mut sorted = DOWNLOAD_HOSTS.to_vec();
        sorted.sort_unstable();
        sorted.dedup();
        assert_eq!(
            DOWNLOAD_HOSTS,
            &sorted[..],
            "$downloads must be sorted and duplicate-free — the union is built from a BTreeSet, \
             so a divergence means the generator changed"
        );
    }
    ...
    /// The earlier form of this test BANNED every write-capable and multi-tenant host, then
    /// relaxed to "never admitted SILENTLY" — a host of that class had to carry a named
    /// `residual` string in the catalog. The settled schema has no per-host object left to
    /// hang one on, and the declaration was never enforcement anyway: what actually bounds
    /// `github.com`'s `git-receive-pack` or `registry.npmjs.org`'s publish route is that
    /// `defaults::lifecycle_scrubbed_env` withholds the whole credential family, so a confined
    /// script holds no credential to use them with. The exposures are recorded in
    /// `wiki/research/build-jail-provenance.md`, per host.
    ///
    /// What survives as a CHECK is the half that is a real structural property: the shapes no
    /// rationale admits.
    #[test]
    fn no_banned_host_shape_reached_downloads() {
        // A residual is not a blanket permit. These shapes stay refused whatever anyone
        // writes: a wildcard (DNS-label exfiltration), a container registry, or a telemetry
        // endpoint whose POST body IS the product. No corpus package needs any of them.
        for banned in [
            "*.googleapis.com",
            "*.amazonaws.com",
            "*.blob.core.windows.net",
