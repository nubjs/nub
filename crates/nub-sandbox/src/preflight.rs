...
mod platform {
    use super::Missing;
    use crate::backend::linux_setup;

    pub(super) fn diagnose() -> Option<Missing> {
        let candidates = crate::backend::bwrap_candidate_paths();
        if !candidates.iter().any(|path| path.is_file()) {
            return Some(Missing::Bubblewrap);
        }
        // The order matters. An unrestricted host with a Bubblewrap present needs nothing, and
        // saying otherwise would send a working machine to a setup it does not need.
        if !linux_setup::userns_restricted() {
            return None;
        }
        match linux_setup::helper_readiness() {
            linux_setup::HelperReadiness::Usable => None,
            linux_setup::HelperReadiness::NotInstalled => Some(Missing::NamespacePermission),
            linux_setup::HelperReadiness::SessionCannotExecute => Some(Missing::SessionGroup),
        }
    }
}
...
mod platform {
    ...
    pub(super) fn diagnose() -> Option<Missing> {
        if crate::backend::macos_setup::enforceable() {
            return None;
        }
