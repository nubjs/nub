...
Network entries accept host patterns and CIDRs. Unlike filesystem grants, network rules retain ordered allow/deny matching. A host grant is not an HTTP-method restriction and does not prevent uploads to that host. The boolean `false` denies egress; `true` requests unrestricted networking. Raw Windows sessions reject unrestricted networking with filesystem confinement. The explicit native adapter instead uses command-owned socket creation for this combination, without granting an AppContainer Internet capability. Unsupported combinations fail preparation rather than launch with reduced permissions.
...
- With `"net": true` and confined filesystem grants, ordinary IPv4/IPv6 TCP and UDP socket creation is handled by the owning process. A private pipe binds each request to a live member of that command's Job; the socket is transferred to that process for ordinary connect, listen and I/O operations. Deny-network and hostname-filtered policies do not receive this channel. The channel closes with the command, independently of retained session resources.
