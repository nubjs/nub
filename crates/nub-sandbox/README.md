...
Network entries accept host patterns and CIDRs. Unlike filesystem grants, network rules retain ordered allow/deny matching. A host grant is not an HTTP-method restriction and does not prevent uploads to that host. The boolean `false` denies egress; `true` requests unrestricted networking. Windows rejects that request when filesystem confinement requires AppContainer, whose networking capabilities do not provide unrestricted host access. Unsupported combinations fail preparation rather than launch with reduced permissions.
