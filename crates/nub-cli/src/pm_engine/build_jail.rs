...
//! - egress gated on PACKAGE IDENTITY: a package the build-jail catalog names reaches the
//!   install-time artifact hosts (`$downloads`); a package it does not name reaches NOTHING.
//!   That default is the point — the attack shape is a new `postinstall` published into a
//!   package that never had one, so an unvetted package gets no egress at all and one that
//!   needs it arrives through a catalog PR first. The fs axis carries NO deny rules at all —
