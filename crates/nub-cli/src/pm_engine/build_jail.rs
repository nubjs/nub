//! Nub's catalog-driven dependency lifecycle jail, applied through the shared sandbox engine.
//!
//! The catalog selects filesystem grants and coarse network access per package and platform.
//! Unlisted packages receive the catalog baseline. The compiler scrubs credential-shaped
//! environment keys and supplies private temporary and home directories.
//!
//! Root-authored scripts remain unconfined. Fetched dependency roots are confined to their
...
/// Catalog-driven policy and launch adapter for dependency lifecycle scripts.
#[derive(Debug)]
struct NubBuildJail {
...
/// Register the build jail before any dependency lifecycle scripts run.
...
impl aube_util::LifecycleSandbox for NubBuildJail {
    ...
    ///
    /// `package_name` IS load-bearing: the root project's `allowScripts: "no-jail"` unconfines one
    ...
    fn run(
        ...
        aube::commands::install::node_gyp_bootstrap::clear_legacy_registry_configs()
            .map_err(|e| std::io::Error::other(format!("preparing build-jail tool cache: {e}")))?;
        ...
        // the interpreter supporting `--import` (18.18+ and 19+) because an unrecognised option in
        ...
        if let Some(msvc) = super::jail_msvc::resolve(&ambient, &spawn, &probe) {
            ...
            if spawn.package_name.as_deref() == Some("cpu-features")
                && spawn.package_version.as_deref() == Some("0.0.10")
        ...
        #[cfg(unix)]
        {
            ...
            let status = child.wait_cancellable(&spawn.cancelled);
        ...
        {
            let launched = std::time::Instant::now();
            let status = prepared.status_cancellable(&spawn.cancelled);
            persist_declared_home_writes(&spawn);
            if let Ok(code) = &status
                && !code.success()
            {
...
fn refusal(detail: &str) -> String {
    format!(
        "{}  The unprivileged sandbox could not be applied on this host.\n\n{detail}\n",
        headline()
    )
}
...
///
/// ONE PACKAGE PER LINE so it stays scannable at any count, then a
...
pub(crate) fn report_jail_failures() {
    ...
    // the `allowScripts` MAP — there is no flag that writes it yet, and printing a command that does
    // not exist is worse than printing the edit that does.
    //
    // ⛔⛔ TOP-LEVEL `allowScripts`, NOT `pnpm.allowScripts` — AND THE BRANDED FORM IS WHAT SHIPPED HERE.
    ...
    let names = failures
        ...
        .join(", ");
    out.push_str(&format!(
        "  ╰─▶ run unconfined:  package.json  \"allowScripts\": {{ {names} }}\n"
    ));
...
fn write_jail_failure_log(root: &Path, failures: &[JailFailure]) -> Option<PathBuf> {
    ...
    body.push_str(
        "\nFIRST, FIND OUT WHETHER THE JAIL IS EVEN THE CAUSE. Re-run the install with the jail off;\n\
         ...
         adding it to allowScripts in package.json, and leave the rest confined:\n\n\
         \x20 \"allowScripts\": { \"<package>\": \"no-jail\" }\n\n\
...
/// The `allowScripts` value that runs one package's scripts UNCONFINED.
///
...
enum Unconfined {
    ...
    GlobalSwitch,
    /// The root project's `allowScripts: { "<pkg>": "no-jail" }` — this one package.
    RootOptOut,
}
...
fn unconfined_notice(name: &str, reason: Unconfined) -> String {
    ...
    let because = match reason {
        ...
        Unconfined::RootOptOut => {
            format!("the root build policy has \"{name}\": \"{NO_JAIL}\" in package.json")
        }
    };
...
fn unconfined_by(package_name: Option<&str>, project_root: &Path) -> Option<Unconfined> {
    ...
    // A ROOT-AUTHORED `allowScripts` value of `"no-jail"` runs THIS package's scripts unconfined.
    //
    ...
    //
    // WHY `allowScripts` RATHER THAN A NEW FIELD. Build approval already lives there; it is already a
...
/// Did the ROOT project ask for this one package to run unconfined, via `allowScripts: "no-jail"`?
///
...
fn root_opted_out_of_jail(project_root: &Path, package_name: &str) -> bool {
    ...
    // ⛔ EXACT PACKAGE IDENTITY ONLY, deliberately narrower than the RUN decision. `allowScripts` keys may
...
/// Does this `allowScripts` key name exactly this package — either bare, or qualified by a spec or
/// version after an `@`?
///
...
///
/// Orthogonal to `approveBuilds`/`allowScripts`, which decide WHETHER a script runs; this
...
/// the Windows stdio shim is delivered on (18.18.0 / 19.0.0). Unparseable ⇒ `false`: the
...
fn supports_import(version: &str) -> bool {
    semver::Version::parse(version).is_ok_and(|v| v.major >= 19 || (v.major == 18 && v.minor >= 18))
}
...
mod tests {
    ...
    fn only_a_node_that_accepts_import_gets_the_shim_stamp() {
        assert!(!supports_import("18.17.1"));
        assert!(supports_import("18.18.0"));
        assert!(supports_import("18.19.0"));
        assert!(supports_import("19.0.0"));
        ...
        assert!(!supports_import("20.6.invalid"));
        assert!(!supports_import("20.6"));
    }
    ...
    /// `allowScripts: "no-jail"` is a ROOT-authored, per-package request to run unconfined — and only
