...
///
/// KNOWN GAP, deliberate: a floor is a lower bound, so a range that STARTS below the
/// band and extends past it (`--target ">=22.15"` under `--smol`) still passes here
/// and can be run on 23.2. Closing that needs the gate to see the whole range rather
/// than its floor, which is a different change; the common shapes (an exact target, a
/// bare major, a `23.x` range) all floor inside the band and are caught.
