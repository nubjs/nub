...
fn main() {
    ...
    // exports __NUB_DOCS_KEY, a content hash of the docs tree, so the bake reruns
    // exactly when the docs differ from the sharer that produced it.
    // The double underscore keeps it out of the `NUB_*` set the brand-boundary
    // test counts in a script's environment: cargo hands its own env to every
    // test binary, and from there it reaches the scripts nub spawns.
    println!("cargo:rerun-if-env-changed=CARGO_MANIFEST_DIR");
    println!("cargo:rerun-if-env-changed=__NUB_DOCS_KEY");
