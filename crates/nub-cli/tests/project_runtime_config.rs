...
//! executable.
//!
//! The fixture config also carries a restrictive `sandbox` value on every route,
//! so the assertions below double as the proof that sandbox config stays inert
//! for the runtime: it never reaches the child's snapshot and never changes what
//! any entrypoint produces.
...
/// The observations every route shares: the config's `envFile`, its `define`
/// substitutions, its `loader` map, its `tsconfig` paths and JSX factory, and
/// its `conditions` all reached the child.
fn assert_config_applied(label: &str, value: &serde_json::Value) {
    assert_eq!(value["define"], "defined", "{label}: {value}");
...
"#,
    ...
    fn assert_probe(&self, args: &[&str]) {
        ...
        // `sandbox` rides alongside `install` and `dlx`: all three are
        // install/exec-time concerns the runtime transport must never carry, and
        // the fixture config sets `sandbox` precisely so this cannot pass vacuously.
...
/// Sandbox config appears in three separate positions, and none of them is a
/// runtime concern. A parent that carried any of them must still produce a child
/// that runs normally — the complement of `assert_probe`'s check that the
/// transport never emits them in the first place.
#[test]
fn inherited_runtime_snapshot_ignores_all_sandbox_config_positions() {
    ...
    assert!(
        output.status.success(),
        "sandbox fields in an inherited snapshot must deserialize inertly: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    assert_eq!(String::from_utf8_lossy(&output.stdout).trim(), "inherited");
}
