...
/// A `pnpm-lock.yaml` written as `importers: { .: {} }` while `package.json`
/// declares a dependency must read as drift (nubjs/nub#657). It used to
...
/// a green CI run with an empty `node_modules`. `pnpm install
/// --frozen-lockfile` rejects the same input, and so must this.
///
/// The `package-lock.json` shape this used to check alongside it is gone with
/// npm incumbency: nub reads no npm lockfile now, so `nub ci` there refuses
/// for want of one it understands and still links nothing — the failure this
/// guards cannot arise through that door.
///
/// The error CODE follows the identity rather than the tool: this fixture
/// carries a `pnpm-lock.yaml`, so the project is pnpm's and the refusal is
/// pnpm's own, verbatim. Both spellings are accepted while two package
/// managers are in the tree.
///
/// No network: the frozen drift check runs before any resolution.
#[test]
fn ci_rejects_a_lockfile_whose_root_importer_is_empty() {
    let dir = pm_tmpdir("pnpm-empty-importer");
    std::fs::write(
        dir.join("package.json"),
    ...
    assert_ne!(
        code, 0,
        "`nub ci` must reject a lockfile that resolves none of the manifest's \
         deps, not report success: {out}\n{err}"
    );
    // Pin the failure to the drift path so it cannot pass on an unrelated
    // network or store error.
    assert!(
        ["ERR_NUB_OUTDATED_LOCKFILE", "ERR_PNPM_OUTDATED_LOCKFILE"]
            .iter()
            .any(|code| err.contains(code)),
        "the rejection must be the outdated-lockfile error: {err}"
    );
    assert!(
        !dir.join("node_modules").join("is-odd").exists(),
        "nothing may be linked when the frozen install is rejected"
    );
}
