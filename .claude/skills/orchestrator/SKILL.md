...
3. **Dispatch scoped sub-agents**, tiered to the right model/effort, with four standing instructions in every prompt:
   ...
   - comments sparse + dense; **name any skill it must load** (`prose-writing`, `impact-analysis`, `ad-hoc-test`, `remote-build` — skills are NOT inherited), and say that heavy builds/tests and Linux-answerable fixture sweeps default to the VM (`remote-build`; `--job adhoc` for fixture scripts);
