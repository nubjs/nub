...
## Scope gate — is this even an implementation-thread? (read first)

**This skill describes the workup for a SUBSTANTIAL change. It is not the shape of every task.** A change confined to a file or two, whose correctness its own diff plus the pre-push loop demonstrates, does not get a design pass, an L2 fleet, or a multi-lens review — you write it, verify it, and open the PR. Invoking this machinery on a small fix costs more than the fix, and every L2 hop returns a claim you then have to re-verify yourself.

Use the full shape when the change crosses module boundaries, touches a default / security posture / public surface / serialized format, or has effects you can't hold in your head while reading the diff. Otherwise take only the phases you need — the phase list below is a menu sized to the change, not a checklist to complete. **The L2 counts throughout are ceilings for the substantial case, never targets.**
...
4. **Verify by RUNNING it — an ad-hoc fixture sweep against a built binary.** This is the step that finds real defects, and it is the one most often skipped in favor of another read of the diff. Build the binary with your change, drive the behavior you touched against real fixtures, and hunt for what you broke *somewhere you weren't looking* — not just the case you set out to fix. Build the sweep to FALSIFY: if every fixture passed on the first run, you tested your intent, not your change. Load the `ad-hoc-test` skill; `FRAY.md` §"SELF-REVIEW BY TESTING" is the canonical method. Promote the durable checks into committed tests. **Reviewers structurally cannot find the bugs that matter most here** — a reviewer reads code and hypothesizes, so it misses the SILENT WRONG ANSWER (the resolution that returns a different module with no error), which is only reachable by executing the thing and checking *which* file answered.

   **Then self-review the diff IN-THREAD — read your own.** You hold the richest context (why each choice was made, what you rejected), so your own read beats a fresh agent that must re-derive it. Spawning a dedicated fresh-context reviewer is an ESCALATION for a change that genuinely earns it — wide blast radius, security posture, serialized format, memory/UB-adjacent — not a default leg. When you do escalate, an `impact-analysis` pass (blast radius: every call site / reader-writer / impl / match-arm of a changed symbol, plus downstream behavioral/serialized/cross-process effects) is the most valuable lens to buy. Never let a review round substitute for the sweep.
...
- **Model tiering:** tier each L2 by the judgment its task actually needs, not by the fact that it belongs to an implementation-thread. A repro, a harvest, a grep-and-report, a doc edit, or a CI watch is Sonnet or Haiku work — or yours. Opus is for the fix that lands and for verdicts you'll act on. Every L2 prompt is SELF-CONTAINED (a fresh context carries nothing over), which is itself a cost: if packing the context takes longer than answering the question, answer it.
