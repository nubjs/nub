...
- **Keep the padding.** The renderer leaves 30px above the heading and below the axis labels, and 56px at either side of the ink (`PAD_Y` and `PAD_X` in `chart.mjs`). Without it the figure reads as cropped, which is obvious the moment it sits on a page or in a tweet rather than on a preview; with less at the sides it reads as cramped even when centered (rule 5).
