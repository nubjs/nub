...
- **The rolling `canary` prerelease is excluded** — it is recreated on every main push, so its counters reset and would corrupt the snapshot deltas. `nub upgrade --canary` downloads are therefore uncounted.
