...
# current — the exact failure the stamp exists to catch. Run from the pre-push
# hook over the pushed range, and from CI against the pull request's base.
#
...
git diff --quiet "$base" "$head" -- "$f" && exit 0     # untouched: nothing to check
