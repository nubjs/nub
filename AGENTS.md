...
- **`dynamic-churn`** — run ONE session, in series, through work bigger than a context: a continuously-rewritten task list that is the only memory surviving compaction, driven by frizz's own `recurring_prompt` (`stop_hook` + `post_compaction`). No hooks to install — the loop is armed by the agent. Covers how to write a task list that converges, and how the loop ends (`ALLDONE` pauses, `action: "stop"` disarms).
