...
- **Wait deliberately for sub-agent returns.** After dispatch, continue useful non-overlapping work. When returns are the only remaining dependency, state the active count and one-line lane summaries, then call one blocking `wait_agent` with a substantive timeout. Do not end the turn assuming a completion notification will start a new one, and never busy-poll with repeated short waits.
- **Never build a waiter for work the harness already tracks — and if you must poll, the predicate has to be able to FIRE.** Decide with this table before writing any wait:
  ...
  The harness's "use an until-loop" hint appears when it blocks a bare `sleep`; it applies to the last row only. When you must poll, never key the predicate on a `"type":"result"` record in an agent transcript — that record is not reliably written, so the loop can only time out. Keying on "last entry is an assistant message with no `tool_use`" is the mirror-image bug: it false-positives on a thinking-only block. A sound predicate needs a non-empty `text` block plus a staleness bound on the transcript mtime. Never arm a second waiter while the first is unfired.
- **A completed sub-agent always becomes an immediate progress update to the user** — what it checked, what it found, what changed because of it, what remains open. Do this even when the finding is "no issue found." Don't silently collect several returns for a later summary.
- **Reconcile sub-agent liveness after every interruption or user message.** Call the live-agent listing before resuming other work, compare against the expected lanes in the root scratchpad, report any missing child at once, then restart only the missing work.
- **Sub-agents never edit the root session's scratchpad.** The root owns its `.fray/threads/<session-id>/scratch.md`; a child returns findings through the sub-agent result channel. Repeat this boundary in every dispatch prompt.
...
- **If you're completing 10 tasks per hour, you're not testing them.** A thoughtfully implemented and tested task takes 15–30 minutes.
...
- **Dialing in the visuals is part of implementing UI.** For any rendered surface, invoke the `visual-review` skill *before* calling it correct — always when placing an icon, glyph, emoji, badge, chip, or counter next to text. Two rules carry most of the value: (a) **you are the first reviewer of your own screenshot** — read it back at a judgeable scale and hunt for what's wrong (a glyph riding high, one mark heavier than its neighbours, a collision at a narrow width); (b) **icon-beside-text alignment is an INK problem and every glyph differs** — `items-center` centers a glyph's box while the eye aligns ink, so one shared nudge cannot fix a cluster. Measure each glyph, correct each in `em`, re-measure until the residual is ~0. Where the pattern exists in a real product (GitHub, Linear, an existing component), measure the real one instead of designing from taste.
