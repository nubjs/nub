...
## Flags
- **`--no-bail`** — Continue recursive execution after a command fails.
...
- **`--parallel`** — Run recursive workspace executions concurrently.
- **`--report-summary`** — Write a recursive exec summary file.

  Parsed for pnpm compatibility.
- **`--reporter-hide-prefix`** — Hide the `<package>: ` label on parallel-exec output lines.
...
- **`-c --shell-mode`** — Run the command through `sh -c`.
