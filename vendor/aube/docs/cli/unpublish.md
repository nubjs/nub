...
## Arguments
- **`[SPEC]`** — Package spec: `name`, `name@version`, or omitted to use the current project's `package.json`.

## Flags
- **`--dry-run`** — Don't talk to the registry; print what the command would do.
- **`-f --force`** — Required for whole-package unpublish (no `@version` in the spec).
