...
## Flags
- **`--auth-type <TYPE>`** — Authentication flow: `legacy` (token paste; default) or `web` (OAuth flow against `{registry}/-/v1/login`).

  **Default:** `legacy`
- **`--scope <SCOPE>`** — Scope to bind this registry to (e.g. `@myorg`).
