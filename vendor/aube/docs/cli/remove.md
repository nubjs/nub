...
## Flags
- **`-D --save-dev`** — Remove only from devDependencies
- **`-g --global`** — Remove from the global install directory instead of the project
- **`--ignore-scripts`** — Skip root lifecycle scripts during the chained reinstall
- **`-w --workspace`** — Remove the dependency from the workspace root's `package.json`.
