...
## Flags
- **`--exit-code`** — Exit with a non-zero status if any deprecations are found.
- **`--json`** — Emit JSON instead of the default text layout.
- **`--transitive`** — Include transitive dependencies as well as direct ones.
- **`-h --help`** — Print help
