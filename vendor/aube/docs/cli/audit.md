...
## Flags
- **`--audit-level <AUDIT_LEVEL>`** — Only print advisories at or above this severity.
...
- **`--ignore-unfixable`** — Drop advisories that have no non-vulnerable upgrade.
...
- **`-i --interactive`** — Pick which advisories to fix interactively.
- **`--json`** — Emit the report as JSON (pnpm-compatible shape) instead of a table.
