...
- **`--no-save`** — Install without persisting the dependency to `package.json`.
...
- **`--no-save-workspace-protocol`** — Inverse of `--save-workspace-protocol`.
...
- **`--save-catalog`** — Save the new dependency into the workspace's default catalog.
...
- **`--save-catalog-name <NAME>`** — Save the new dependency into a *named* catalog.
...
- **`--save-peer`** — Add as a peer dependency (written to `peerDependencies` in package.json).
...
- **`--save-workspace-protocol`** — Force the manifest specifier into `workspace:` form for this invocation, overriding `saveWorkspaceProtocol` from the workspace yaml / `.npmrc` / env.
...
- **`-w --workspace`** — Add the dependency to the workspace root's `package.json`.
...
- **`-W --ignore-workspace-root-check`** — Allow `add` to run in a workspace root.
