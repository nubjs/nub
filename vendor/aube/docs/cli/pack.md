...
## Flags
- **`--dry-run`** — Don't write the tarball; print what would be packed
- **`--ignore-scripts`** — Skip `prepack` / `prepare` / `postpack` lifecycle scripts.
- **`--json`** — Print the result as a JSON object
- **`--pack-destination <DIR>`** — Directory to write the tarball into (default: current directory)
- **`-h --help`** — Print help
