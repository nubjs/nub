...
- **Usage:** `aube publish [FLAGS] [TARBALL|FOLDER]`
...
## Flags
- **`--access <LEVEL>`** — Publish as `public` or `restricted`.
...
- **`--dry-run`** — Don't upload; print what would be published.
- **`--force`** — Republish even when the version is already on the registry.
...
- **`--ignore-scripts`** — Skip publish lifecycle scripts.
...
- **`--no-git-checks`** — Skip the "working tree must be clean" check.
...
- **`--otp <CODE>`** — One-time password for registries that require 2FA.

  Sent verbatim as the `npm-otp` header.
- **`--provenance`** — Generate a SLSA provenance attestation and attach it to the publish body.
...
- **`--tag <TAG>`** — Default dist-tag to publish under (default: `latest`).
- **`-h --help`** — Print help
