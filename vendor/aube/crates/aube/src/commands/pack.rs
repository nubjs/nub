...
pub(crate) async fn run_root_lifecycle_script(
    ...
    // Every caller (pack / publish / version) acts on a package the user is
    // shipping from their own checkout. The git-dep prepare path never routes
    // here: it packs the prepared tree with `build_archive`, which runs no scripts.
