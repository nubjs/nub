...
fn test_pick_version_minimum_release_age_exclude_version_union() {
    ...
    let cutoff = "2020-01-01T00:00:00.000Z";
    let (rules, errs) = crate::trust::PackageVersionPolicy::parse_lossy(["axios@0.18.1 || 0.21.1"]);
    assert!(errs.is_empty(), "union pattern parses cleanly");
    let is_age_exempt = |ver: &str, parsed: Option<&node_semver::Version>| match parsed {
        Some(v) => rules.matches("axios", v),
        None => node_semver::Version::parse(ver)
            .map(|v| rules.matches("axios", &v))
            .unwrap_or_else(|_| rules.matches_name_only("axios")),
    };
...
fn test_pick_version_minimum_release_age_exclude_name_only() {
    ...
    let cutoff = "2020-01-01T00:00:00.000Z";
    let (rules, _) = crate::trust::PackageVersionPolicy::parse_lossy(["lodash"]);
    assert!(rules.matches_name_only("lodash"));
    let is_age_exempt = |ver: &str, parsed: Option<&node_semver::Version>| match parsed {
        Some(v) => rules.matches("lodash", v),
        None => node_semver::Version::parse(ver)
            .map(|v| rules.matches("lodash", &v))
            .unwrap_or_else(|_| rules.matches_name_only("lodash")),
    };
