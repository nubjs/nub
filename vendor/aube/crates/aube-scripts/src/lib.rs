...
fn apply_script_settings_env(cmd: &mut tokio::process::Command, settings: &ScriptSettings) {
    ...
    // lockstep. Both var names follow the embedder rather than the engine's
    // brand: a host's build jail withholds them BY NAME, and the shim was
    // rendered with the same prefix, so both ends must be composed the same way.
    // `…_NODE_GYP_PROJECT_DIR` pins the dir whose `.npmrc` the bootstrap
    // inherits; it is optional — the shim falls back to its own cwd when it is
    // absent, which for a dep hook would be the dep's package dir.
