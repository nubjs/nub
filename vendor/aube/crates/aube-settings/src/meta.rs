...
/// [`Embedder::unsupported_settings`](aube_util::Embedder::unsupported_settings),
/// and this is where that declaration takes effect for every enumerating
/// surface at once — `config list`, `config get`, the TUI, `config find`.
/// Identity for standalone aube, whose list is empty.
///
...
mod tests {
    ...
    /// The default profile declares nothing unsupported, so the filtered views
    /// are the raw table — this is the "byte-for-byte unchanged for standalone
    /// aube" claim, checked rather than asserted in a comment.
    ///
    /// It can only ever exercise the EMPTY list: the filter is driven by the
    /// active embedder, and this crate's tests run under aube's own profile. A
    /// host that populates the list tests the populated case on its own side.
    #[test]
    fn the_default_profile_filters_nothing() {
        assert!(aube_util::embedder().unsupported_settings.is_empty());
        assert_eq!(all().count(), all_unfiltered().len());
        for s in all_unfiltered() {
            assert!(is_supported(s.name), "{} filtered out unexpectedly", s.name);
            assert!(find(s.name).is_some(), "{} vanished from find()", s.name);
            assert!(
                unsupported_for_key(s.name).is_none(),
